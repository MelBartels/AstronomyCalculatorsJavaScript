...
// copyright Mel Bartels, 2016, 2017
...
MLB.telescopeCriteriaCalc.graphEquatorialTable = function () {
    ...
    */
    ...
    common.ETResults().html('latitude = '
            ...
            + ' deg<br>max yaw or twist = '
            + roundToDecimal(state.equatorialTable.platformMaxYawDeg, config.decimalPointsAngle)
            + ' deg<br>south tracking radius = '
            + roundToDecimal(frontViewSouthTrackArc.radius / state.scalingFactor, config.decimalPointsDimension)
            + uomLengthLit
            + '; bearing separation angle = '
            + roundToDecimal(frontViewSouthTrackArc.bearingAngleRad * 2 / uom.degToRad, config.decimalPointsAngle)
            + ' deg; arc width = '
            + roundToDecimal(frontViewSouthDrawnArc.width / state.scalingFactor, config.decimalPointsDimension)
            + uomLengthLit
            + '<br>north tracking radius = '
            + roundToDecimal(frontViewNorthTrackArc.radius / state.scalingFactor, config.decimalPointsDimension)
            + uomLengthLit
            + '; bearing separation angle = '
            + roundToDecimal(frontViewNorthTrackArc.bearingAngleRad * 2 / uom.degToRad, config.decimalPointsAngle)
            + ' deg; arc width = '
            + roundToDecimal(frontViewNorthDrawnArc.width / state.scalingFactor, config.decimalPointsDimension)
            + uomLengthLit
            + '<br>platform size = '
...
MLB.telescopeCriteriaCalc.processUomChange = function (startup) {
    ...
    common.lowriderSecondaryMirrorSizeLabel().html(config.lowriderSecondaryMirrorSizeLabelLit + uomDistanceLit + config.equalsLit);
    common.lowriderSecondaryOffsetLabel().html(config.lowriderSecondaryOffsetLabelLit + uomDistanceLit + config.equalsLit);
...
MLB.telescopeCriteriaCalc.writeSummary = function (startup) {
    ...
    // begin copy canvases...
    id = common.baffleCanvasID();
    common.summaryBaffleCanvasDiv().append(buildCanvasElement('summaryBaffleCanvas', id.width, id.height));
    common.summaryBaffleCanvasID().getContext('2d').drawImage(id, 0, 0);

    id = common.lowriderCanvasID();
    common.summaryLowriderCanvasDiv().append(buildCanvasElement('summaryLowriderCanvas', id.width, id.height));
    common.summaryLowriderCanvasID().getContext('2d').drawImage(id, 0, 0);

    id = common.rockerCanvasID();
    common.summaryRockerCanvasDiv().append(buildCanvasElement('summaryRockerCanvas', id.width, id.height));
    common.summaryRockerCanvasID().getContext('2d').drawImage(id, 0, 0);

    id = common.flexRockerCanvasID();
    common.summaryFlexRockerCanvasDiv().append(buildCanvasElement('summaryFlexRockerCanvas', config.baffleCanvasWidth, config.baffleCanvasHeight));
    common.summaryFlexRockerCanvasID().getContext('2d').drawImage(id, 0, 0);
    // ...end copy canvases
