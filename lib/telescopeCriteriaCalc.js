...
MLB.telescopeCriteriaCalc.writeSummary = function (startup) {
	...
	offsets = calcDiagOffset5(diagSize, focalPlaneToDiagDistance, common.convertMmToUom(eyepieceFieldStopmm), focalRatio);
	common.summaryDiagOffset().html(roundToDecimal(offsets.fieldDiaOffset, config.decimalPointsDiag)
        + uomLengthLit
        + ' at the focal point to '
        + roundToDecimal(offsets.focalPlaneOffset, config.decimalPointsDiag)
        + uomLengthLit
