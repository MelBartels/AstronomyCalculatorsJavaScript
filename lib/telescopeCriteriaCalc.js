...
MLB.telescopeCriteriaCalc.graphFoldingReflector = function () {
	...
	if (foldingSecondaryMirrorSize < focalPlaneToSecondaryDistance / focalRatio) {
		...
		context.fillText('Secondary too small or focal plane to secondary distance too long.', 0, canvas.height / 2);
		return;
	}
	if (isNaN(model.elbowAngleDeg)) {
		context.fillStyle = config.canvasErrorStyle;
		context.fillText('Focal point to folding secondary mirror distance too long, or, focal point offset from edge of primary mirror too long.', 0, canvas.height / 2);
		return;
	}
    ...
    focuserBarreltubeLowerFrontPt = point(focuserBarrelBottomMidPt.x + xDelta, focuserBarrelBottomMidPt.y - yDelta);
    focuserBarreltubeLowerRearPt = point(focuserBarrelBottomMidPt.x - xDelta, focuserBarrelBottomMidPt.y + yDelta);
    focuserBarreltubeUpperFrontPt = point(focuserBarrelTopMidPt.x + xDelta, focuserBarrelTopMidPt.y - yDelta);
    focuserBarreltubeUpperRearPt = point(focuserBarrelTopMidPt.x - xDelta, focuserBarrelTopMidPt.y + yDelta);
