...
MLB.telescopeCriteriaCalc.graphFoldingReflector = function () {
	...
	if (foldingSecondaryMirrorSize < focalPlaneToSecondaryDistance / focalRatio) {
    ...
    diagUpperRightPt = point(diagPt.x + xDelta, diagPt.y - yDelta);
    diagLowerLeftPt = point(diagPt.x - xDelta, diagPt.y + yDelta);
    ...
    */
	// top baffle
	xDelta = diagUpperRightPt.x - focalPlaneLeftPt.x;
	yDelta = diagUpperRightPt.y - focalPlaneLeftPt.y;
	...
	diagUpperToLeftFocalPlaneReflectedCanvasAngleRad = uom.oneRev - diagUpperToLeftFocalPlaneReflectedAngleRad;
	diagUpperToTubeUpperInsideYDelta = diagUpperRightPt.y - tubeIDUpperY;
    tanDiagUpperToLeftFocalPlaneReflectedAngle = Math.tan(uom.halfRev - diagUpperToLeftFocalPlaneReflectedAngleRad);
    diagUpperToTubeUpperInsideXDelta = diagUpperToTubeUpperInsideYDelta / tanDiagUpperToLeftFocalPlaneReflectedAngle;
	primaryMirrorBaffleLowerIntersectPt = point(diagUpperRightPt.x - diagUpperToTubeUpperInsideXDelta, diagUpperRightPt.y - diagUpperToTubeUpperInsideYDelta);
	// bottom baffle
	xDelta = diagLowerLeftPt.x - focalPlaneRightPt.x;
	yDelta = diagLowerLeftPt.y - focalPlaneRightPt.y;
    ...
    diagLowerToRightFocalPlaneReflectedCanvasAngleRad = uom.oneRev - diagLowerToRightFocalPlaneReflectedAngleRad;
	diagLowerToTubeUpperInsideYDelta = diagLowerLeftPt.y - tubeIDLowerY;
    tanDiagLowerToRightFocalPlaneReflectedAngle = Math.tan(uom.halfRev - diagLowerToRightFocalPlaneReflectedAngleRad);
    diagLowerToTubeLowerInsideXDelta = diagLowerToTubeUpperInsideYDelta / tanDiagLowerToRightFocalPlaneReflectedAngle;
	primaryMirrorBaffleUpperIntersectPt = point(diagLowerLeftPt.x - diagLowerToTubeLowerInsideXDelta, diagLowerLeftPt.y - diagLowerToTubeUpperInsideYDelta);
	// longest baffle 'x'
    ...
    if (config.drawTestLines) {
		...
		context.strokeStyle = config.canvasTestStyle;
		context.beginPath();
		context.moveTo(diagUpperRightPt.x, diagUpperRightPt.y);
		context.lineTo(diagUpperRightPt.x + config.canvasTestLineLength * Math.cos(diagUpperToLeftFocalPlaneReflectedCanvasAngleRad), diagUpperRightPt.y + config.canvasTestLineLength * Math.sin(diagUpperToLeftFocalPlaneReflectedCanvasAngleRad));
		context.stroke();
		context.beginPath();
		context.moveTo(diagLowerLeftPt.x, diagLowerLeftPt.y);
		context.lineTo(diagLowerLeftPt.x + config.canvasTestLineLength * Math.cos(diagLowerToRightFocalPlaneReflectedCanvasAngleRad), diagLowerLeftPt.y + config.canvasTestLineLength * Math.sin(diagLowerToRightFocalPlaneReflectedCanvasAngleRad));
		context.stroke();
		a = (180 + model.elbowAngleDeg / 2) * uom.degToRad;
		context.beginPath();
		context.moveTo(diagUpperRightPt.x, diagUpperRightPt.y);
		context.lineTo(diagUpperRightPt.x + config.canvasTestLineLength * Math.cos(a), diagUpperRightPt.y + config.canvasTestLineLength * Math.sin(a));
		context.stroke();
		context.beginPath();
		context.moveTo(diagLowerLeftPt.x, diagLowerLeftPt.y);
		context.lineTo(diagLowerLeftPt.x + config.canvasTestLineLength * Math.cos(a), diagLowerLeftPt.y + config.canvasTestLineLength * Math.sin(a));
		context.stroke();
	}
	// draw in order: light rays, optics, structure, baffles
    ...
    drawLine(context, config.canvasGlassStyle, config.canvasLineWidth, mirrorUpperBackPt, mirrorLowerBackPt);
    // draw diagonal
    drawLine(context, config.canvasGlassStyle, config.canvasLineWidth, diagUpperRightPt, diagLowerLeftPt);
    // draw focuser barrel tube
    drawLine(context, config.canvasStructureStyle, config.canvasLineWidth, focuserBarrelTubeLowerLeftPt, focuserBarrelTubeLowerRightPt);
    drawLine(context, config.canvasStructureStyle, config.canvasLineWidth, focuserBarrelTubeLowerRightPt, focuserBarrelTubeUpperRightPt);
    drawLine(context, config.canvasStructureStyle, config.canvasLineWidth, focuserBarrelTubeUpperRightPt, focuserBarrelTubeUpperLeftPt);
    ...
    drawLine(context, config.canvasBaffleStyle, config.canvasLineWidth, diagUpperRightPt, focalPlaneLeftPt);
    drawLine(context, config.canvasBaffleStyle, config.canvasLineWidth, diagUpperRightPt, primaryMirrorBaffleLowerIntersectPt);
    drawLine(context, config.canvasBaffleStyle, config.canvasLineWidth, diagLowerLeftPt, focalPlaneRightPt);
    drawLine(context, config.canvasBaffleStyle, config.canvasLineWidth, diagLowerLeftPt, primaryMirrorBaffleUpperIntersectPt);
	// draw focal plane
    drawLine(context, config.canvasGlassStyle, config.canvasLineWidth, focalPlaneLeftPt, focalPlaneRightPt);
    ...
    diagonalString = 'bending angle = '
        ...
        + roundToDecimal(model.elbowAngleDeg / 2, config.decimalPointsDimension)
        + ' deg';
