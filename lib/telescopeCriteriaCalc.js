...
MLB.telescopeCriteriaCalc.graphDiagIllum = function () {
    ...
    minDiagWithOffset = state.minDiag + offset / focalLen / aperture;
    state.minDiagWithOffset = minDiagWithOffset;
    common.diagResults().html('minimum m.a. size = '
            + roundToDecimal(state.minDiag, config.decimalPointsDiag)
            + uomLengthLit
            + ', '
            + roundToDecimal(minDiagWithOffset, config.decimalPointsDiag)
            + uomLengthLit
