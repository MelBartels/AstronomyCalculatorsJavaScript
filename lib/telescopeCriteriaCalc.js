...
MLB.telescopeCriteriaCalc.common = {
    ...
    foldingReflectorCanvasDiv: function () {
        return $('#foldingReflectorCanvasDiv');
    },
    ...
    summaryFoldingReflectorCanvasDiv: function () {
        return $('#summaryFoldingReflectorCanvasDiv');
    },
    ...
    summaryFoldingReflectorCanvasID: function () {
        return $('#summaryFoldingReflectorCanvas')[0];
    },
...
MLB.telescopeCriteriaCalc.graphRocker = function () {
    ...
    padHeight = (rockerSideBoardRect.y - bearingArcRightPt.y) / state.scalingFactor;
    ...
    materialArea.bearingBoard = altBearingSeparation * padHeight;
    ...
    materialCG.momentArm.bearingBoard = materialArea.bearingBoard * (padHeight / 2 + rockerSideHeight + woodThickness);
    ...
    rocker.height = rockerSideHeight + rocker.baseBoardThickness + padHeight;
    ...
    text = config.bearingHeightText + roundToDecimal(padHeight, config.decimalPointsDimension) + uomLengthLit;
...
MLB.telescopeCriteriaCalc.graphEquatorialTable = function () {
    ...
    // get combined CG: show work in progress
    platform.CG = {};
    platform.CG.weight = {};
    ...
    platform.CG.CG = {};
    platform.CG.OTA = state.CG.heightToClearRockerBottomDistance + state.rocker.baseBoardThickness + platform.thickness;
    platform.CG.rocker = state.rocker.CG / 2 + platform.thickness;
    ...
    // flex rocker...
    // equatorial platform sectors will be attached directly to the base ring, so no separate platform piece to act as ground board
    flexRockerPlatform.CG = {};
    flexRockerPlatform.CG.weight = {};
    flexRockerPlatform.CG.weight.OTA = common.tubeWeightVal();
    // mlb wip
    //flexRockerPlatform.CG.weight.flexRocker = ;
    //flexRockerPlatform.CG.weight.baseRing = ;
    flexRockerPlatform.CG.CG = {};

    // write results
    common.ETResults().html('latitude = '
            + common.ETLatitudeDegVal()
            + '<br> deg; tracking time = '
            + common.ETTrackingTimeMinVal()
            + '<br> min; max tilt = '
            + roundToDecimal(state.equatorialTable.platformMaxTiltDeg, 1)
            + '<br> deg; max yaw or twist = '
            + roundToDecimal(state.equatorialTable.platformMaxYawDeg, 1)
            + '<br> deg; platform weight = '
...
MLB.telescopeCriteriaCalc.writeSummary = function (startup) {
    ...
    common.summaryBaffleCanvasID().getContext('2d').drawImage(common.baffleCanvasID(), 0, 0);
    ...
    common.summaryLowriderFoldingMirrorSize().html(roundToDecimal(common.foldingSecondaryMirrorSizeVal(), config.decimalPointsDimension)
            ...
            + roundToDecimal(state.lowRiderModel.diagMajorAxisSize, config.decimalPointsDimension)
            + uomLengthLit);
    common.summaryFoldingReflectorCanvasID().getContext('2d').drawImage(common.foldingReflectorCanvasID(), 0, 0);
    ...
    common.summaryAzPadSize().html(roundedPadSizeSideUom
            ...
            + roundedPadSizeSideUom
            + uomLengthLit);
    common.summaryRockerCanvasID().getContext('2d').drawImage(common.rockerCanvasID(), 0, 0);
    ...
    common.summaryFlexRockerAzPadSize().html(roundedFlexRockerPadSizeSideUom
            ...
            + roundedFlexRockerPadSizeSideUom
            + uomLengthLit);
    common.summaryFlexRockerCanvasID().getContext('2d').drawImage(common.flexRockerCanvasID(), 0, 0);
...
$(window).ready(function () {
    ...
    */
    ...
    // build canvases
    common.baffleCanvasDiv().append(buildCanvasElement('baffleCanvas', config.canvasWidth, config.canvasHeight));
    common.foldingReflectorCanvasDiv().append(buildCanvasElement('foldingReflectorCanvas', config.canvasWidth, config.canvasHeight));
    common.rockerCanvasDiv().append(buildCanvasElement('rockerCanvas', config.canvasWidth, config.canvasHeight));
    common.flexRockerCanvasDiv().append(buildCanvasElement('flexRockerCanvas', config.canvasWidth, config.canvasHeight));
    common.summaryBaffleCanvasDiv().append(buildCanvasElement('summaryBaffleCanvas', config.canvasWidth, config.canvasHeight));
    common.summaryFoldingReflectorCanvasDiv().append(buildCanvasElement('summaryFoldingReflectorCanvas', config.canvasWidth, config.canvasHeight));
    common.summaryRockerCanvasDiv().append(buildCanvasElement('summaryRockerCanvas', config.canvasWidth, config.canvasHeight));
    common.summaryFlexRockerCanvasDiv().append(buildCanvasElement('summaryFlexRockerCanvas', config.canvasWidth, config.canvasHeight));
