...
MLB.telescopeCriteriaCalc.state = {
    ...
    etendue: undefined,
	vignettedEtendue: undefined,
    radiance: undefined,
	radianceVignettedEtenduePerWeight: undefined,
    eyeOptRowSet: [],
	offaxisMask: {},
    scalingFactor: undefined,
    mirrorFrontEdgeToFocalPlaneDistance: undefined,
    tubeBackEndToFocalPlaneDistance: undefined,
	projectedFocuserBaffleTiltAngleDeg: undefined,
	projectedFocuserBaffleTiltedDistance: undefined,
	projectedFocuserBaffleFlatDistance: undefined,
	baffle: {},
    CG: {},
	friction: {},
...
MLB.telescopeCriteriaCalc.config = {
	...
	radianceEtenduePerWeightLit: ' cm^2deg^2/kg',
    equalsLit: ' = ',
...
MLB.telescopeCriteriaCalc.common = {
	...
	summaryEtendue: function () {
        return $('[id=summaryEtendue]');
    },
	summaryVignettedEtendue: function () {
        return $('[id=summaryVignettedEtendue]');
    },
	summaryVignettedEtendueRadiancePerWeight: function () {
        return $('[id=summaryVignettedEtendueRadiancePerWeight]');
    },
...
MLB.telescopeCriteriaCalc.calcVignettedIllumPercent = function () {
    var common = MLB.telescopeCriteriaCalc.common,
		...
		offAxisIncrement = common.imperial() ? 0.1 : 2,
...
MLB.telescopeCriteriaCalc.graphDiagIllumSubr = function (chart, diagonals) {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        ...
        diagObstructionArea = MLB.calcLib.diagObstructionArea,
        getDiagIllumArray = MLB.calcLib.getDiagIllumArray,
        calcDiagOffset5 = MLB.calcLib.calcDiagOffset5,
    ...
    offsets = calcDiagOffset5(diagSize, focalPlaneToDiagDistance, maxField, focalRatio);
	// offsets are negative by convention...
    minDiagWhenOffset = minDiag + -offsets.focalPlaneOffset / (parseFloat(focalLen) / parseFloat(aperture));
    ...
    common.diagResults().html('minimum m.a. size = '
        ...
        + uomLengthLit
        + ' when offset;<br>offset towards primary mirror varies from '
        + roundToDecimal(offsets.fieldDiaOffset, config.decimalPointsDiag)
        + uomLengthLit
        + ' at the focal point to '
        + roundToDecimal(offsets.focalPlaneOffset, config.decimalPointsDiag)
        + uomLengthLit
        + ' when emcompassing the field of view.');

    common.optimizedDiagSize().val(roundToDecimal(suitableDiags[0], config.decimalPointsDiag));
    common.diagOffset().val(roundToDecimal(offsets.fieldDiaOffset, config.decimalPointsDiag));
...
MLB.telescopeCriteriaCalc.graphBaffles = function () {
    ...
    */
	...
	state.projectedFocuserBaffleTiltAngleDeg = increasedReflectionAngleRad / uom.degToRad;
	state.projectedFocuserBaffleTiltedDistance = Math.sqrt(angledLengthX * angledLengthX + angledLengthY * angledLengthY) / state.scalingFactor;
	state.projectedFocuserBaffleFlatDistance = (projectedFocuserBaffleRightPt.x - projectedFocuserBaffleLeftPt.x - angledLengthX * 2) / state.scalingFactor;
    ...
    // write out notes
	vignettedEtendue = state.etendue * calcVignettedIllumPercent();
	state.vignettedEtendue = vignettedEtendue;

    common.baffleResults().html('projected focuser baffle tilt angle = '
		+ roundToDecimal(state.projectedFocuserBaffleTiltAngleDeg, config.decimalPointsDimension)
		+ ' deg<br>projected focuser baffle tilted length = '
		+ roundToDecimal(state.projectedFocuserBaffleTiltedDistance, config.decimalPointsDimension)
		+ uomLengthLit
		+ '<br>projectedfocuser baffle flat length = '
		+ roundToDecimal(state.projectedFocuserBaffleFlatDistance, config.decimalPointsDimension)
		+ uomLengthLit
		+ '<br>mirror sagitta = '
		+ roundToDecimal(sagitta, config.decimalPointsDimension)
		+ uomLengthLit
		+ '<br>vignetted etendue = '
		+ roundToDecimal(vignettedEtendue, config.decimalPointsEtendue)
		+ config.etendueParensLit);
};
...
MLB.telescopeCriteriaCalc.graphLowRider = function () {
    ...
    */
    ...
    primaryMirrorBaffleUpperRearPt = point(telescopeTubeRect.x, tubeRearEndFrontCenterPt.y - scaledHalfTubeID);
    primaryMirrorBaffleUpperRearTopPt = point(telescopeTubeRect.x, primaryMirrorBaffleUpperRearPt.y - scaledTelescopeTubeThickness);
    ...
    text = config.primaryMirrorToFocalPlaneDimensionText + roundToDecimal(model.focalPointToPrimaryMirrorDistance, config.decimalPointsDimension) + uomLengthLit;
    dimensionY += 4 * config.canvasDimensionHalfHeight;
    ...
    text = config.primaryMirrorToFoldingMirrorText + roundToDecimal(model.diagToPrimaryMirrorDistance, config.decimalPointsDimension) + uomLengthLit;
    dimensionY += 4 * config.canvasDimensionHalfHeight;
    ...
    // write mirror front edge to tube front dimension
    text = config.primaryMirrorToTubeEndDimensionText + roundToDecimal((focuserBaffleRightPt.x - mirrorLowerFacePt.x) / state.scalingFactor, config.decimalPointsDimension) + uomLengthLit;
    dimensionY += 4 * config.canvasDimensionHalfHeight;
...
MLB.telescopeCriteriaCalc.updateFrictionOfMovement = function () {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        ...
        radianceVignettedEtenduePerWeight = state.vignettedEtendue * state.radiance / common.convertUomToKg(totalWeight);
	...
	state.radianceVignettedEtenduePerWeight = radianceVignettedEtenduePerWeight;
    common.frictionOfMovementResults().html('Altitude friction of movement at eyepiece = '
        ...
        + roundToDecimal(totalWeight, config.decimalPointsCG)
        + uomWeightLit
        + '<br>vignetted etendue-radiance-weight = '
        + roundToDecimal(radianceVignettedEtenduePerWeight, config.decimalPointsRadiance) + config.radianceEtenduePerWeightLit);
};

MLB.telescopeCriteriaCalc.updateFrictionOfMovementStrategy = function (startup) {
...
MLB.telescopeCriteriaCalc.calcCGStrategy = function (startup) {
    var calcCG = MLB.telescopeCriteriaCalc.calcCG,
        updateFrictionOfMovementStrategy = MLB.telescopeCriteriaCalc.updateFrictionOfMovementStrategy;

	calcCG();
	updateFrictionOfMovementStrategy(startup);
};

MLB.telescopeCriteriaCalc.updateBafflesStrategy = function (startup) {
    var graphBaffles = MLB.telescopeCriteriaCalc.graphBaffles,
        ...
        calcCGStrategy = MLB.telescopeCriteriaCalc.calcCGStrategy;

	graphBaffles();
	graphLowRider();
	updateCGDistances();
	calcCGStrategy(startup);
};

MLB.telescopeCriteriaCalc.updateDiagIllumStrategy = function (startup) {
    var graphDiagIllum = MLB.telescopeCriteriaCalc.graphDiagIllum,
        updateBafflesStrategy = MLB.telescopeCriteriaCalc.updateBafflesStrategy;

	graphDiagIllum();
	updateBafflesStrategy(startup);
};
...
MLB.telescopeCriteriaCalc.updateFollowOnFieldsStrategy = function (startup) {
    var updateFieldsDependentOnAperture = MLB.telescopeCriteriaCalc.updateFieldsDependentOnAperture,
		...
		updateDiagIllumStrategy = MLB.telescopeCriteriaCalc.updateDiagIllumStrategy;

    updateFieldsDependentOnAperture();
    updateTelescopeResults();
    updateEyepieceOptimizerRows();
	updateDiagIllumStrategy(startup);
};
...
MLB.telescopeCriteriaCalc.writeSummary = function (startup) {
	...
	common.summaryEtendue().html(roundToDecimal(state.etendue, config.decimalPointsEtendue) + config.etendueLit);
	common.summaryVignettedEtendue().html(roundToDecimal(state.vignettedEtendue, config.decimalPointsEtendue) + config.etendueLit);
	common.summaryVignettedEtendueRadiancePerWeight().html(roundToDecimal(state.radianceVignettedEtenduePerWeight, config.decimalPointsRadiance) + config.radianceEtenduePerWeightLit);

	common.summaryAperture().html(common.apertureVal() + uomLengthLit);
	common.summaryFocalRatio().html(common.focalRatioVal());
	...
	common.summaryOptimizedEyepieceFieldStop().html(common.eyepieceFieldStopmmVal() + config.mmLitNS);
	...
	common.summaryDiagonalSize().html(common.optimizedDiagSizeVal() + uomLengthLit);
	common.summaryDiagOffset().html(common.diagOffsetVal() + uomLengthLit);
...
$(window).ready(function () {
    ...
    // if .click(xyz) then event passed to function which we don't want here because function has optional parms
