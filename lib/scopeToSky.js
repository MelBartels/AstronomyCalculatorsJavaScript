...
MLB.scopeToSky.scopeToSkyState = {
	...
	encoders: [new MLB.encoderLib.encoder(), new MLB.encoderLib.encoder()],

	inputEncoderMinCounts: function () {
		return [$('input[name=encoderMinCount1]'), $('input[name=encoderMinCount2]')];
	},
	inputEncoderMaxCounts: function () {
		return [$('input[name=encoderMaxCount1]'), $('input[name=encoderMaxCount2]')];
	},
	inputEncoderGearReductions: function () {
		return [$('input[name=encoderGearReduction1]'), $('input[name=encoderGearReduction2]')];
	},
	inputEncoderAngleOffsetDegs: function () {
		return [$('input[name=encoderAngleOffsetDeg1]'), $('input[name=encoderAngleOffsetDeg2]')];
	},
	inOutEncoderCurrentCounts: function () {
		return [$('input[name=encoderCurrentCount1]'), $('input[name=encoderCurrentCount2]')];
	},
	inOutEncoderLastCounts: function () {
		return [$('input[name=encoderLastCount1]'), $('input[name=encoderLastCount2]')];
	},
	inOutEncoderRotations: function () {
		return [$('input[name=encoderRotations1]'), $('input[name=encoderRotations2]')];
	},
	outEncoderDirections: function () {
		return [$('input[name=encoderDirection1]'), $('input[name=encoderDirection2]')];
	},
	outEncoderAngleDegs: function () {
		return [$('input[name=encoderAngleDeg1]'), $('input[name=encoderAngleDeg2]')];
	},
	outEncoderGearAngleDegs: function () {
		return [$('input[name=encoderGearAngleDeg1]'), $('input[name=encoderGearAngleDeg2]')];
	},
	outEncoderGearAngleWithOffsetDegs: function () {
		return [$('input[name=encoderGearAngleWithOffsetDeg1]'), $('input[name=encoderGearAngleWithOffsetDeg2]')];
	},
...
MLB.scopeToSky.updateTrackingRates = function (rates) {
	var outputScopeAzimuthRate = $('td[id=azRate]'),
		...
		outputScopeAzFiveMinuteError = $('td[id=fiveMinuteAzError]'),
		outputScopeAltFiveMinuteError = $('td[id=fiveMinuteAltError]'),
		scopeToSkyState = MLB.scopeToSky.scopeToSkyState,
	...
	// apparent errors
	outputScopeAzFiveMinuteError.html(roundToDecimal(rates.fiveMinuteAzError / uom.arcsecToRad * Math.cos(rates.initialAlt), rateDecimals));
	outputScopeAltFiveMinuteError.html(roundToDecimal(rates.fiveMinuteAltError / uom.arcsecToRad, rateDecimals));
};
...
MLB.scopeToSky.writeEncoderValues = function (encoder, ix) {
	var uom = MLB.sharedLib.uom,
	    roundToDecimal = MLB.sharedLib.roundToDecimal,
		scopeToSkyState = MLB.scopeToSky.scopeToSkyState,
		inOutEncoderCurrentCounts = scopeToSkyState.inOutEncoderCurrentCounts,
		inOutEncoderLastCounts = scopeToSkyState.inOutEncoderLastCounts,
		inOutEncoderRotations = scopeToSkyState.inOutEncoderRotations,
		outEncoderDirections = scopeToSkyState.outEncoderDirections,
		outEncoderAngleDegs = scopeToSkyState.outEncoderAngleDegs,
		outEncoderGearAngleDegs = scopeToSkyState.outEncoderGearAngleDegs,
		outEncoderGearAngleWithOffsetDegs = scopeToSkyState.outEncoderGearAngleWithOffsetDegs,
		positionDecimals = scopeToSkyState.positionDecimals;

	inOutEncoderCurrentCounts()[ix].val(encoder.currentCount);
	inOutEncoderLastCounts()[ix].val(encoder.lastCount);
	inOutEncoderRotations()[ix].val(encoder.rotations);
	outEncoderDirections()[ix].val(encoder.direction);
	outEncoderAngleDegs()[ix].val(roundToDecimal(encoder.encoderAngle / uom.degToRad, positionDecimals));
	outEncoderGearAngleDegs()[ix].val(roundToDecimal(encoder.gearAngle / uom.degToRad, positionDecimals));
	outEncoderGearAngleWithOffsetDegs()[ix].val(roundToDecimal(encoder.gearAngleWithOffset / uom.degToRad, positionDecimals));
};

MLB.scopeToSky.updateAnEncoder = function (encoder, ix) {
	var uom = MLB.sharedLib.uom,
	    getReduction = MLB.sharedLib.getReduction,
	    update = MLB.encoderLib.update,
		writeEncoderValues = MLB.scopeToSky.writeEncoderValues,
		scopeToSkyState = MLB.scopeToSky.scopeToSkyState,
		inputEncoderMinCounts = scopeToSkyState.inputEncoderMinCounts,
		inputEncoderMaxCounts = scopeToSkyState.inputEncoderMaxCounts,
		inputEncoderGearReductions = scopeToSkyState.inputEncoderGearReductions,
		inputEncoderAngleOffsetDegs = scopeToSkyState.inputEncoderAngleOffsetDegs,
		inOutEncoderCurrentCounts = scopeToSkyState.inOutEncoderCurrentCounts,
		inOutEncoderLastCounts = scopeToSkyState.inOutEncoderLastCounts,
		inOutEncoderRotations = scopeToSkyState.inOutEncoderRotations;

	encoder.minCount = +inputEncoderMinCounts()[ix].val();
	encoder.maxCount = +inputEncoderMaxCounts()[ix].val();
	encoder.totalCounts = encoder.maxCount - encoder.minCount + 1;
	encoder.gearReduction = getReduction(inputEncoderGearReductions()[ix].val());
	encoder.offsetAngle = +inputEncoderAngleOffsetDegs()[ix].val() * uom.degToRad;

	encoder.lastCount = +inOutEncoderLastCounts()[ix].val();
	encoder.rotations = +inOutEncoderRotations()[ix].val();

	update(encoder, +inOutEncoderCurrentCounts()[ix].val());

	// writes current value which does not change
	writeEncoderValues(encoder, ix);
};

MLB.scopeToSky.updateEncoders = function () {
	var scopeToSkyState = MLB.scopeToSky.scopeToSkyState,
	    encoders = scopeToSkyState.encoders,
	    updateAnEncoder = MLB.scopeToSky.updateAnEncoder,
		ix = 0;

	_.each(encoders, function (encoder) {
		updateAnEncoder(encoder, ix++);
	});
};

MLB.scopeToSky.resetAnEncoder = function (encoder, ix) {
	var uom = MLB.sharedLib.uom,
	    roundToDecimal = MLB.sharedLib.roundToDecimal,
	...
	inputEncoderAngleOffsetDegs()[ix].val(roundToDecimal(encoder.offsetAngle / uom.degToRad, positionDecimals));
	writeEncoderValues(encoder, ix);
};

MLB.scopeToSky.scopeToEncoders = function () {
	var setToAngle = MLB.encoderLib.setToAngle,
		writeEncoderValues = MLB.scopeToSky.writeEncoderValues,
		scopeToSkyState = MLB.scopeToSky.scopeToSkyState,
	    inOutScopeAzimuth = scopeToSkyState.inOutScopeAzimuth,
	    inOutScopeAltitude = scopeToSkyState.inOutScopeAltitude,
		parseCoordinateGetValueInRadians = MLB.coordLib.parseCoordinateGetValueInRadians,
		az = parseCoordinateGetValueInRadians(inOutScopeAzimuth().val()).radians,
		alt = parseCoordinateGetValueInRadians(inOutScopeAltitude().val()).radians,
		encoders = scopeToSkyState.encoders,
		ix = 0;

	setToAngle(encoders[0], az);
	setToAngle(encoders[1], alt);
...
MLB.scopeToSky.encodersToScope = function () {
	...
	inOutScopeAzimuth().val(roundToDecimal(encoders[0].gearAngleWithOffset / uom.degToRad, positionDecimals));
	inOutScopeAltitude().val(roundToDecimal(encoders[1].gearAngleWithOffset / uom.degToRad, positionDecimals));
...
MLB.scopeToSky.setInitialValues = function () {
	var startingLatitudeDeg = 40,
		...
		btnUpdateEncoder1 = $('input[id=btnUpdateEncoder1]'),
		btnUpdateEncoder2 = $('input[id=btnUpdateEncoder2]'),
		btnResetEncoder1 = $('input[id=btnResetEncoder1]'),
		btnResetEncoder2 = $('input[id=btnResetEncoder2]'),
		btnScopeToEncoders = $('input[id=btnScopeToEncoders]'),
		btnEncodersToScope = $('input[id=btnEncodersToScope]'),
		dst,
		startingRA,
		scopeToSkyState = MLB.scopeToSky.scopeToSkyState,
		...
		encoders = scopeToSkyState.encoders,
	    updateAnEncoder = MLB.scopeToSky.updateAnEncoder,
	    resetAnEncoder = MLB.scopeToSky.resetAnEncoder,
		scopeToEncoders = MLB.scopeToSky.scopeToEncoders,
		encodersToScope = MLB.scopeToSky.encodersToScope,
	...
	btnUpdateEncoder1.click(function () {
		updateAnEncoder(encoders[0], 0);
	});

	btnUpdateEncoder2.click(function () {
		updateAnEncoder(encoders[1], 1);
	});

	btnResetEncoder1.click(function () {
		resetAnEncoder(encoders[0], 0);
	});

	btnResetEncoder2.click(function () {
		resetAnEncoder(encoders[1], 1);
	});

	btnScopeToEncoders.click(function () {
		scopeToEncoders();
	});

	btnEncodersToScope.click(function () {
		encodersToScope();
	});

	encoders[0].name = 'primary axis';
	encoders[1].name = 'secondary axis';
	MLB.scopeToSky.updateEncoders();
...
$(window).ready(function () {
	...
	jQuery(".collapsableHeader").click(function () {
		jQuery(this).next(".collapsableContent").slideToggle(250);
	});
