...
MLB.coordLib.calcAngularSepUsingRaDec = function (RaA, DecA, RaB, DecB) {
    var uom = MLB.sharedLib.uom;

    return Math.acos(Math.cos(uom.qtrRev - DecA) * Math.cos(uom.qtrRev - DecB) + Math.sin(uom.qtrRev - DecA) * Math.sin(uom.qtrRev - DecB) * Math.cos(RaA - RaB));
};
