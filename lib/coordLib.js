...
MLB.coordLib.validRev = function (rad) {
...
MLB.coordLib.validHalfRev = function (rad) {
...
MLB.coordLib.validDec = function (Dec) {
	var halfRevDec,
	    correctedDec,
		uom = MLB.sharedLib.uom,
...
MLB.coordLib.reverseRev = function (value) {
	var uom = MLB.sharedLib.uom;

	return uom.oneRev - value;
};

MLB.coordLib.calcDecIsFlipped = function (Dec) {
	var uom = MLB.sharedLib.uom;

	return Dec > uom.qtrRev || Dec < -uom.qtrRev;
};

MLB.coordLib.flipRa = function (Ra) {
	var uom = MLB.sharedLib.uom,
...
MLB.coordLib.flipDec = function (Dec) {
	var halfRevDec,
	    flippedDec,
		uom = MLB.sharedLib.uom,
		validHalfRev = MLB.coordLib.validHalfRev;
...
MLB.coordLib.convertHMSMToRad = function (hours, minutes, seconds, milliSeconds) {
	var uom = MLB.sharedLib.uom;

	return (hours + minutes / 60 + seconds / 3600 + milliSeconds / 3600000) * uom.hrToRad;
};

MLB.coordLib.convertDMSMToRad = function (degrees, minutes, seconds, milliSeconds) {
	var absDegrees,
	    absMinutes,
		absSeconds,
		absMilliSeconds,
...
MLB.coordLib.convertRadToHMSM = function (radians) {
	var sign,
	    remainder,
		factor,
		hours,
...
MLB.coordLib.convertRadToHMS = function (radians) {
	var sign,
	    remainder,
		factor,
		hours,
...
MLB.coordLib.convertRadToDMSM = function (radians) {
	var sign,
	    remainder,
		factor,
		degrees,
...
MLB.coordLib.convertRadToDMS = function (radians) {
	var sign,
	    remainder,
		factor,
		degrees,
...
MLB.coordLib.convertHMSMToString = function (hmsm, limitToHundredsArcsec) {
	var string = '',
	    int = MLB.sharedLib.int,
		uom = MLB.sharedLib.uom;
...
MLB.coordLib.convertDMSMToString = function (dmsm, limitToTenthsArcsec) {
	var string = '',
	    int = MLB.sharedLib.int,
		uom = MLB.sharedLib.uom;
...
MLB.coordLib.convertRadiansToHMSMString = function (radians, limitToHundredsArcsec) {
	var convertRadToHMSM = MLB.coordLib.convertRadToHMSM,
...
MLB.coordLib.convertRadiansToHMSString = function (radians) {
	var convertRadToHMS = MLB.coordLib.convertRadToHMS,
	    convertHMSMToString = MLB.coordLib.convertHMSMToString;

	return convertHMSMToString(convertRadToHMS(radians));
};

MLB.coordLib.convertRadiansToDMSMString = function (radians, limitToTenthsArcsec) {
	var convertRadToDMSM = MLB.coordLib.convertRadToDMSM,
...
MLB.coordLib.convertRadiansToDMSString = function (radians) {
	var convertRadToDMS = MLB.coordLib.convertRadToDMS,
	    convertDMSMToString = MLB.coordLib.convertDMSMToString;

	return convertDMSMToString(convertRadToDMS(radians));
};

MLB.coordLib.dateFromString = function (string) {
	return new Date(Date.parse(string));
};

MLB.coordLib.getMonth = function (monthNumber) {
	return ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][monthNumber];
};

MLB.coordLib.getMDY = function (date) {
	var getMonth = MLB.coordLib.getMonth;

	return getMonth(date.getMonth()) + ' ' + date.getDate() + ', ' + date.getFullYear();
};

MLB.coordLib.dateToString = function (date) {
	var convertHMSMToString = MLB.coordLib.convertHMSMToString,
	    getMDY = MLB.coordLib.getMDY,
	    hmsm = {
			hours: date.getHours(),
...
MLB.coordLib.dateTodayString = function () {
	var getMDY = MLB.coordLib.getMDY;

	return getMDY(new Date());
};

MLB.coordLib.isLeapYear = function (year) {
	return year % 4 === 0 && (year % 100 !== 0 || year % 400 === 0);
};

// from Meeus
MLB.coordLib.calcDayOfTheYear = function (year, month, day, hours, minutes, seconds, milliseconds) {
...
MLB.coordLib.calcJulianYear = function (JD) {
	var uom = MLB.sharedLib.uom;

	return (JD - uom.JD2000) / 365.25 + 2000;
};

MLB.coordLib.calcJDFromJulianYear = function (JulianYear) {
	var uom = MLB.sharedLib.uom;
...
MLB.coordLib.calcJD = function (year, month, day, hours, minutes, seconds, milliseconds, timeZoneOffsetHrs) {
...
MLB.coordLib.calcDateFromJD = function (JD, timeZoneOffsetHrs) {
	var tzJD,
	    z,
		f,
		a,
...
MLB.coordLib.calcSidTFromJD = function (JD, longitudeDeg) {
	var daysSinceJD2000,
	    GMSTHrs,
	    uom = MLB.sharedLib.uom,
		validRev = MLB.coordLib.validRev;
...
MLB.coordLib.calcSidT = function (year, month, day, hour, minute, second, millisecond, timeZoneOffsetHrs, longitudeDeg) {
	var calcJD = MLB.coordLib.calcJD,
...
MLB.coordLib.calcProperMotion = function (properMotionRA, properMotionDec, deltaJulianYear) {
	return {
...
MLB.coordLib.calcPrecessionQuick = function (RA, Dec, deltaYr) {
	var m,
	    n,
		deltaRA,
		deltaDec,
...
MLB.coordLib.calcPrecessionRigorous = function (RA, Dec, startJulianYear, deltaJulianYear) {
	var t1,
	    t2,
		t1Sqr,
		t2Sqr,
...
MLB.coordLib.celestialParms = function (JulianYearsSinceJD2000) {
	var t,
	    degToRad,
		arcsecToRad,
		SunMeanLon,
...
MLB.coordLib.calcNutation = function (RA, Dec, eclipticObliquity, nutationLon, nutationObliquity) {
	var cosRA,
	    sinRA,
		tanDec,
...
MLB.coordLib.calcAnnualAberration = function (Ra, Dec, SunTrueLon, eclipticObliquity, eEarthOrbit, EarthPerihelionLon) {
	var k = 20.49552,
	    cosRa,
		sinRa,
		cosDec,
...
MLB.coordLib.calcProperMotionPrecessionNutationAnnualAberration = function (RA, Dec, coordJD, JD, properMotionRA, properMotionDec) {
	var coordJDYear,
	    JDYear,
		deltaJulianYear,
...
MLB.coordLib.Position = function (RA, Dec, az, alt, SidT, HA) {
	this.RA = RA;
	this.Dec = Dec;
	this.az = az;
	this.alt = alt;
...
MLB.coordLib.calcAngularSepUsingEquat = function (a, z) {
	var deltaHA;
...
MLB.coordLib.calcAngularSepUsingAltaz = function (a, z) {
...
MLB.coordLib.angSepDiff = function (a, z) {
	var calcAngularSepUsingEquat = MLB.coordLib.calcAngularSepUsingEquat,
...
MLB.coordLib.calcAirMass = function (elevationDegrees) {
...
MLB.coordLib.calcAirMass2 = function (apparentElevationDegrees) {
	var uom = MLB.sharedLib.uom;
...
MLB.coordLib.setRefractionWorkVars = function (elevationDegrees) {
	var ix,
	    refractTable = MLB.coordLib.refractTable,
	    r = refractTable.r;
...
MLB.coordLib.calcRefractionFromTrue = function (trueElevationDegrees) {
	var wv,
	    refractionArcminutes,
		uom = MLB.sharedLib.uom,
		setRefractionWorkVars = MLB.coordLib.setRefractionWorkVars;
...
MLB.coordLib.calcRefractionFromApparent = function (apparentElevationDegrees) {
	var wv,
...
MLB.coordLib.parseCoordinateAddWIP = function (wip, workToAdd) {
...
MLB.coordLib.parseCoordinateNumberStrategy = function (isNum, isNumMatch, wip, work, c) {
	var parseCoordinateAddWIP = MLB.coordLib.parseCoordinateAddWIP;
...
MLB.coordLib.parseCoordinateIntoStringArray = function (string) {
	var wip = [],
	    wip2 = [],
		stringLength,
		ix,
		...
		isNum = [false],
		work = [''],
		parseCoordinateAddWIP = MLB.coordLib.parseCoordinateAddWIP,
		parseCoordinateNumberStrategy = MLB.coordLib.parseCoordinateNumberStrategy;
...
MLB.coordLib.ParseCoordinatesTypes = {
	radians: 0,
	hours: 1,
	degrees: 2,
	minutes: 3,
...
MLB.coordLib.ParseCoordinatesLits = [
...
MLB.coordLib.parseCoordinatesAddError = function (errors, result, nextResult) {
...
MLB.coordLib.parseCoordinatesTwoNumbersStrategy = function (result, values, errors, useHours) {
	var valueSet = false,
	    ParseCoordinatesTypes = MLB.coordLib.ParseCoordinatesTypes,
		parseCoordinatesAddError = MLB.coordLib.parseCoordinatesAddError;
...
MLB.coordLib.parseCoordinatesOneNumberStrategy = function (result, nextResult, values, errors) {
	var valueSet = false,
	    tIx = 0,
		lIx,
		ParseCoordinatesTypes = MLB.coordLib.ParseCoordinatesTypes,
		ParseCoordinatesLits = MLB.coordLib.ParseCoordinatesLits,
		parseCoordinatesAddError = MLB.coordLib.parseCoordinatesAddError;
...
MLB.coordLib.parseCoordinateGetResultIsNum = function (token) {
	var result, isNum;
...
MLB.coordLib.parseCoordinate = function (string, useHours) {
	var values = [],
		results,
	    result,
		resultIsNum,
		...
		parseCoordinatesTwoNumbersStrategy = MLB.coordLib.parseCoordinatesTwoNumbersStrategy,
		parseCoordinatesOneNumberStrategy = MLB.coordLib.parseCoordinatesOneNumberStrategy,
		parseCoordinateGetResultIsNum = MLB.coordLib.parseCoordinateGetResultIsNum,
...
MLB.coordLib.ParseCoordinatesHoursConversion = [
	[1],
...
MLB.coordLib.ParseCoordinatesDegreesConversion = [
	[1],
	[MLB.sharedLib.uom.hrToRad],
	[MLB.sharedLib.uom.degToRad],
...
MLB.coordLib.isNegNumber = function (num) {
...
MLB.coordLib.parseCoordinateSetNegativeValues = function (values) {
	var ParseCoordinatesTypes = MLB.coordLib.ParseCoordinatesTypes,
	    isNegNumber = MLB.coordLib.isNegNumber;
...
MLB.coordLib.parseCoordinateGetValueInRadians = function (string, useHours) {
	var radians = 0,
		hoursFound,
		conversion,
		valuesLength,
		...
		ParseCoordinatesTypes = MLB.coordLib.ParseCoordinatesTypes,
		ParseCoordinatesHoursConversion = MLB.coordLib.ParseCoordinatesHoursConversion,
		ParseCoordinatesDegreesConversion = MLB.coordLib.ParseCoordinatesDegreesConversion,
		parseCoordinateSetNegativeValues = MLB.coordLib.parseCoordinateSetNegativeValues;
...
MLB.coordLib.AxisTuple = function () {
	this.pri = 0;
	this.sec = 0;
};

MLB.coordLib.setPositionDeg = function (RAdeg, DecDeg, azDeg, altDeg, sidTdeg, HAdeg, position) {
	var uom = MLB.sharedLib.uom;
...
MLB.coordLib.setPositionDegFromString = function (string, position) {
	var setPositionDeg = MLB.coordLib.setPositionDeg,
...
MLB.coordLib.copyPosition = function (from, to) {
	to.RA = from.RA;
	to.Dec = from.Dec;
	to.az = from.az;
	to.alt = from.alt;
...
MLB.coordLib.clearPosition = function (position) {
	position.RA = undefined;
	position.Dec = undefined;
	position.az = undefined;
	position.alt = undefined;
...
MLB.coordLib.positionDegToString = function (position) {
	var uom = MLB.sharedLib.uom;
...
MLB.coordLib.isValidPosition = function (position) {
	return (position !== undefined) && !isNaN(position.RA) && !isNaN(position.Dec) && !isNaN(position.az) && !isNaN(position.alt) && !isNaN(position.SidT);
};

MLB.coordLib.FabErrors = function () {
	this.z1 = 0;
	this.z2 = 0;
	this.z3 = 0;
};

MLB.coordLib.setFabErrorsDeg = function (z1deg, z2deg, z3deg, fabErrors) {
	var uom = MLB.sharedLib.uom;
...
MLB.coordLib.MeridianFlipStates = {
	onEastSidePointingWest: 'onEastSidePointingWest',
	onWestSidePointingEast: 'onWestSidePointingEast'
};

MLB.coordLib.MeridianFlip = function () {
	var MeridianFlipStates = MLB.coordLib.MeridianFlipStates;
...
MLB.coordLib.setFlipState = function (isFlipped, meridianFlip) {
	var MeridianFlipStates = MLB.coordLib.MeridianFlipStates;
...
MLB.coordLib.isFlipped = function (meridianFlip) {
...
MLB.coordLib.flipAltazAcrossPole = function (position) {
	var uom = MLB.sharedLib.uom,
	    validRev = MLB.coordLib.validRev;
...
MLB.coordLib.translateAltazAcrossPoleBasedOnMeridianFlip = function (position, meridianFlip) {
	var isFlipped = MLB.coordLib.isFlipped,
...
MLB.coordLib.calcMeridianFlipFromCurrentAltaz = function (position, fabErrors) {
	var uom = MLB.sharedLib.uom,
...
MLB.coordLib.convertPriAxis = function (lat, toSec, fromPri, fromSec) {
	var cosToPri,
	    toPri,
	    uom = MLB.sharedLib.uom,
		validRev = MLB.coordLib.validRev,
...
MLB.coordLib.convertSecAxis = function (lat, fromPri, fromSec) {
...
*/
MLB.coordLib.getAltazTrig = function (position, meridianFlip, latitude) {
	var absLatitude,
	    dec,
		holdRa,
		uom = MLB.sharedLib.uom,
		...
		convertPriAxis = MLB.coordLib.convertPriAxis,
		convertSecAxis = MLB.coordLib.convertSecAxis;
...
*/
MLB.coordLib.getEquatTrig = function (position, meridianFlip, latitude) {
	var holdAz,
	    holdAlt,
		absLatitude,
		az,
...
MLB.coordLib.create2DArray = function (dimen) {
	var a = [],
	    ix;
...
MLB.coordLib.zeroArrays = function (cmws) {
	var i,
	    j;
...
MLB.coordLib.ConvertSubroutineType = {
...
MLB.coordLib.ConvertMatrixWorkingStorage = function () {
	var FabErrors = MLB.coordLib.FabErrors,
	    MeridianFlip = MLB.coordLib.MeridianFlip,
	    Position = MLB.coordLib.Position,
	    create2DArray = MLB.coordLib.create2DArray,
	    zeroArrays = MLB.coordLib.zeroArrays,
...
*/
MLB.coordLib.getApparentAlt = function (cosZ1, cosZ2, sinZ1, sinZ2, cmws) {
	var v1 = (Math.sin(cmws.sec) - sinZ1 * sinZ2) * cosZ1 * (cosZ2 / ((sinZ1 * sinZ1 - 1) * (sinZ2 * sinZ2 - 1)));

	return Math.asin(v1);
};

MLB.coordLib.angleSubr = function (cmws) {
	var uom = MLB.sharedLib.uom,
	    validRev = MLB.coordLib.validRev,
		reverseRev = MLB.coordLib.reverseRev,
...
//per Taki's eq 5.3-4
MLB.coordLib.z12FuncTakiSimple = function (cosPri, cosSec, sinPri, sinSec, cmws) {
...
//per Taki's eq 5.3-2
MLB.coordLib.z12FuncTakiSmallAngle = function (cosPri, cosSec, sinPri, sinSec, cosZ1, cosZ2, sinZ1, sinZ2, cmws) {
...
MLB.coordLib.z12FuncBellIterative = function (cosPri, cosSec, sinPri, sinSec, cosZ1, cosZ2, sinZ1, sinZ2, cmws) {
	var trueAz,
	    tanTrueAz,
		apparentAlt,
		apparentAz,
		...
		subrLCount,
		uom = MLB.sharedLib.uom,
		getApparentAlt = MLB.coordLib.getApparentAlt,
		angleSubr = MLB.coordLib.angleSubr,
		z12FuncTakiSmallAngle = MLB.coordLib.z12FuncTakiSmallAngle;
...
*/
MLB.coordLib.z12FuncTakilIterative = function (cosPri, cosSec, sinPri, sinSec, cosZ1, cosZ2, sinZ1, sinZ2, cmws) {
	var MaxLoopCount,
	    holdPri,
		holdSec,
		last,
		...
		sinF1,
		uom = MLB.sharedLib.uom,
		AxisTuple = MLB.coordLib.AxisTuple,
		angleSubr = MLB.coordLib.angleSubr,
		z12FuncTakiSmallAngle = MLB.coordLib.z12FuncTakiSmallAngle,
		z12FuncBellIterative = MLB.coordLib.z12FuncBellIterative;
...
MLB.coordLib.z12FuncBellTakilIterative = function (cosPri, cosSec, sinPri, sinSec, cosZ1, cosZ2, sinZ1, sinZ2, cmws) {
	var getApparentAlt = MLB.coordLib.getApparentAlt,
	    angleSubr = MLB.coordLib.angleSubr,
		z12FuncTakilIterative = MLB.coordLib.z12FuncTakilIterative,
	    apparentAlt = getApparentAlt(cosZ1, cosZ2, sinZ1, sinZ2, cmws);
...
MLB.coordLib.subrSwitcher = function (cmws) {
	var cosPri,
	    cosSec,
		sinPri,
		sinSec,
		...
		sinZ2,
		ConvertSubroutineType = MLB.coordLib.ConvertSubroutineType,
		z12FuncTakiSimple = MLB.coordLib.z12FuncTakiSimple,
		z12FuncTakiSmallAngle = MLB.coordLib.z12FuncTakiSmallAngle,
		z12FuncBellIterative = MLB.coordLib.z12FuncBellIterative,
		z12FuncTakilIterative = MLB.coordLib.z12FuncTakilIterative,
		z12FuncBellTakilIterative = MLB.coordLib.z12FuncBellTakilIterative;
...
MLB.coordLib.determinateSubr = function (cmws) {
...
MLB.coordLib.calcRectCoords = function (cmws) {
	var cosPri,
	    cosSec,
		cosZ1,
		cosZ2,
...
MLB.coordLib.getAltazMatrix = function (cmws) {
	var i,
	    j,
		HA,
		cosDec,
		...
		translateAltazAcrossPoleBasedOnMeridianFlip = MLB.coordLib.translateAltazAcrossPoleBasedOnMeridianFlip,
		angleSubr = MLB.coordLib.angleSubr,
		subrSwitcher = MLB.coordLib.subrSwitcher;
...
MLB.coordLib.getEquatMatrix = function (cmws) {
	var i,
	    j,
		holdAlt,
		holdAz,
		...
		translateAltazAcrossPoleBasedOnMeridianFlip = MLB.coordLib.translateAltazAcrossPoleBasedOnMeridianFlip,
		angleSubr = MLB.coordLib.angleSubr,
		calcRectCoords = MLB.coordLib.calcRectCoords;
...
MLB.coordLib.arrayAssignInit = function (init, cmws) {
	var HA,
	    cosDec,
		sinDec,
		reverseRev = MLB.coordLib.reverseRev,
...
MLB.coordLib.generateThirdInit = function (cmws) {
	var i,
	    a;
...
MLB.coordLib.transformMatrix = function (cmws) {
	var i,
	    j,
		l,
		m,
		n,
		e,
		determinateSubr = MLB.coordLib.determinateSubr;
...
MLB.coordLib.initMatrix = function (init, cmws) {
	var copyPosition = MLB.coordLib.copyPosition,
	    arrayAssignInit = MLB.coordLib.arrayAssignInit,
		generateThirdInit = MLB.coordLib.generateThirdInit,
		transformMatrix = MLB.coordLib.transformMatrix;
...
MLB.coordLib.initMatrixFacade = function (cmws, init) {
	var copyPosition = MLB.coordLib.copyPosition,
...
MLB.coordLib.Hemisphere = {
	northern: 'northern',
	southern: 'southern'
};

MLB.coordLib.presetEquat = function (hemisphere, cmws) {
	var setPositionDeg = MLB.coordLib.setPositionDeg,
...
MLB.coordLib.presetAltaz = function (latitudeRad, cmws) {
	var decDeg,
	    absLatitudeDeg,
		uom = MLB.sharedLib.uom,
...
MLB.coordLib.initAltazResults = function (hemisphere, cmws) {
	var azOffset,
	    HAOffset,
		latitude,
		uom = MLB.sharedLib.uom,
...
MLB.coordLib.calcAltOffsetDirectly = function (a, z) {
	var n,
	    m,
		x,
		a1,
...
// +- 45 deg search range
MLB.coordLib.calcAltOffsetIteratively = function (a, z) {
	var searchRangeDeg,
	    aAlt,
		zAlt,
		altIncr,
		...
		bestAltOff,
		uom = MLB.sharedLib.uom,
		angSepDiff = MLB.coordLib.angSepDiff;
...
MLB.coordLib.calcFieldRotationAngle = function (position, HAOffset, latitude) {
	var a,
	    sinHA,
		FR,
		uom = MLB.sharedLib.uom,
...
MLB.coordLib.getAltOffset = function (a, z) {
	var calcAltOffsetDirectly = MLB.coordLib.calcAltOffsetDirectly,
...
MLB.coordLib.bestZ3 = function (positions, cmws) {
	var accumAltOffset,
	    count,
		positionsLength,
		ix,
		ixB,
		getAltOffset = MLB.coordLib.getAltOffset;
...
MLB.coordLib.calcPointingError = function (positions, cmws) {
...
MLB.coordLib.bestZ12Core = function (cmws, combinedPositions, z1, z2, bestZ1, bestZ2, bestZ3, bestPointingErrorRMS) {
	var pointingErrorRMS,
		uom = MLB.sharedLib.uom,
		copyPosition = MLB.coordLib.copyPosition,
		initMatrix = MLB.coordLib.initMatrix,
		calcPointingError = MLB.coordLib.calcPointingError;
...
MLB.coordLib.bestZ12 = function (positions, cmws, fabErrors, bestZ3, z1Range, z2Range, resolution) {
	var ix,
	    combinedPositions,
		positionsLength,
		startZ1,
		...
		copyPosition = MLB.coordLib.copyPosition,
		initMatrix = MLB.coordLib.initMatrix,
		bestZ12Core = MLB.coordLib.bestZ12Core;
...
MLB.coordLib.bestZ123 = function (positions, cmws, fabErrors) {
	var uom = MLB.sharedLib.uom,
	    bestZ3 = MLB.coordLib.bestZ3,
		bestZ12 = MLB.coordLib.bestZ12,
	    z1Range = uom.degToRad,
	    z2Range = uom.degToRad,
...
MLB.coordLib.bestZ13 = function (positions, cmws, fabErrors) {
	var uom = MLB.sharedLib.uom,
	    bestZ3 = MLB.coordLib.bestZ3,
		bestZ12 = MLB.coordLib.bestZ12,
...
MLB.coordLib.getLatitude = function (initType, latitude) {
	var uom = MLB.sharedLib.uom,
	    InitType = MLB.coordLib.InitType;
...
MLB.coordLib.getHemisphere = function (latitude) {
	var Hemisphere = MLB.coordLib.Hemisphere;
...
MLB.coordLib.XForm = function (convertStyle, latitude) {
	...
	this.calcFieldRotationAngle = function (HAOffset) {
		var calcFieldRotationAngle = MLB.coordLib.calcFieldRotationAngle,
		    getLatitude = MLB.coordLib.getLatitude;
	...
	this.setMeridianFlip = function (canFlip, isFlipped) {
		var setFlipState = MLB.coordLib.setFlipState;
		...
		// convert using matrix math
			...
			this.presetEquat = function () {
				var InitType = MLB.coordLib.InitType,
					...
					getHemisphere = MLB.coordLib.getHemisphere;
			...
			this.getEquat = function () {
				var getEquatMatrix = MLB.coordLib.getEquatMatrix;

				getEquatMatrix(this.cmws);
			};
			this.getAltaz = function () {
				var getAltazMatrix = MLB.coordLib.getAltazMatrix;
...
*/
MLB.coordLib.calcKingRateMinutesPerDay = function (RA, Dec, SidT, latitude) {
	var cosLat,
	    sinLat,
		cotLat,
		cosDec,
...
MLB.coordLib.calcKingRateArcsecPerSec = function (KingRateMinutesPerDay) {
	var uom = MLB.sharedLib.uom;
...
*/
MLB.coordLib.calcFieldRotationRateSidTrackFormula = function (position, meridianFlip, latitude, HAOffset) {
	var initialDec,
	    hourAngle,
		DecAtZenith,
		tanDecAtZenith,
...
MLB.coordLib.calcAltazTrackingRates = function (az, alt, latitude) {
	var azRate,
	    altRate,
		cot = MLB.sharedLib.cot,
		uom = MLB.sharedLib.uom;
...
MLB.coordLib.calcRefractionFromTrueEquatorialCorrection = function (RA, Dec, SidT, latitude, refractPosition) {
	var az,
	    alt,
		refraction,
		deltaRA,
...
MLB.coordLib.calcRefractionFromApparentEquatorialCorrection = function (RA, Dec, SidT, latitude, refractPosition) {
	var az,
	    alt,
		refraction,
		deltaRA,
...
MLB.coordLib.undefinedRefraction = function () {
...
MLB.coordLib.trackingRatesGetAltazCheckRefraction = function (xform, FR, HAOffset, savePosition, refractPosition) {
	var calcRefractionFromTrueEquatorialCorrection = MLB.coordLib.calcRefractionFromTrueEquatorialCorrection,
...
MLB.coordLib.setAzAltFRRates = function (position2, position1, FR2, FR1, timeIntervalSecs, azRate, altRate, FRRate) {
	var validHalfRev = MLB.coordLib.validHalfRev;
...
MLB.coordLib.calcTimeIntervalSidT = function (timeIntervalSecs) {
	var uom = MLB.sharedLib.uom;

	return timeIntervalSecs * uom.sidRate * uom.secToRad;
};

MLB.coordLib.TrackingRatesStrategy = {
...
MLB.coordLib.TrackingRates = function () {
	this.savePosition = new MLB.coordLib.Position();
	this.work0Position = new MLB.coordLib.Position();
	this.work1Position = new MLB.coordLib.Position();
	this.work2Position = new MLB.coordLib.Position();
	this.work3Position = new MLB.coordLib.Position();

	this.trackingRatesStrategy = undefined;

	// uses three getAltaz(); with refraction, uses 3 getAltaz(), 3 getAltazTrig(), 3 getEquatTrig()
	this.getRatesViaDeltaTime = function (xform, timeIntervalSecs, HAOffset, includeRefraction) {
		var timeIntervalSidT,
		    refractionResults,
			azRate1 = [],
			altRate1 = [],
			FRRate1 = [],
			...
			getAltazTrig = MLB.coordLib.getAltazTrig,
			copyPosition = MLB.coordLib.copyPosition,
			undefinedRefraction = MLB.coordLib.undefinedRefraction,
			trackingRatesGetAltazCheckRefraction = MLB.coordLib.trackingRatesGetAltazCheckRefraction,
			setAzAltFRRates = MLB.coordLib.setAzAltFRRates,
			calcTimeIntervalSidT = MLB.coordLib.calcTimeIntervalSidT,
			TrackingRatesStrategy = MLB.coordLib.TrackingRatesStrategy;

		timeIntervalSidT = calcTimeIntervalSidT(timeIntervalSecs);
		copyPosition(xform.position, this.savePosition);
		...
		if (includeRefraction) {
			...
			refractionResults = trackingRatesGetAltazCheckRefraction(xform, FR1, HAOffset, this.savePosition, this.work0Position);
			copyPosition(xform.position, this.work1Position);

			xform.position.SidT += timeIntervalSidT;
			trackingRatesGetAltazCheckRefraction(xform, FR2, HAOffset, this.savePosition, this.work0Position);
			copyPosition(xform.position, this.work2Position);

			xform.position.SidT += timeIntervalSidT;
			trackingRatesGetAltazCheckRefraction(xform, FR3, HAOffset, this.savePosition, this.work0Position);
			copyPosition(xform.position, this.work3Position);
		} else {
			this.trackingRatesStrategy = TrackingRatesStrategy.deltaTimeNoRefraction;
			// site altaz
			copyPosition(xform.position, this.work0Position);
			getAltazTrig(this.work0Position, null, xform.latitude);
		...
		setAzAltFRRates(this.work2Position, this.work1Position, FR2, FR1, timeIntervalSecs, azRate1, altRate1, FRRate1);
		setAzAltFRRates(this.work3Position, this.work2Position, FR3, FR2, timeIntervalSecs, azRate2, altRate2, FRRate2);
		...
		copyPosition(this.savePosition, xform.position);

		if (refractionResults === undefined) {
			refractionResults = undefinedRefraction();
			siteAz = this.work0Position.az;
			siteAlt = this.work0Position.alt;
		} else {
		...
		return {
			initialAz: this.work1Position.az,
			// includes refraction, if turned on
			initialAlt: this.work1Position.alt,
			initialFR: FR1[0],
			azRate: azRate1[0],
			altRate: altRate1[0],
			FRRate: FRRate1[0],
	...
	this.getKingRates = function (xform, timeIntervalSecs) {
		var sitePosition = this.work0Position,
		    latitude,
			scopePosition = this.work1Position,
			refractionResults,
			refractPosition = this.work2Position,
			timeIntervalSidT,
			KingRate0,
			KingRate1,
			rateChange,
	...
	// uses two getAltazTrig(), two getEquatTrig()
	this.getRatesViaFormulaeNoRefraction = function (xform, timeIntervalSecs, HAOffset) {
		...
		copyPosition(this.work1Position, xform.position);
...
*/
MLB.coordLib.ThreeAxis = function () {
	var XForm = MLB.coordLib.XForm,
	    Position = MLB.coordLib.Position,
		uom = MLB.sharedLib.uom,
		ConvertStyle = MLB.coordLib.ConvertStyle;
