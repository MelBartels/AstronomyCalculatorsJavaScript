...
MLB.pitchLapCalc.common = {
    ...
    gravity: function () {
        return $('[name=btnGravity]:checked').val();
    },
    btnStrokeType: function () {
        return $('[name=btnStrokeType]');
    },
    strokeType: function () {
        return $('[name=btnStrokeType]:checked').val();
    },
...
MLB.pitchLapCalc.buildStrokeArray = function () {
    var common = MLB.pitchLapCalc.common,
        ...
        strokeLength = +common.strokeLength().val(),
        strokeOffset = +common.strokeOffset().val(),
        radius = arraySide / 2,
        scaledStrokeLength = strokeLength * radius,
        scaledStrokeOffset = strokeOffset * radius,
        uom = MLB.sharedLib.uom,
    ...
    /*
    for each strokeLength increment from mirror center to end of strokeLength (break stroke into series of steps)
        for each mirror zone from center to edge
            zero out pitch contact for mirror zone
            for each angle within a full circle
                get pitch contact from center of lap (offset per strokeLength above) or if 'W' stroke, accumulate contacts from center to stroke offset
    ...
    */
    ...
    if (debug) {console.log('strokeLength = ' + strokeLength + '; strokeOffset = ' + strokeOffset + '; radius = ' + radius);}
    // accuracy/resolution scaled to pixels
    for (s = 0; s < scaledStrokeLength; s += 1) {
        if (debug) {console.log('starting new stroke position = ' + s + ' of ' + scaledStrokeLength);}
        for (mirrorZone = 0; mirrorZone < radius; mirrorZone += 1) {
            if (debug) {console.log('    starting new zone = ' + mirrorZone + ' of ' + radius);}
            accumContact = 0;
            // 12 deg resolution
            for (a = 0; a < uom.oneRev; a += uom.oneRev / 30) {
                if (debug) {console.log('        starting new angle = ' + a / uom.degToRad);}
                // get x, y distance from center of mirror's zone
                x = Math.sin(a) * mirrorZone;
                y = Math.cos(a) * mirrorZone;

                if (strokeType === 'straight') {
                    // lap's center is offset from mirror's center by 's', the strokeLength
                    lapZone = Math.sqrt((x + scaledStrokeOffset) * (x + scaledStrokeOffset) + (y - s) * (y - s)) / radius;
                    if (debug) {console.log('            x, y relative to mirror center = ' + x + ', ' + y + '; lap zone = ' + lapZone);}
                    if (lapZone <= 1.0) {
                        contact = getPitchLapContactForZone(lapZone);
                    } else {
                        contact = 0;
                    }
                } else if (strokeType === 'W') {
                    contact = 0;
                    for (sso = 0; sso <= scaledStrokeOffset; sso +=1) {
                        // lap's center is offset from mirror's center by 's', the strokeLength
                        lapZone = Math.sqrt((x + sso) * (x + sso) + (y - s) * (y - s)) / radius;
                        if (debug) {console.log('            x, y relative to mirror center = ' + x + ', ' + y + '; scaledStrokeOffset = ' + sso + '; lap zone = ' + lapZone);}
                        if (lapZone <= 1.0) {
                            contact += getPitchLapContactForZone(lapZone);
                        }
                    }
            ...
            strokeArray[mirrorZone] += scaledAccumContact;
            if (debug) {console.log('    added scaledAccumContact = ' + scaledAccumContact + ' to mirrorZone = ' + mirrorZone + '; strokeArray[mirrorZone] now = ' + strokeArray[mirrorZone]);}
        }
    }
...
MLB.pitchLapCalc.processPreset = function () {
    ...
    if (common.preset() === 'fullContact') {
        ...
        $('[id=zone9]').val('');
        $('[id=zone10]').val('');
    } else if (common.preset() === 'petalLap') {
        $('[id=zone1]').val('0.0');
        $('[id=zoneContact1]').val('1.0');
        $('[id=zone2]').val('0.2');
        $('[id=zoneContact2]').val('1.0');
        ...
        $('[id=zone9]').val('');
        $('[id=zone10]').val('');
    } else if (common.preset() === 'reversePetalLap') {
        $('[id=zone1]').val('0.0');
...
$(document).ready(function () {
    ...
    common.btnStrokeType().change(processInput);
    common.strokeLength().change(processInput);
    common.strokeOffset().change(processInput);
