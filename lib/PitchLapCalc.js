...
MLB.pitchLapCalc.buildStrokeArray = function () {
    ...
    /*
    for each strokeLength increment from mirror center to end of strokeLength
        for each mirror zone from center to edge
            zero out pitch contact for mirror zone
            for each angle within a full circle
                get pitch contact from center of lap (offset per strokeLength above)
    ...
    for (s = 0; s < scaledStrokeLength; s += 1) {
        ...
        for (z = 0; z < radius; z += 1) {
            ...
            for (a = 0; a < uom.oneRev; a += uom.oneRev / 30) {
                ...
                zone = Math.sqrt((x + scaledStrokeOffset) * (x + scaledStrokeOffset) + (y - s) * (y - s)) / radius;
