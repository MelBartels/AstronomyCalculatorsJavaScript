...
MLB.pitchLapCalc.getPolishingActionForStrokeAtOffset = function () {
    ...
    for (yOffset = 0; yOffset <= strokeAndOffsetInXY.yStroke; yOffset += 1) {
        ...
        if (pressure > 0) {
            getPolishingPressureForMirror(strokeAndOffsetInXY.xOffset, yOffset);
        }
    }
};
...
MLB.pitchLapCalc.getPolishingPressureForMirror = function (xOffset, yOffset) {
    var common = MLB.pitchLapCalc.common,
        ...
        pressure = +common.pressure().val() / mirrorXYSize,
        overhang,
        maxLeverage,
        startMirrorRadius,
        endMirrorRadius,
    ...
    // overhang creates leverage which ranges from nothing to max at mirror edge that is being overhung
...
MLB.pitchLapCalc.getPolishingActionForAMirrorNode = function (xOffset, yOffset, mirrorNode) {
    ...
    if (xTool >= toolNodes[0].x && xTool <= toolNodes[toolNodesLength - 1].x) {
        ...
        if (yTool >= yNodesTool[0].y && yTool <= yNodesTool[yNodesToolLength - 1].y) {
            ...
            common.mirrorPolishingActionPerRadius[radius] += toolNode.contact;
        }
    }
};
...
MLB.pitchLapCalc.calcWeightedMirrorPolishingActions = function () {
    ...
    common.mirrorPolishingActionPerRadius.forEach(function (action, ix) {
...
MLB.pitchLapCalc.displayPolishingActionsTopView = function () {
    var common = MLB.pitchLapCalc.common,
        polishActions = common.mirrorPolishingActionPerRadiusWeighted,
        //polishActions = common.mirrorPressurePerRadius,
...
MLB.pitchLapCalc.calculatePitchLapAndAction = function () {
    var buildPitchContactsArray = MLB.pitchLapCalc.buildPitchContactsArray,
...
$(document).ready(function () {
    ...
    // starting values
    common.btnPressure()[common.ixMOT].checked = true;
    ...
    common.pressure().change(calculatePitchLapAndAction);
    common.btnPressure().change(calculatePitchLapAndAction);
