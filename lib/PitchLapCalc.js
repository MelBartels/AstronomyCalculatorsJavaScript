...
/* get vector due to rotation of 'nodes' for each node x,; rotation centered at 0,0;
   ...
   quadrants:
    ...
    quadrant    node    swapx/y    CW    negate_after_swap_for_CW?    fix_for_CW
    1           --      --         +-    x                            y
    2           +-      -+         ++    x                            none
    3           ++      ++         -+    x                            x
    4           -+      +-         --    x                            x,y
...
MLB.pitchLapCalc.getRotationalVectorForAMirrorNode = function (mirrorNode, yOffset, toolToMirrorSizeRatio) {
    ...
    } else { // no toolNode therefore no polishing action
        mirrorNode.netVector.x = 0;
        mirrorNode.netVector.y = 0;
    }
};
...
MLB.pitchLapCalc.calcNetRotationalVectorForTOT = function (toolToMirrorSizeRatio, toolDirection, yOffset) {
    ...
    mirrorNodes.forEach(xMirrorNode => {
        xMirrorNode.yNodes.forEach(mirrorNode => {
            ...
            nodeDirectionX = mirrorNode.netVector.x;
            nodeDirectionY = mirrorNode.netVector.y;

            switch (mirrorNode.quadrant) {
            case 1:
                nodeDirectionY = -nodeDirectionY;
                break;
            case 2:
                break;
            case 3:
                nodeDirectionX = -nodeDirectionX;
                break;
            case 4:
                nodeDirectionX = -nodeDirectionX;
                nodeDirectionY = -nodeDirectionY;
            ...
            totalDirectionX += nodeDirectionX;
            totalDirectionY += nodeDirectionY;
        });
    });

    // eg, if net direction=1 (CW), then totalDirectionX = 3328.919999999965, totalDirectionY = 3328.919999999961
    return {
        totalDirectionX: totalDirectionX,
        totalDirectionY: totalDirectionY,
        totalDirection: totalDirectionX + totalDirectionY
    };
};

/*
directions progress linearly, eg, for toolToMirrorSizeRatio = 0.5 (1/2 that of the mirror), yOffset = 25 (halfway to edge); 1.4 tool speed is best fit:
0, 1971, 2113, 4085
0.1, 1894, 1902, 3796
0.2, 1817, 1690, 3508
0.3, 1740, 1479, 3219
...
1.9, 503, -1903, -1399
*/
MLB.pitchLapCalc.freelyRotatingToolRotationToMirrorRatio = function () {
    var common = MLB.pitchLapCalc.common,
    ...
    yOffset = 25;
    for (toolDirection = 0; toolDirection <= 2; toolDirection += 0.1) {
