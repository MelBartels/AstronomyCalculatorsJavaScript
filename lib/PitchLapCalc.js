...
netVector:
    ...
    directionCW: 2.6364431633905143
    distance: 4.770744176750625
    x: 4
    y: 2.6
*/

MLB.pitchLapCalc.calcNetRotationalVectorForTOT = function (toolToMirrorSizeRatio, mirrorDirection, toolDirection, yOffset, debug) {
    var common = MLB.pitchLapCalc.common,
    ...
    mirrorNodes.forEach(xMirrorNode => {
        xMirrorNode.yNodes.forEach(mirrorNode => {
            ...
            if (mirrorNode.toolNode) {
                ...
                if (debug && mirrorNode.x === -13 && mirrorNode.y === -30) {
    ...
    console.log('toolVectorCount=' + toolVectorCount);

    return totalNetDirection;
};

MLB.pitchLapCalc.testRotationalVectorForTOT = function () {
    var common = MLB.pitchLapCalc.common,
        toolToMirrorSizeRatio = 0.5,
        mirrorDirection = 10,
        toolDirection = 20,
        yOffset = 25,
        debug = true,
    ...
    calcNetRotationalVectorForTOT(toolToMirrorSizeRatio, mirrorDirection, toolDirection, yOffset, debug);
};
...
MLB.pitchLapCalc.freelyRotatingToolToMirrorRotationRatio = function () {
    ...
    for (yOffset = 0; yOffset <= 100; yOffset += 5) {
        bestDirection = Number.MAX_VALUE;
        for (toolDirection = -10; toolDirection <= 10; toolDirection += 0.1) {
            direction = calcNetRotationalVectorForTOT(toolToMirrorSizeRatio, mirrorDirection, toolDirection, yOffset);
            //console.log('direction=' + direction + ', toolDirection=' + toolDirection);
            if (Math.abs(direction) < Math.abs(bestDirection)) {
                bestDirection = direction;
                bestToolDirection = toolDirection;
                //console.log('bestDirection=' + bestDirection + ', bestToolDirection=' + bestToolDirection);
            }
        }
        console.log('yOffset=' + yOffset + '. best toolDirection = ' + bestToolDirection);
    }
};
...
$(document).ready(function () {
    ...
    common.JSversion().text('JavaScript version Feb 28, 2021');
