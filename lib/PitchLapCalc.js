...
MLB.pitchLapCalc.common = {
    ...
    testToolToMirrorSizeRatio: function () {
        return $('[id=testToolToMirrorSizeRatio]');
    },
    testMirrorRotation: function () {
        return $('[id=testMirrorRotation]');
    },
    testToolRotation: function () {
        return $('[id=testToolRotation]');
    },
    testToolOffset: function () {
        return $('[id=testToolOffset]');
    },
    testCanvas: function () {
        return $('#testCanvas')[0];
    },
    testResults: function () {
        return $('[id=testResults]');
    }
};
...
MLB.pitchLapCalc.calcNetRotationalVectorForTOT = function (toolToMirrorSizeRatio, mirrorDirection, toolDirection, yOffset) {
    var common = MLB.pitchLapCalc.common,
        createMirrorNodes = MLB.pitchLapCalc.createMirrorNodes,
        createToolNodes = MLB.pitchLapCalc.createToolNodes,
    ...
    mirrorNodes.forEach(xMirrorNode => {
        xMirrorNode.yNodes.forEach(mirrorNode => {
            ...
            if (mirrorNode.toolNode) {
                ...
                // test Sketchup example where: toolToMirrorSizeRatio = 0.5; yOffset = 25; mirrorDirection = 10; toolDirection = 20
                if (mirrorNode.x === -13 && mirrorNode.y === -30) {
...
MLB.pitchLapCalc.populateTestCanvas = function (rotationalVectorResults) {
    var common = MLB.pitchLapCalc.common,
        mirrorNodes = common.mirrorNodes,
        xySize = common.mirrorXYSize,
        canvas = common.testCanvas(),
        context = canvas.getContext('2d'),
        point = MLB.sharedLib.point,
        fillCircle = MLB.sharedLib.fillCircle,
        minSide,
    ...
    // draw in tool
    fillCircle(context, new point(center.x, center.y - rotationalVectorResults.yOffset * scalingFactor), radiusScaled * rotationalVectorResults.toolToMirrorSizeRatio, 'darkgray');
...
MLB.pitchLapCalc.runTest = function () {
    var common = MLB.pitchLapCalc.common,
        toolToMirrorSizeRatio = +common.testToolToMirrorSizeRatio().val(),
        mirrorDirection = +common.testMirrorRotation().val(),
        toolDirection = +common.testToolRotation().val(),
        yOffset = +common.testToolOffset().val() * common.mirrorXYSize,
        rotationalVectorResults,
        calcNetRotationalVectorForTOT = MLB.pitchLapCalc.calcNetRotationalVectorForTOT,
        populateTestCanvas = MLB.pitchLapCalc.populateTestCanvas;

    rotationalVectorResults = calcNetRotationalVectorForTOT(toolToMirrorSizeRatio, mirrorDirection, toolDirection, yOffset);
    populateTestCanvas(rotationalVectorResults);
    common.testResults().text('Net direction (positive is CW, negative is CCW) = ' + Math.floor(rotationalVectorResults.totalNetDirection + 0.5));
};
...
$(document).ready(function () {
    var common = MLB.pitchLapCalc.common,
        ...
        ix,
        htmlStr,
        runTest = MLB.pitchLapCalc.runTest;

    common.JSversion().text('JavaScript version March 2, 2021');
    ...
    common.testToolToMirrorSizeRatio().change(runTest);
    common.testMirrorRotation().change(runTest);
    common.testToolRotation().change(runTest);
    common.testToolOffset().change(runTest);
