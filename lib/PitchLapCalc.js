...
MLB.pitchLapCalc.getPolishingActionForStrokeAtOffset = function () {
    var common = MLB.pitchLapCalc.common,
        yOffset,
    ...
    common.polishingActionPerRadius = [];

    // step through the stroke at offset...
    strokeAndOffsetInXY = getStrokeLengthAndOffsetInXY();
    for (yOffset = 0; yOffset <= strokeAndOffsetInXY.yStroke; yOffset += 1) {
        getPolishingActionForMirror(strokeAndOffsetInXY.xOffset, yOffset);
    }
};

MLB.pitchLapCalc.getPolishingActionForMirror = function (xOffset, yOffset) {
    var common = MLB.pitchLapCalc.common,
    ...
    mirrorNodes.forEach(function (xMirrorNode) {
        xMirrorNode.yNodes.forEach(function (mirrorNode) {
            getPolishingActionForAMirrorNode(xOffset, yOffset, mirrorNode);
        });
    });
};

MLB.pitchLapCalc.getPolishingActionForAMirrorNode = function (xOffset, yOffset, mirrorNode) {
    var common = MLB.pitchLapCalc.common,
        xTool,
        yTool,
    ...
    if (xTool >= toolNodes[0].x && xTool <= toolNodes[toolNodesLength - 1].x) {
        ...
        if (yTool >= yNodesTool[0].y && yTool <= yNodesTool[yNodesToolLength - 1].y) {
            ...
            if (common.polishingActionPerRadius[radius] === undefined) {
                common.polishingActionPerRadius[radius] = 0;
            }
            // add tool node's contact to the mirror's polishing action per radius
            common.polishingActionPerRadius[radius] += toolNode.contact;
        }
    }
};

MLB.pitchLapCalc.calcWeightedPolishingActions = function () {
    var common = MLB.pitchLapCalc.common;

    common.polishingActionPerRadiusWeighted = [];

    common.polishingActionPerRadius.forEach(function (action, ix) {
        common.polishingActionPerRadiusWeighted[ix] = action / common.mirrorNodeCountPerRadius[ix];
    });
};
...
MLB.pitchLapCalc.displayPolishingActionsSideView = function () {
    ...
    polishActions.forEach(function (polishAction, ix) {
...
MLB.pitchLapCalc.displayPolishingActionsTopView = function () {
    ...
    fillCircle(context, center, canvas.width / 2, 'black');

    polishActions.forEach(function (polishAction, ix) {
        strokeStyle = 'rgb(0, 0, ' + polishAction * 255 / maxPolish + ')';
...
MLB.pitchLapCalc.calculatePitchLapAndAction = function () {
    var common = MLB.pitchLapCalc.common,
        ...
        getPolishingActionForStrokeAtOffset = MLB.pitchLapCalc.getPolishingActionForStrokeAtOffset,
        calcWeightedPolishingActions = MLB.pitchLapCalc.calcWeightedPolishingActions,
    ...
    createMirrorNodes();
    createToolNodes();
    getPolishingActionForStrokeAtOffset();
    calcWeightedPolishingActions();
