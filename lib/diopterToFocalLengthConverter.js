...
MLB.diopterToFocalLengthConverter.writeResults = function () {
    var common = MLB.diopterToFocalLengthConverter.common,
        ...
        eyeDiopter = common.diopterEVal(),
        power = calcMagnifyingLensPower(eyeDiopter).lensHeldCloseToEye;

    common.results().html('Eyepiece magnifying power = '
            + roundToDecimal(power, 1)
            + 'x when held close to eye'
            + '<br>Telescope magnification (eyepiece diopter divided by telescope diopter) = '
            + roundToDecimal(eyeDiopter / common.diopterTVal(), 0)
            + 'x');
};
