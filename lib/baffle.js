...
MLB.baffle.plot = function () {
	...
	model = calcNewtBaffle(focalPlaneDia, focuserBarrelBottomToFocalPlaneDistance, focuserBarrelID, diagSize, diagToFocalPlaneDistance, diagtoFocuserBaffleDistance, diagToOppositeSideBaffleDistance);
	...
	scalingFactorX = canvas.width / model.oppositeSideBaffleOD *  (1 - 2 * borderFactor);
	...
	oppositeBaffleLength = scalingFactor * model.oppositeSideBaffleOD;
	...
	// draw diagonal
	diagDelta = diagSize * scalingFactor / 2 / 1.414213562373095;
	...
	oppositeSideBaffleString = 'opposite side baffle = ' + roundToDecimal(model.oppositeSideBaffleOD, decimals);
	context.fillText(oppositeSideBaffleString, cg.x, oppositeBaffleRightPt.y - border / decimals);
	...
	$('td[id=dimensions]').html(focuserBaffleString + '<br>' + oppositeSideBaffleString);
};

MLB.baffle.load = function () {
