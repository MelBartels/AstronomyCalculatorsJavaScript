...
MLB.calcLib.wavefrontError = function (mirrorDia, radiusOfCurvature, correctionFactor) {
    ...
    // 70% zone [index of 1] will have greatest deviation when smoothly under/over corrected (center and edge will have zero error);
    // wavefront error is twice that of surface error
    return Math.abs(2 * surfaceErrors[1]);
};

// same as above function but wavelength of light is now a parameter
MLB.calcLib.wavefrontErrorFrom_MirrorDia_RoC_CorrectionFactor_WavelengthLight = function (mirrorDia, radiusOfCurvature, correctionFactor, wavelengthLight) {
    var mirrorRadius = mirrorDia / 2,
        zones = [0.3, 0.7, 0.93],
        zonalRadii = [],
        readings = [],
    ...
    // 70% zone [index of 1] will have greatest deviation when smoothly under/over corrected (center and edge will have zero error);
    // wavefront error is twice that of surface error
    // allow - wave errors to be reported
    return 2 * surfaceErrors[1];
};
...
MLB.calcLib.wavefrontErrorFromCorrectionsArray = function (mirrorDia, radiusOfCurvature, wavelengthLight, correctionsArray, wavefrontErrorsArraySize) {
    ...
    */
    ...
    for (ix = 0; ix < wavefrontErrorsArraySize; ix++) {
        ...
        rmsCount += zr * zr;
        rms += surfaceError * surfaceError * zr * zr; // always ends up positive because of the double squaring
    ...
    wavefrontRMS = 2 * Math.sqrt(rms / rmsCount);
...
// same result as above but simpler calculation
MLB.calcLib.calcWavefrontErrorForSphericalMirror = function (mirrorDiaInches, focalRatio) {
	return 22.5 * mirrorDiaInches / (focalRatio * focalRatio * focalRatio);
};
...
// see binoLayoutForNewtDesigner1.jpg, binoLayoutForNewtDesigner2.jpg, binoLayoutForNewtDesigner3.jpg, binoLayoutForNewtDesigner4.jpg, eyepiece divergence study.jpg
MLB.calcLib.calcBinoscope = function (mirrorDia, focalRatio, focalPointOffsetFromEdgeOfPrimary, focalPlaneToTertiaryDistance, focalPlaneToSecondaryDistance, secondaryBendingAngleDeg, tertiaryBendingAngleDeg, fieldDia, eyepieceLength) {
    var uom = MLB.sharedLib.uom,
        focalLength,
    ...
    */
    ...
    secondaryOffset = (secondaryLowerLength - secondaryUpperLength) / 2;
    ...
    */
    ...
    primaryToP3HorizontalDistance = primaryToSecondaryDistance - secondaryToTertiaryHorizontalLength + focalPlaneToTertiaryDistance * Math.cos(tertiaryBendingAngleRad);

    return {
		mirrorDia: mirrorDia,
		focalRatio: focalRatio,
        focalLength: focalLength,
		focalPlaneToSecondaryDistance: focalPlaneToSecondaryDistance,
		focalPlaneToTertiaryDistance: focalPlaneToTertiaryDistance,
        primaryToSecondaryDistance: primaryToSecondaryDistance,
		eyepieceLength: eyepieceLength,
		secondaryBendingAngleDeg: secondaryBendingAngleDeg,
		focalPointOffsetFromEdgeOfPrimary: focalPointOffsetFromEdgeOfPrimary,
...
// refactoring of binoscope calculation above;
// this after eyepiece divergence including the eyepiece's end became an important factor; focalPointOffsetFromEdgeOfPrimary is dropped because what counts is the end of the eyepiece
// see test layout 10F5.jpg, test layout 10F5 sec tert mirrors.jpg, test layout 10F5 tert mirror.jpg
MLB.calcLib.calcBinoscope2 = function (mirrorDia, focalRatio, focalPlaneToTertiaryDistance, focalPlaneToSecondaryDistance, secondaryBendingAngleDeg, eyepieceDivergenceAngleDeg, fieldDia, eyepieceLength) {
    var uom = MLB.sharedLib.uom,
		secondaryBendingAngleRad,
		eyepieceDivergenceAngleRad,
        focalLength,
		primaryMirrorToSecondaryHorizontalLength,
		secondaryToTertiaryLength,
		secondaryToTertiaryHorizontalLength,
		secondaryToTertiaryVerticalLength,
		primaryMirrorToTertiaryHorizontalLength,
		secondaryToP2HorizontalLength,
		secondaryToP2VerticalLength,
		primaryMirrorToP2HorizontalLength,
		tertiaryToFocalPlaneHorizontalLength,
		tertiaryToFocalPlaneVerticalLength,
		primaryMirrorToFocalPlaneHorizontalLength,
		primaryMirrorToFocalPlaneVerticalLength,
		primaryMirrorEdgeToFocalPlaneVerticalLength,
		focalPlaneToEyepieceEndHorizontalLength,
		focalPlaneToEyepieceEndVerticalLength,
		primaryMirrorToEyepieceEndHorizontalLength,
		primaryMirrorToEyepieceEndVerticalLength,
		primaryMirrorEdgeToEyepieceEndVerticalLength,
		secondaryAngleDeg,
		secondaryAngleRad,
		tertiaryAngleDeg,
		tertiaryAngleRad,
		angleAtP1BetweenMirrorCenterandMirrorEdgeDeg,
		angleAtP1BetweenMirrorCenterandMirrorEdgeRad,
		angleSecondaryUpperHalfSlantedToP1Deg,
		angleSecondaryLowerHalfSlantedToP1Deg,
		secondaryUpperHalfSlantedLength,
		secondaryLowerHalfSlantedLength,
		secondaryMajorAxisLength,
		secondaryMinorAxisLength,
		secondaryOffset,
		secondaryUpperEndHorizontalLength,
		secondaryUpperEndVerticalLength,
		secondaryLowerEndHorizontalLength,
		secondaryLowerEndVerticalLength,
		P2AngleDeg,
		P2AngleRad,
		P2EndHorizontalLength,
		P2EndVerticalLength,
		P3EndHorizontalLength,
		P3EndVerticalLength,
		secondaryToP2UpperSlope,
		secondaryToP2LowerSlope,
		yUpperCross,
		yLowerCross,
		yUpperCrossAngleRad,
		tertiaryUpperCrossAngleRad,
		tertiaryUpperLength,
		tertiaryUpperPointHorizontalLength,
		tertiaryUpperPointVerticalLength,
		yLowerCrossAngleRad,
		tertiaryLowerCrossAngleRad,
		tertiaryLowerLength,
		tertiaryLowerPointHorizontalLength,
		tertiaryLowerPointVerticalLength,
		tertiaryLength,
		tertiaryMinorAxisLength,
		tertiaryOffset;

	secondaryBendingAngleRad = secondaryBendingAngleDeg * uom.degToRad;
	eyepieceDivergenceAngleRad = eyepieceDivergenceAngleDeg * uom.degToRad;

    focalLength = mirrorDia * focalRatio;

	// secondary...
	primaryMirrorToSecondaryHorizontalLength = focalLength - focalPlaneToSecondaryDistance;

	// tertiary...
	// this length can be angled per the secondaryBendingAngleDeg, that is, this length is not necessarily vertical
	secondaryToTertiaryLength = focalPlaneToSecondaryDistance - focalPlaneToTertiaryDistance;
	// bending angle cannot exceed 90 deg, so secondaryToTertiaryHorizontalLength is always negative
	secondaryToTertiaryHorizontalLength = -secondaryToTertiaryLength * Math.cos(secondaryBendingAngleRad);
	secondaryToTertiaryVerticalLength = secondaryToTertiaryLength * Math.sin(secondaryBendingAngleRad);
	primaryMirrorToTertiaryHorizontalLength = primaryMirrorToSecondaryHorizontalLength + secondaryToTertiaryHorizontalLength;

	// P2...
	secondaryToP2HorizontalLength = -focalPlaneToSecondaryDistance * Math.cos(secondaryBendingAngleRad);
	secondaryToP2VerticalLength = focalPlaneToSecondaryDistance * Math.sin(secondaryBendingAngleRad);
	primaryMirrorToP2HorizontalLength = primaryMirrorToSecondaryHorizontalLength + secondaryToP2HorizontalLength;

	// focal plane (P3)...
	tertiaryToFocalPlaneHorizontalLength = focalPlaneToTertiaryDistance * Math.cos(eyepieceDivergenceAngleRad);
	tertiaryToFocalPlaneVerticalLength = focalPlaneToTertiaryDistance * Math.sin(eyepieceDivergenceAngleRad);
	primaryMirrorToFocalPlaneHorizontalLength = primaryMirrorToTertiaryHorizontalLength + tertiaryToFocalPlaneHorizontalLength;
	primaryMirrorToFocalPlaneVerticalLength = secondaryToTertiaryVerticalLength + tertiaryToFocalPlaneVerticalLength;
	primaryMirrorEdgeToFocalPlaneVerticalLength = primaryMirrorToFocalPlaneVerticalLength - mirrorDia / 2;

	// eyepiece end...
	focalPlaneToEyepieceEndHorizontalLength = eyepieceLength * Math.cos(eyepieceDivergenceAngleRad);
	focalPlaneToEyepieceEndVerticalLength = eyepieceLength * Math.sin(eyepieceDivergenceAngleRad);
	primaryMirrorToEyepieceEndHorizontalLength = primaryMirrorToFocalPlaneHorizontalLength + focalPlaneToEyepieceEndHorizontalLength;
	primaryMirrorToEyepieceEndVerticalLength = primaryMirrorToFocalPlaneVerticalLength + focalPlaneToEyepieceEndVerticalLength;
	primaryMirrorEdgeToEyepieceEndVerticalLength = primaryMirrorToEyepieceEndVerticalLength - mirrorDia / 2;

	// angles...
	secondaryAngleDeg = 90 - secondaryBendingAngleDeg / 2;
	secondaryAngleRad = secondaryAngleDeg * uom.degToRad;
	tertiaryAngleDeg = secondaryAngleDeg + eyepieceDivergenceAngleDeg / 2;
	tertiaryAngleRad = tertiaryAngleDeg * uom.degToRad;

	// secondary size
	angleAtP1BetweenMirrorCenterandMirrorEdgeRad = Math.atan(1 / (focalRatio * 2));
	angleAtP1BetweenMirrorCenterandMirrorEdgeDeg = angleAtP1BetweenMirrorCenterandMirrorEdgeRad / uom.degToRad;
	// triangle angles must add to 180
	angleSecondaryUpperHalfSlantedToP1Deg = 180 - secondaryAngleDeg - angleAtP1BetweenMirrorCenterandMirrorEdgeDeg;
	angleSecondaryLowerHalfSlantedToP1Deg = secondaryAngleDeg - angleAtP1BetweenMirrorCenterandMirrorEdgeDeg;
	// law of sines
	secondaryUpperHalfSlantedLength = focalPlaneToSecondaryDistance / Math.sin(angleSecondaryUpperHalfSlantedToP1Deg * uom.degToRad) * Math.sin(angleAtP1BetweenMirrorCenterandMirrorEdgeRad);
	secondaryLowerHalfSlantedLength = focalPlaneToSecondaryDistance / Math.sin(angleSecondaryLowerHalfSlantedToP1Deg * uom.degToRad) * Math.sin(angleAtP1BetweenMirrorCenterandMirrorEdgeRad);
	// sizing
	secondaryMajorAxisLength = secondaryUpperHalfSlantedLength + secondaryLowerHalfSlantedLength;
	secondaryMinorAxisLength = focalPlaneToSecondaryDistance / focalRatio;
	secondaryOffset = (secondaryLowerHalfSlantedLength - secondaryUpperHalfSlantedLength) / 2;
	// end points
	secondaryUpperEndHorizontalLength = secondaryUpperHalfSlantedLength * Math.cos(secondaryAngleRad);
	secondaryUpperEndVerticalLength = secondaryUpperHalfSlantedLength * Math.sin(secondaryAngleRad);
	secondaryLowerEndHorizontalLength = -secondaryLowerHalfSlantedLength * Math.sin(uom.qtrRev - secondaryAngleRad);
	secondaryLowerEndVerticalLength = -secondaryLowerHalfSlantedLength * Math.cos(uom.qtrRev - secondaryAngleRad);

	// P2
	P2AngleDeg = 90 - secondaryBendingAngleDeg;
	P2AngleRad = P2AngleDeg * uom.degToRad;
	P2EndHorizontalLength = fieldDia / 2 * Math.cos(P2AngleRad);
	P2EndVerticalLength = fieldDia / 2 * Math.sin(P2AngleRad);

	// P3 (focal plane field); angled at eyepieceDivergenceAngleDeg
	P3EndHorizontalLength = fieldDia / 2 * Math.sin(eyepieceDivergenceAngleRad);
	P3EndVerticalLength = fieldDia / 2 * Math.cos(eyepieceDivergenceAngleRad);

	// tertiary: see test layout 10F5 tert mirror.jpg
	secondaryToP2UpperSlope = (-secondaryToTertiaryVerticalLength + secondaryUpperEndVerticalLength - (secondaryToP2VerticalLength - secondaryToTertiaryVerticalLength + P2EndVerticalLength)) /
	(-secondaryToTertiaryHorizontalLength + secondaryUpperEndHorizontalLength - (secondaryToP2HorizontalLength - secondaryToTertiaryHorizontalLength + P2EndHorizontalLength));
	secondaryToP2LowerSlope = (-secondaryToTertiaryVerticalLength + secondaryLowerEndVerticalLength - (secondaryToP2VerticalLength - secondaryToTertiaryVerticalLength - P2EndVerticalLength)) /
	(-secondaryToTertiaryHorizontalLength + secondaryLowerEndHorizontalLength  - (secondaryToP2HorizontalLength - secondaryToTertiaryHorizontalLength - P2EndHorizontalLength));
	yUpperCross = (-secondaryToTertiaryVerticalLength + secondaryUpperEndVerticalLength) / -secondaryToP2UpperSlope + -secondaryToTertiaryHorizontalLength + secondaryUpperEndHorizontalLength;
	yLowerCross = (-secondaryToTertiaryVerticalLength + secondaryLowerEndVerticalLength) / -secondaryToP2LowerSlope + -secondaryToTertiaryHorizontalLength + secondaryLowerEndHorizontalLength;
	// law of sines: upper part of tertiary
	yUpperCrossAngleRad = uom.qtrRev - Math.atan(1 / -secondaryToP2UpperSlope);
	tertiaryUpperCrossAngleRad = uom.halfRev - yUpperCrossAngleRad - tertiaryAngleRad;
	tertiaryUpperLength = yUpperCross / Math.sin(tertiaryUpperCrossAngleRad) * Math.sin(yUpperCrossAngleRad);
    tertiaryUpperPointHorizontalLength = tertiaryUpperLength * Math.cos(tertiaryAngleRad);
    tertiaryUpperPointVerticalLength = tertiaryUpperLength * Math.sin(tertiaryAngleRad);
	// law of sines: lower part of tertiary
	yLowerCrossAngleRad = uom.qtrRev - Math.atan(1 / -secondaryToP2LowerSlope);
	tertiaryLowerCrossAngleRad = uom.halfRev - yLowerCrossAngleRad - tertiaryAngleRad;
	tertiaryLowerLength = -yLowerCross / Math.sin(tertiaryLowerCrossAngleRad) * Math.sin(yLowerCrossAngleRad);    // length is positive in order to graph
    tertiaryLowerPointHorizontalLength = -tertiaryLowerLength * Math.cos(tertiaryAngleRad);                       // therefore this length need to be switched back to negative
    tertiaryLowerPointVerticalLength = -tertiaryLowerLength * Math.sin(tertiaryAngleRad);                         // therefore this length need to be switched back to negative
	// length
	tertiaryLength = tertiaryUpperLength + tertiaryLowerLength;
	tertiaryMinorAxisLength = fieldDia + (secondaryMinorAxisLength - fieldDia) * (focalPlaneToSecondaryDistance - secondaryToTertiaryLength) / focalPlaneToSecondaryDistance;
	tertiaryOffset = (tertiaryLowerLength - tertiaryUpperLength) / 2;
    ...
    return {
		...
		focalPlaneToSecondaryDistance: focalPlaneToSecondaryDistance,
		secondaryBendingAngleDeg: secondaryBendingAngleDeg,
		eyepieceDivergenceAngleDeg: eyepieceDivergenceAngleDeg,
		fieldDia: fieldDia,
		eyepieceLength: eyepieceLength,

		// simple calcs
        focalLength: focalLength,
		primaryMirrorToSecondaryHorizontalLength: primaryMirrorToSecondaryHorizontalLength,
		secondaryToTertiaryLength: secondaryToTertiaryLength,
		secondaryToTertiaryHorizontalLength: secondaryToTertiaryHorizontalLength,
		secondaryToTertiaryVerticalLength: secondaryToTertiaryVerticalLength,
		primaryMirrorToTertiaryHorizontalLength: primaryMirrorToTertiaryHorizontalLength,
		secondaryToP2HorizontalLength: secondaryToP2HorizontalLength,
		secondaryToP2VerticalLength: secondaryToP2VerticalLength,
		primaryMirrorToP2HorizontalLength: primaryMirrorToP2HorizontalLength,
		tertiaryToFocalPlaneHorizontalLength: tertiaryToFocalPlaneHorizontalLength,
		tertiaryToFocalPlaneVerticalLength: tertiaryToFocalPlaneVerticalLength,
		primaryMirrorToFocalPlaneHorizontalLength: primaryMirrorToFocalPlaneHorizontalLength,
		primaryMirrorToFocalPlaneVerticalLength: primaryMirrorToFocalPlaneVerticalLength,
		primaryMirrorEdgeToFocalPlaneVerticalLength: primaryMirrorEdgeToFocalPlaneVerticalLength,
		focalPlaneToEyepieceEndHorizontalLength: focalPlaneToEyepieceEndHorizontalLength,
		focalPlaneToEyepieceEndVerticalLength: focalPlaneToEyepieceEndVerticalLength,
		primaryMirrorToEyepieceEndHorizontalLength: primaryMirrorToEyepieceEndHorizontalLength,
		primaryMirrorToEyepieceEndVerticalLength: primaryMirrorToEyepieceEndVerticalLength,
		primaryMirrorEdgeToEyepieceEndVerticalLength: primaryMirrorEdgeToEyepieceEndVerticalLength,

		// angles
		secondaryAngleDeg: secondaryAngleDeg,
		secondaryAngleRad: secondaryAngleRad,
		tertiaryAngleDeg: tertiaryAngleDeg,
		tertiaryAngleRad: tertiaryAngleRad,
		angleAtP1BetweenMirrorCenterandMirrorEdgeDeg: angleAtP1BetweenMirrorCenterandMirrorEdgeDeg,
		angleAtP1BetweenMirrorCenterandMirrorEdgeRad: angleAtP1BetweenMirrorCenterandMirrorEdgeRad,
		eyepieceDivergenceAngleRad: eyepieceDivergenceAngleRad,

		// secondary
		angleSecondaryUpperHalfSlantedToP1Deg: angleSecondaryUpperHalfSlantedToP1Deg,
		angleSecondaryLowerHalfSlantedToP1Deg: angleSecondaryLowerHalfSlantedToP1Deg,
		secondaryUpperHalfSlantedLength: secondaryUpperHalfSlantedLength,
		secondaryLowerHalfSlantedLength: secondaryLowerHalfSlantedLength,
		secondaryMajorAxisLength: secondaryMajorAxisLength,
		secondaryMinorAxisLength: secondaryMinorAxisLength,
		secondaryOffset: secondaryOffset,
		secondaryUpperEndHorizontalLength: secondaryUpperEndHorizontalLength,
		secondaryUpperEndVerticalLength: secondaryUpperEndVerticalLength,
		secondaryLowerEndHorizontalLength: secondaryLowerEndHorizontalLength,
		secondaryLowerEndVerticalLength: secondaryLowerEndVerticalLength,

		// P2
		P2AngleDeg: P2AngleDeg,
		P2AngleRad: P2AngleRad,
		P2EndHorizontalLength: P2EndHorizontalLength,
		P2EndVerticalLength: P2EndVerticalLength,

		// P3
		P3EndHorizontalLength: P3EndHorizontalLength,
		P3EndVerticalLength: P3EndVerticalLength,

		// tertiary
		secondaryToP2UpperSlope: secondaryToP2UpperSlope,
		secondaryToP2LowerSlope: secondaryToP2LowerSlope,
		yUpperCross: yUpperCross,
		yLowerCross: yLowerCross,
		yUpperCrossAngleRad: yUpperCrossAngleRad,
		tertiaryUpperCrossAngleRad: tertiaryUpperCrossAngleRad,
		tertiaryUpperLength: tertiaryUpperLength,
		tertiaryUpperPointHorizontalLength: tertiaryUpperPointHorizontalLength,
		tertiaryUpperPointVerticalLength: tertiaryUpperPointVerticalLength,
		yLowerCrossAngleRad: yLowerCrossAngleRad,
		tertiaryLowerCrossAngleRad: tertiaryLowerCrossAngleRad,
		tertiaryLowerLength: tertiaryLowerLength,
		tertiaryLowerPointHorizontalLength: tertiaryLowerPointHorizontalLength,
		tertiaryLowerPointVerticalLength: tertiaryLowerPointVerticalLength,
		tertiaryLength: tertiaryLength,
		tertiaryMinorAxisLength: tertiaryMinorAxisLength,
		tertiaryOffset: tertiaryOffset
    };
};

MLB.calcLib.calcMirrorRadian = (focalRatio) => {
...
MLB.calcLib.calcSkyFromObjectPlusContrast = function (objectSurfaceBrightness, contrastPercent) {
...
MLB.calcLib.calcMilliesLacroixTolerance = function (wavelengthLight, mirrorDia, radiusOfCurvature, zonalRadius) {
...
MLB.calcLib.calcCollimationToleranceInches = function (apertureInches, focalRatio) {
    return {
        focuserAlignment: 0.0015 * apertureInches * focalRatio * focalRatio,
        primaryMirrorAlignment: 0.0007 * focalRatio * focalRatio * focalRatio
    };
};
...
MLB.calcLib.calcToolVolume = function (mirrorDia, mirrorFocalRatio, plasterEdgeThickness) {
    var calcSagittalVolume = MLB.calcLib.calcSagittalVolume,
        sagittalVolumeCubicInches = calcSagittalVolume(mirrorDia, mirrorFocalRatio),
        cylinderVolumeCubicInches = mirrorDia * mirrorDia / 4 * Math.PI * plasterEdgeThickness,
        totalVolume = sagittalVolumeCubicInches + cylinderVolumeCubicInches;

    return totalVolume;
};

// imperial units: inches and pounds
MLB.calcLib.calcMirrorWeight = function (mirrorDia, mirrorFocalRatio, mirrorEdgeThickness, isMeniscus) {
    var glassPoundsPerCubicInch = 0.09,
        calcSagittalVolume = MLB.calcLib.calcSagittalVolume,
        cylinderVolumeCubicInches,
        sagittalVolumeCubicInches,
        totalVolumeCubicInches;

    cylinderVolumeCubicInches = mirrorDia * mirrorDia / 4 * Math.PI * mirrorEdgeThickness;
    if (isMeniscus) {
        sagittalVolumeCubicInches = 0;
    } else {
        sagittalVolumeCubicInches = calcSagittalVolume(mirrorDia, mirrorFocalRatio);
    }
    totalVolumeCubicInches = cylinderVolumeCubicInches - sagittalVolumeCubicInches;

    return totalVolumeCubicInches * glassPoundsPerCubicInch;
};

// from https://www.youtube.com/watch?v=d_eVSTIIsXs; see lens CG.jpg
// CG measured from len's flat base
MLB.calcLib.calcLensCG = function (thickness, radius) {
    var S = thickness,
        R = radius;

    return S * (4 * R - S) / (4 * (3 * R - S));
};

// the 'anti-lens' CG
// get volume, CG of cylinder, get volume, CG of sagitta, then get volume, CG of cylinder-sagitta
MLB.calcLib.calcMirrorCG = function (mirrorDia, mirrorFocalRatio, mirrorEdgeThickness, isMeniscus) {
    var calcSagittalVolume = MLB.calcLib.calcSagittalVolume,
        ...
        antiLensMomentArm,
        antiLensCG,
        meniscusMirrorVolume,
        meniscusPortionMomentArm,
        meniscusMirrorMomentArm,
        meniscusMirrorCG;

    cylinderVolume = Math.PI * mirrorDia * mirrorDia / 4 * mirrorEdgeThickness;
    cylinderCG = mirrorEdgeThickness / 2;
    cylinderMomentArm = cylinderVolume * cylinderCG;

    sagittalVolume = calcSagittalVolume(mirrorDia, mirrorFocalRatio);
    RoC = mirrorDia * mirrorFocalRatio * 2;
    sagittalCG = calcLensCG(mirrorEdgeThickness, RoC);
    sagittalMomentArm = sagittalVolume * sagittalCG;

    antiLensVolume = cylinderVolume - sagittalVolume;
    antiLensMomentArm = cylinderMomentArm - sagittalMomentArm;
    antiLensCG = antiLensMomentArm / antiLensVolume;
    if (!isMeniscus) {
        return antiLensCG;
    }

    // isMeniscus...
    // back portion of meniscus mirror is the inverse of the sagittal portion and is tacked on to the back of the mirror
    meniscusMirrorVolume = antiLensVolume + sagittalVolume;
    meniscusPortionMomentArm = sagittalVolume * (mirrorEdgeThickness + sagittalCG);
    meniscusMirrorMomentArm = antiLensMomentArm + meniscusPortionMomentArm;
    meniscusMirrorCG = meniscusMirrorMomentArm / meniscusMirrorVolume;
    return meniscusMirrorCG;
};

// derived by myself using the meniscus mirror CG formula above; mirror minimum thickness = the mirror's sagitta
MLB.calcLib.calcMinimumMeniscusMirrorDia = (mirrorDia, mirrorFocalRatio) => {
	return mirrorDia / (mirrorFocalRatio * 16);
};

MLB.calcLib.eyepieceTravel = (focalLength, distanceFromObjectiveToObject) => {
	return focalLength * focalLength / (distanceFromObjectiveToObject - focalLength);
};
