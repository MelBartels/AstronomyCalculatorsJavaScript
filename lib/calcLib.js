...
MLB.calcLib.calcFoldedNewtonian = function (mirrorDia, focalRatio, diagSize, focalPlaneToTertiaryDistance, tertiaryOffsetFromEdgeOfPrimary) {
	var diagToMirrorDistance,
		...
		focalPlaneOffsetFromEdgeOfPrimary,
		diagTooSmall,
		diagTooLarge,
		elbowAngleRad,
		elbowAngleDeg,
	...
	focalPlaneOffsetFromEdgeOfPrimary = focalPlaneToTertiaryDistance - tertiaryOffsetFromEdgeOfPrimary;
	...
	diagToMirrorDistance = focalRatio * (mirrorDia - diagSize) - focalPlaneOffsetFromEdgeOfPrimary;
	// the distance that the eyepiece location can be 'bent backwards' is one side of a triangle with the other side the mirror radius + the tertiary offset and the hypotenuse the diag * focal ratio length - the tertiary distance to the focal plane 
	eyepieceToMirrorDistance = diagToMirrorDistance - Math.sqrt(Math.pow(diagSize * focalRatio - focalPlaneToTertiaryDistance, 2) - Math.pow(mirrorDia / 2 + tertiaryOffsetFromEdgeOfPrimary, 2));
...
MLB.calcLib.createAltazConstantMotionTrackingErrors = function (latitudeRad, constantTrackRateTimeRad, spacingDeg) {
	...
	for (altDeg = 0; altDeg < 90; altDeg += spacingDeg) {
		...
		for (azDeg = 0; azDeg < 360; azDeg += azSpacingDeg) {
			...
			rmsErrorRad = Math.sqrt(rates.constantTrackRateAzError * rates.constantTrackRateAzError + rates.constantTrackRateAltError * rates.constantTrackRateAltError);
...
/*
From my 13.2 inch F3.0 ZipDob article, http://www.bbastrodesigns.com/ZipDob/ZipDob.html
...
Also, field of view = eyepiece field stop * exit pupil * 57.3 / (mirror diameter * eyepiece focal length * 25.4)
...
MLB.calcLib.calcApertureFromEyepieceExitPupilFOV = function (desiredFOVdeg, eyepieceFocalLengthmm, eyepieceFieldStopmm, exitPupilmm) {
	return 2.256 * eyepieceFieldStopmm * exitPupilmm / desiredFOVdeg / eyepieceFocalLengthmm;
};

MLB.calcLib.calcFOVFromEyepieceExitPupilMirrorDiameter = function (mirrorDiameterInches, eyepieceFocalLengthmm, eyepieceFieldStopmm, exitPupilmm) {
	return 2.256 * eyepieceFieldStopmm * exitPupilmm / mirrorDiameterInches / eyepieceFocalLengthmm;
};
