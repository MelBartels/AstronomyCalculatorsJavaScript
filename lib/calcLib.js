...
MLB.calcLib.calcSagittalVolumeParabolic = function (mirrorDia, focalRatio) {
    var sagitta = MLB.calcLib.calcSagittaParabolic(mirrorDia, focalRatio),
        RoC = mirrorDia * focalRatio * 2;
...
MLB.calcLib.glassRemovalDuringPolishingFrom10MicronAluminumOxideCubicInches = function (mirrorDia) {
    return mirrorDia * mirrorDia / 4 * Math.PI * 0.000393701;
};
...
// from http://www.telescope-optics.net/newtonian.htm
MLB.calcLib.calcDiagOffset3 = function (diagMinorAxis, diagToEyeDistance) {
...
MLB.calcLib.calcFoldedNewtonian = function (mirrorDia, focalRatio, diagSize, focalPlaneToTertiaryDistance, tertiaryOffsetFromEdgeOfPrimary, focalPointToDiagDistance) {
...
MLB.calcLib.VisualDetectCalc = function () {
    ...
    this.includeResultAsJSON = function (result) {
        ...
        p = r.parms;
        json = [
            {label: 'aperture (inches)', result: p.apertureIn},
            {label: 'eye limiting magnitude', result: p.eyeLimitMag},
            {label: 'eye max exit pupil (mm)', result: p.exitPupilmm},
            {label: 'sky background brightness', result: p.bkgndBrightEye},
            {label: 'object name', result: p.objName},
            {label: 'object integrated magnitude', result: p.objMag},
            {label: 'object dimensions (arcminutes)', result: p.maxObjArcmin + ' x ' + p.minObjArcmin},
            {label: 'eyepiece apparent field (degrees)', result: p.apparentFoV},
            {label: 'best  detection magnification \'X\'', result: Math.round(r.x)},
            {label: 'minimum useful magnification', result: Math.round(r.minX)},
            {label: 'maximum useful magnification', result: Math.round((r.maxXMinObj + r.maxXMaxObj) / 2)},
            {label: 'actual field (degrees)', result: rnd(r.actualFoV)},
            {label: 'Does the object fit into the field? ', result: r.fitsFoV},
            {label: 'faintest detectable star', result: rnd(r.limitMag)},
            {label: 'magnitude reduction using one eye', result: rnd(r.singleEyeMagChange)},
            {label: 'magnitude reduction due to scope transmission factor', result: rnd(r.scopeTransMagChange)},
            {label: 'object surface brightness (magnitude/arcsec^2)', result: rnd(r.surfBrightObj)},
            {label: 'object brightness reduction at \'X\' (magnitude/arcsec^2)', result: rnd(r.brightReductionAtX)},
            {label: 'object brightness at \'X\' (magnitude/arcsec^2)', result: rnd(r.surfBrightObjAtX)},
            {label: 'sky background brightness at \'X\' (magnitude/arcsec^2)', result: rnd(r.bkgndBrightAtX)},
            {label: 'brightness of object plus sky at \'X\' (magnitude/arcsec^2)', result: rnd(r.surfBrightObjPlusBkgndAtX)},
            {label: 'object contrast (log)', result: rnd(r.logContrastObject)},
            {label: 'detection contrast required (log)', result: rnd(r.logContrastRequired)},
            {label: 'object contrast minus detection contrast required (log)', result: rnd(r.logContrastDiff)},
...
MLB.calcLib.calcMirrorCellDimensions = function (supportPoints, radius) {
    var calcMinimumMirrorSupportPoints = MLB.calcLib.calcMinimumMirrorSupportPoints,
