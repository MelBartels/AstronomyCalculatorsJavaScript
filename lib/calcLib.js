...
// where corrections is an array of correction factors (1=perfectly parabolized) for the 30, 70 and 93% zones;
// also see mirrors.xls spreadsheet
    ...
    */
    for (ix = 0; ix < wavefrontErrorsArraySize; ix++) {
        ...
        wavefrontErrors.push({
                ...
                wavefrontError: surfaceError * 2
        });
    }
    return wavefrontErrors;
};
...
// binoscope secondary and tertiary mirrors calculator; see binoLayoutForNewtDesigner1.jpg, binoLayoutForNewtDesigner2.jpg, binoLayoutForNewtDesigner3.jpg

MLB.calcLib.calcBinoscope = function (mirrorDia, focalRatio, tertiaryOffsetFromEdgeOfPrimary, focalPlaneToTertiaryDistance, focalPlaneToSecondaryDistance, fieldDia) {
    var uom = MLB.sharedLib.uom,
        ...
        tertiarySizedForP3FieldAndFocalRatioMajorAxis,
        tertiarySizedForP3FieldAndFocalRatioMinorAxis,
        tertiarySizedForP3FieldAndFocalRatioOffset;
    ...
    secondaryToTertiaryVerticalLength = mirrorDia / 2 + tertiaryOffsetFromEdgeOfPrimary;
    secondaryToTertiaryHorizontalLength = Math.sqrt(Math.pow(secondaryToTertiaryDistance, 2) - Math.pow(secondaryToTertiaryVerticalLength, 2));

    // if no elbow, then angle is 90 deg
    elbowAngleRad = Math.acos(secondaryToTertiaryHorizontalLength / secondaryToTertiaryDistance);
...
MLB.calcLib.calcFoldedNewtonian = function (mirrorDia, focalRatio, diagSize, tertiaryOffsetFromEdgeOfPrimary, focalPointToTertiaryDistance, focalPointToDiagDistance) {
...
MLB.calcLib.getFoldedNewtonianScalingFactor = function (width, height, focalRatio, diagSize, focalPointToTertiaryDistance, tertiaryOffsetFromEdgeOfPrimary, diagToMirrorDistance) {
    var maxWidth,
        maxHeight,
...
B=.66*100=66cmDeg   <-needs P2 magnification factor of 1.15 to match: either 76/1.15=66 or 66*1.14=76)
    76 reduced to 66 can be thought of as stretching or decrearcsing the solid angle subtended by the focal ratio from 2.74 to 3.15
    66 increased to 76 can be thought of as increarcsing the pupil or apparent eyepiece field of view by 1.15
...
MLB.calcLib.VisualDetectCalc = function () {
    ...
    this.includeResultAsJSON_NewtDesigner = function (result, telescopeFLmm) {
        ...
        json = [
            ...
            {label: 'Object surface brightness (and at X)', result: rnd(r.surfBrightObj) + ' (' + rnd(r.surfBrightObjAtX) + ') MPAS'},
            {label: 'Sky background brightness (and at X)', result: p.bkgndBrightEye + ' (' + rnd(r.bkgndBrightAtX) + ') MPAS'},
            {label: 'Object+sky brightness (and at X)', result: rnd(r.objPlusBkgnd) + ' (' + rnd(r.surfBrightObjPlusBkgndAtX) + ') MPAS'},
            {label: 'Illumination multipler at X', result: rnd(r.illumReductionAtX)},
