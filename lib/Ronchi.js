...
MLB.ronchi.setNameIndexedElementValue = function (name, ix, val) {
    return MLB.ronchi.getNameIndexedElement(name, ix).val(val);
};
...
MLB.ronchi.setNameIndexedElementChBoxValue = function (name, ix, val) {
    return MLB.ronchi.getNameIndexedElement(name, ix).prop('checked', val);
};
...
MLB.ronchi.displayMLTolerances = function () {
    ...
    for (ix = 0; ix < constants.zoneCount; ix += 1) {
        ...
        setNameIndexedElementValue(constants.MLToleranceIdLit, ix, displayToleranceStr);
    }
};
