...
MLB.ronchi.constants = {
    ...
    exampleImages: {
    // mirrorDia, radiusOfCurvature, centralObstruction, gratingFreq, gratingOffsetSeries, userParabolicCorrection, RonchigramSize, borderSize, invertBands, RonchiTape, pastedImageTransparency, pastedImageWidth, pastedImageHeight, pastedImageOffsetX, pastedImageOffsetY
        '16in': [16.25, 93.6, 4, 65, -0.05, 1, 500, 0, true, false, 0.86, 500, 500, 0, 0, '16/final Ronchigram inside RoC.jpg'],
        '16out': [16.25, 93.6, 4, 65, 0.674, 1, 500, 0, true, false, 0.86, 500, 500, 0, 0, '16/final Ronchigram outside RoC.jpg'],
        '25': [25.1, 131, 5.5, 65, 0.776, 1, 400, 10, true, false, 0.8, 454, 461, -16, -19, 'RonchiExamples/25 example.jpg'],
        '13': [13.1, 80.6, 3.1, 65, 0.570, 1, 250, 10, true, false, 0.75, 304, 320, -20, -28, 'RonchiExamples/13 example.jpg'],
        '10': [10.4, 58.4, 3.1, 100, 0.362, 1, 400, 10, true, false, 0.9, 416, 428, 7, -5, 'RonchiExamples/10 example.jpg'],
        '6': [5.9, 33.9, 2.14, 100, 0.294, 1, 400, 10, true, false, 0.9, 530, 496, -60, -40, 'RonchiExamples/6 example.jpg']
    },
...
MLB.ronchi.plotRonchiBands = function (mirrorDia, radiusOfCurvature, gratingFreq, gratingOffset, scalingFactor, imageData, cg, invertBands, RonchiTape, userParabolicCorrection) {
    ...
    scaledTapeHeight = scaledMirrorRadius / 20;
    ...
    for (y = 0; y < scaledMirrorRadius; y++) {
        ...
        for (x = 0; x < scaledMirrorRadius; x++) {
            ...
            z = scaledRadiusOfCurvature + userParabolicCorrection * scaledMirrorZoneSquared * correctionFactor / scaledRadiusOfCurvature;
            ...
            if (RonchiTape) { // inverts bands for the horizontal Ronchi tape height
                if ((band && !invertBands) || (!band && invertBands)) { // draw bands
                    if (y > scaledTapeHeight) { // draw the regions above/below tape
                        quadrantSetPixel(imageData, cg, x, y, red, green, blue, opaque);
                    }
                } else { // don't draw bands
                    if (y < scaledTapeHeight) { // instead draw black tape if fits within height
                        quadrantSetPixel(imageData, cg, x, y, 0, 0, 0, opaque);
                    }
                }
            } else { // not RonchiTape
                if ((band && !invertBands) || (!band && invertBands)) { // draw band
                    quadrantSetPixel(imageData, cg, x, y, red, green, blue, opaque);                }
            }
        }
    }
...
MLB.ronchi.matchingRonchiTapeBand = function (mirrorDia, radiusOfCurvature, gratingFreq, gratingOffset, invertBands, RonchiTape) {
    var constants = MLB.ronchi.constants,
...
MLB.ronchi.drawRonchigramOnCanvas = function (canvas, gratingOffset, invertBands, RonchiTape, userParabolicCorrection) {
    var constants = MLB.ronchi.constants,
    ...
    plotRonchiBands(mirrorDia, radiusOfCurvature, gratingFreq, gratingOffset, scalingFactor, imageData, ronchiCenter, invertBands, RonchiTape, userParabolicCorrection);
    matchingRonchiTapeBand(mirrorDia, radiusOfCurvature, gratingFreq, gratingOffset, invertBands, RonchiTape);
...
MLB.ronchi.plotRonchigrams = function () {
    ...
    for (ix = 0; ix < gratingOffsetSeriesValuesLength; ix++) {
        ...
        drawRonchigramOnCanvas($('[id=RonchiCanvas' + ix + ']')[0], gratingOffsetSeriesValues[ix], invertBands, RonchiTape, userParabolicCorrection);
    }
    ...
    matchingTapeBandStr += '<br>See the <a href="#MatchingRonchiTape">Matching Ronchi tape band discussion</a> below.';
...
MLB.ronchi.resetCorrectionFactors = function () {
    ...
    for (ix = 0; ix < constants.zoneCount; ix += 1) {
        ...
        if (userEnteredParabolicCorrection !== '' && !isNaN(userEnteredParabolicCorrection)) {
