...
MLB.ronchi.drawRonchigramOnCanvas = function (canvas, gratingOffset, invertBands) {
    ...
    if (constants.drawZonalRuler()) {
        for (ix = 1; ix < constants.numZonalRulers; ix += 1) {
            drawCircle(context, circleCenter, scaledRadius * ix / constants.numZonalRulers, constants.rulerThickness, constants.rulerColor);
        }
        ...
        constants.zonesForAnalysis.forEach(z => drawCircle(context, circleCenter, scaledRadius * z, constants.rulerThickness, constants.rulerColor));
    }
};
