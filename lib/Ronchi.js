...
transitions for 1st Ronchigram w/ defaults as above: [0, 0.528, 1.488, 2.352, 3.12, 3.792, 4.368, 4.896, 5.376, 5.808] (length = 10)
...
and for inverted bands #1: 1.01, 2.74, 4.08, 5.14, 5.69; Ronchigram #2: 4.06, 5.4; Ronchigram #3: 1.06, 3.79; Ronchigram #4: 0.53, 1.56, 2.64, 3.96, 5.16

scaling up the Ronchigram size makes these values more accurate
*/
...
MLB.ronchi.createTapeBandsFromTransitions = function (mirrorRadius, transitions) {
    ...
    while (true) {
        ...
        transitionB = transitions[ix++];
        tapeMarks.push(roundToDecimal((transitionA + transitionB ) / 2, tapeBandPrecision));
        // if transitionsLength is 10: ix is 9 at the end of the transitions sequence, ix is 10 if at the end of invertBand transitions sequence
        if (ix >= transitionsLength - 1) {
            if (invertBands) {
                // keep transitionB already set above
                tapeMarks.push(roundToDecimal((mirrorRadius + transitionB ) / 2, tapeBandPrecision));
            }
            break;
        }
    }
...
MLB.ronchi.drawRonchigramOnCanvas = function (canvas, gratingOffset, userParabolicCorrection) {
    ...
    calcRonchiBands(mirrorDia, radiusOfCurvature, gratingFreq, gratingOffset, scalingFactor, userParabolicCorrection);
    processPixelArray();
...
MLB.ronchi.copyBandPositionsToErrorTable = function () {
    ...
    setNameIndexedElementChBoxValue(constants.useZonalCorrectionIdLit, ix, true);
    // subsequent entries
    for (ix = 1; ix < constants.zoneCount; ix++) {
        tapeIx = ix - 1;
