...
MLB.ronchi.constants = {
    ...
    corrections: [],
    defaultZonesAndCorrections: [[1, 1], [0, 1]],
    zoneIdLit: 'id=zone',
    zoneId2Lit: 'id="zone',
...
MLB.ronchi.pasteExampleRonchigram = function (exampleImage) {
    var constants = MLB.ronchi.constants,
        ...
        setPastedImageActive = MLB.ronchi.setPastedImageActive,
        plotRonchigrams = MLB.ronchi.plotRonchigrams,
        displayMLTolerances = MLB.ronchi.displayMLTolerances,
        canvas = constants.pastedImageRonchiCanvas(),
        image = new Image(),
        ix = 0;
...
MLB.ronchi.putData = function () {
    ...
    for (ix = 0; ix < constants.zoneCount; ix += 1) {
        zone = parseFloat($('[' + constants.zoneIdLit + ix + ']').val());
        correction = parseFloat($('[' + constants.zoneCorrectionIdLit + ix + ']').val());
        useCorrection = $('[' + constants.useZonalCorrectionIdLit + ix + ']').is(':checked');
...
MLB.ronchi.getData = function () {
    ...
    for (ix = 0; ix < constants.zoneCount; ix += 1) {
        ...
        $('[' + constants.zoneIdLit + ix + ']').val(row.zone);
        $('[' + constants.zoneCorrectionIdLit + ix + ']').val(row.correctionFactor);
        $('[' + constants.useZonalCorrectionIdLit + ix + ']').prop('checked', row.use);
    }
};
...
MLB.ronchi.buildZoneAndCorrectionTable = function () {
    ...
    for (ix = 0; ix < constants.zoneCount; ix += 1) {
        htmlStr = '<tr>\r\n'
                ...
                + '<td> <input class="inputText" ' + constants.MLToleranceId2Lit + ix + '" readonly="readonly" type="text"> </td>\r\n'
...
MLB.ronchi.buildZoneAndCorrectionTableDefaultValues = function () {
    ...
    for (ix = 0; ix < constants.zoneCount; ix += 1) {
        ...
        $('[' + constants.zoneIdLit + ix + ']').val(constants.defaultZonesAndCorrections[ix][0]);
        $('[' + constants.zoneCorrectionIdLit + ix + ']').val(constants.defaultZonesAndCorrections[ix][1]);
    }
};
...
MLB.ronchi.sortZonalCorrectionTableByZone = function () {
    ...
    for (ix = 0; ix < constants.zoneCount; ix += 1) {
        ...
        $('[' + constants.zoneIdLit + ix + ']').val(val);

        val = sortedCorrections[ix][1];
        if (isNaN(val)) {
            val = '';
        }
        $('[' + constants.zoneCorrectionIdLit + ix + ']').val(val);

        $('[' + constants.useZonalCorrectionIdLit + ix + ']').prop('checked', sortedCorrections[ix][2]);
    }
 };

MLB.ronchi.displayMLTolerances = function () {
    var constants = MLB.ronchi.constants,
        roundToDecimal = MLB.sharedLib.roundToDecimal,
    ...
    for (ix = 0; ix < constants.zoneCount; ix += 1) {
        ...
        if (isNaN(zone) || isNaN(correction)) {
            ...
            if (MLTolerance === Infinity) {
                ...
                $('[' + constants.MLToleranceIdLit + ix + ']').val();
            } else {
        ...
        $('[' + constants.MLToleranceIdLit + ix + ']').val(displayToleranceStr);
    }
};
...
$(window).ready(function () {
    ...
    for (ix = 0; ix < constants.zoneCount; ix += 1) {
        $('[' + constants.zoneIdLit + ix + ']').change(zonalErrorTableAltered);
        $('[' + constants.zoneCorrectionIdLit + ix + ']').change(zonalErrorTableAltered);
        $('[' + constants.useZonalCorrectionIdLit + ix + ']').change(zonalErrorTableAltered);
    }
