...
MLB.ronchi.calcRonchiBands = function (radiusOfCurvature, gratingFreq, gratingOffset, scalingFactor, userParabolicCorrection) {
    var constants = MLB.ronchi.constants,
...
MLB.ronchi.drawRonchigramOnCanvas = function (canvas, gratingOffset, userParabolicCorrection) {
    ...
    transitions = calcRonchiBands(radiusOfCurvature, gratingFreq, gratingOffset, scalingFactor, userParabolicCorrection);
...
MLB.ronchi.calcNulledStretchedScaledIx = (v, RGD, DRF, maxDistortion, scalingFactor) => {
    var constants = MLB.ronchi.constants,
        int = MLB.sharedLib.int,
        mirrorDia = constants.UIMirrorDia,
        RonchiBandRelativeX,
...
MLB.ronchi.calcNullRonchiBandRulers = function (RonchiCenter, RGD, DRF, maxDistortion, scalingFactor) {
    ...
    RonchiBands[0].forEach(t => {
        nullIxStretchScaled = calcNulledStretchedScaledIx (t, RGD, DRF, maxDistortion, scalingFactor);
...
MLB.ronchi.calcZonalCircularRulers = function (RonchiCenter, RGD, DRF, maxDistortion, scalingFactor) {
    ...
    for (zIx = 0; zIx < zoneCount; zIx++) {
        ...
        nullIxStretchScaled = calcNulledStretchedScaledIx (z, RGD, DRF, maxDistortion, scalingFactor);
...
MLB.ronchi.drawZonalCircularRulers = (context, RonchigramSize) => {
    ...
    zonalCircularRulersNulled.forEach(ix => drawCircle(context, point(RonchigramSize / 2, RonchigramSize / 2), ix, rulerThickness, rulerColor));
};
...
MLB.ronchi.calcCentralObstructionNull = (RonchiCenter, RGD, DRF, maxDistortion, scalingFactor) => {
    ...
    nullIxStretchScaled = calcNulledStretchedScaledIx(centralObstruction / 2, RGD, DRF, maxDistortion, scalingFactor);
...
MLB.ronchi.processZonalErrors = function () {
    ...
    for (ix = 0; ix <= zoneCount; ix++) {
        ...
        calcWaveErrorForZone(+zone, radiusOfCurvature, userParabolicCorrection, userEnteredZonalCorrection, wavelengthLight, offsetRoC, waveErrors);
    }
...
MLB.ronchi.calcWaveErrorForZone = function (zone, radiusOfCurvature, userParabolicCorrection, userEnteredZonalCorrection, wavelengthLight, offsetRoC, waveErrors) {
    var constants = MLB.ronchi.constants,
        mirrorDia = constants.UIMirrorDia,
        calcSphereParabolaDifference = MLB.calcLib.calcSphereParabolaDifference,
        calcParabolicCorrectionForZone = MLB.calcLib.calcParabolicCorrectionForZone,
