...
MLB.ronchi.constants = {
	...
	RonchigramSize: function () {
		return +$('[name=RonchigramSize]').val();
	},
	canvas: function () {
	...
	errorZone: function () {
		return +$('[name=errorZone]').val();
	},
	wavefrontDeviation: function () {
		return +$('[name=wavefrontDeviation]').val();
	},
	...
	errorZone2: function () {
		return +$('[name=errorZone2]').val();
	},
	wavefrontDeviation2: function () {
		return +$('[name=wavefrontDeviation2]').val();
	},
...
MLB.ronchi.plotRonchigrams = function () {
	var constants = MLB.ronchi.constants,
		...
		decimalWaves = 1,
		decimalPercent = 0,
		calcSphereParabolaDiff = MLB.calcLib.calcSphereParabolaDiff,
		...
		RonchigramSize = constants.RonchigramSize(),
	...
	zoneSqrt = constants.errorZone() * constants.errorZone();
	zoneSqrt2 = constants.errorZone2() * constants.errorZone2();
	wavesCorrection = calcSphereParabolaDiff(mirrorDia, radiusOfCurvature);
	allowableParabolicDeviation = calcAllowableParabolicDeviationForQuarterWavefront(focalRatio) * constants.wavefrontDeviation() / 0.25;
	wavefrontDeviation = calcAllowableParabolicDeviationForQuarterWavefront(focalRatio) * constants.wavefrontDeviation2() / 0.25;
	...
	for (ix = 0; ix < gratingOffsetSeriesLength; ix++) {
		constants.Ronchigrams().append("<canvas id='RonchiCanvas" + ix + "' width='" + RonchigramSize + "' height='" + RonchigramSize + "'></canvas>");
	...
	if (displayErrorRonchigram) {
		for (ix = 0; ix < gratingOffsetSeriesLength; ix++) {
			constants.errorRonchigrams().append("<canvas id='errorRonchiCanvas" + ix + "' width='" + RonchigramSize + "' height='" + RonchigramSize + "'></canvas>");
	...
	if (displayErrorRonchigram2) {
		for (ix = 0; ix < gratingOffsetSeriesLength; ix++) {
			constants.errorRonchigrams2().append("<canvas id='errorRonchiCanvas2" + ix + "' width='" + RonchigramSize + "' height='" + RonchigramSize + "'></canvas>");
...
$(window).ready(function () {
	...
	constants.btnPlot2().click(function () {
		plotRonchigrams();
	});
	constants.btnPlot3().click(function () {
		plotRonchigrams();
	});
