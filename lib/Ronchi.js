...
MLB.ronchi.constants = {
    ...
    // mirrorDia, radiusOfCurvature, centralObstruction, gratingFreq, gratingOffsetSeries, userParabolicCorrection, RonchigramSize, borderSize, invertBands, RonchiTape, RonchiGrid, pastedImageTransparency, pastedImageWidth, pastedImageHeight, pastedImageOffsetX, pastedImageOffsetY, imageSource, displayUnwrappedRonchigram, unwrapY, stretchX, unwrapBandRuler
        '16in':  [16.25, 93.6, 0,  65, -0.05, 1, 500, 0,  true,  false, false,  0.9, 500, 500,   1,   0, MLB.base64images._16insideRoC,  true,  false, 1, true],
        '16out': [16.25, 93.6, 0,  65, 0.674, 1, 500, 0,  false, false, false,  0.9, 500, 500,   0,   0, MLB.base64images._16outsideRoC, true,  false, 2, true],
        '25':    [25.1, 131,   0,  65, 0.776, 1, 400, 10, true,  false, false,  0.8, 454, 461, -16, -19, MLB.base64images._25,           false, false, 2, true],
        '13':    [13.1,  80.6, 0,  65, 0.570, 1, 250, 10, true,  false, false, 0.75, 304, 320, -20, -28, MLB.base64images._13,           true,  false, 2, true],
        '10in':  [10.4,  58.4, 0, 100, -0.04, 1, 250,  0, true,  false, false,  0.9, 250, 250,   0,   0, MLB.base64images._10insideRoC,  true,  false, 1, true],
        '10':    [10.4,  58.4, 0, 100, 0.356, 1, 400, 10, true,  false, false,  0.9, 416, 428,   7,  -5, MLB.base64images._10,           true,  false, 2, true],
        '6':     [5.9,   33.9, 0, 100, 0.294, 1, 400, 10, true,  false, false,  0.9, 530, 496, -60, -40, MLB.base64images._6,            true,  false, 2, true]
    },
    images: {},
    ...
    btnPasteExampleRonchigram10in: function () {
        return $('input[id=btnPasteExampleRonchigram10in]');
    },
    ...
    btnUnwrapExample: function () {
        return $('input[id=btnUnwrapExample]');
    },
...
MLB.ronchi.setImagePixels = function (imageData, circleCenter) {
    ...
    for (x = 0; x < xyLength; x++) {
        for (y = 0; y < xyLength; y++) {
            ...
            if (!pixelSet) {
                setImagePixelsQuadrants(imageData, circleCenter, x, y, backgroundRed, backgroundGreen, backgroundBlue, opaque);
            }
        }
    }
};
...
MLB.ronchi.calcRonchiBands = function (mirrorDia, radiusOfCurvature, gratingFreq, gratingOffset, scalingFactor, userParabolicCorrection) {
    var constants = MLB.ronchi.constants,
        ...
        // MLB.ronchi.constants.RonchigramDistortions[Ronchigram#][y][x]
        RonchigramDistortions = constants.RonchigramDistortions,
        RonchigramDistortionsLength = RonchigramDistortions.push([]),
        RGD = RonchigramDistortions[RonchigramDistortionsLength - 1];
    ...
    for (y = 0; y < scaledMirrorRadius; y++) {
        ...
        for (x = 0; x < scaledMirrorRadius; x++) {
            ...
            // Ronchi band numbers for this x,y...
...
MLB.ronchi.stringifyRonchigramDistortions = function (distortionIx, y) {
    var constants = MLB.ronchi.constants,
        RonchigramDistortions = constants.RonchigramDistortions,
        RGD = RonchigramDistortions[distortionIx],
        xs = RGD[y],
        s = '';
...
MLB.ronchi.plotRonchigrams = function () {
    ...
    // reset the sphere to parabolic distortions
    constants.RonchigramDistortions = [];
...
// called by plotRonchigrams(), controlled by constants.displayUnwrappedRonchigram
MLB.ronchi.unwrapFirstRonchigram = function () {
    var constants = MLB.ronchi.constants,
        ...
        ronchiCenter,
        unwrapRonchiCenter,
        // use 1st Ronchigram's parabolic distortions
        RGD = constants.RonchigramDistortions[0],
        distortionLength,
        distortion,
        dx,
        maxDistortion,
    ...
    constants.Ronchigrams().append("&emsp; <canvas id='RonchiCanvas" + unwrapRonchigramIx + "'></canvas>");

    // reset the canvas size in case it has changed
    unwrapCanvas = $('[id=RonchiCanvas' + unwrapRonchigramIx + ']')[0];
    // canvasSize is size of first Ronchigram
    unwrapCanvas.width = canvasSize * stretchX;
    unwrapCanvas.height = canvasSize;
    ...
    RGD.forEach((RGDy, yIx) => {
        RGDy.forEach((RGDx, xIx) => {
            ...
            /* eg, distortionRatios for default Ronchigram (no Y axis unwrap):
                yIx,xIx: 0,0   = NaN
                         0,1   = 0.577
                         0,124 = 1
                         62,1  = 0.683
                   ...
                   distortionRatio for 16F2.9 example (no Y axis unwrap):
                         0,1   = 0.126
            steps determined empirically: balance between zigzags and filling in skipped pixels;
            */
            xSteps = 0.5 + distortionRatio * 2;
            if (unwrapY) {
                ySteps = xSteps;
            } else {
                ySteps = 1;
            ...
            xDir = 1;
            yDir = -1;
            unwrapSetPixel(yIx, xIx, yDir, xDir, ySteps, xSteps, distortionOffsetY, distortionOffsetX, ronchiCenter, unwrapRonchiCenter, unwrapImageData, origImg);
    ...
    unwrapContext.fillStyle = constants.rulerColor;
    unwrapContext.font = constants.font;
    unwrapContext.fillText('Unwrapped', 10, 15);
    unwrapContext.fillText('Ronchigram', 10, 30);
    ...
    //constants.tapes set in createTapeBandsFromTransitions(), called from drawRonchigramOnCanvas(), called from plotRonchigrams()
    // eg, [0, '2.62', '4.34', '5.54']
    constants.tapes[0].forEach(t => {
        ...
        // eg, 0, 54, 90, 115
        wrappedIx = int(tapeBandRelativeX * maxIx);
        ...
        // eg, 0, 35, 71, 108
        unwrapIx = int(wrappedIx * distortionRatio);
        unwrapIxStretch = int(wrappedIx * distortionRatio * stretchX);

        // vertical ruler
        drawLine(unwrapContext, constants.rulerColor, constants.rulerThickness, point(unwrapRonchiCenter.x + unwrapIxStretch, 0), point(unwrapRonchiCenter.x + unwrapIxStretch, unwrapCanvas.height));
        drawLine(unwrapContext, constants.rulerColor, constants.rulerThickness, point(unwrapRonchiCenter.x - unwrapIxStretch, 0), point(unwrapRonchiCenter.x - unwrapIxStretch, unwrapCanvas.height));
        // horizontal ruler
        if (unwrapY) {
            drawLine(unwrapContext, constants.rulerColor, constants.rulerThickness, point(0, unwrapRonchiCenter.y + unwrapIx), point(unwrapCanvas.width, unwrapRonchiCenter.y + unwrapIx));
            drawLine(unwrapContext, constants.rulerColor, constants.rulerThickness, point(0, unwrapRonchiCenter.y - unwrapIx), point(unwrapCanvas.width, unwrapRonchiCenter.y - unwrapIx));
        }
    });
};

MLB.ronchi.unwrapSetPixel = function (yIx, xIx, yDir, xDir, ySteps, xSteps, distortionOffsetY, distortionOffsetX, ronchiCenter, unwrapRonchiCenter, unwrapImageData, origImg) {
    var setPixel = MLB.sharedLib.setPixel,
        originalY = ronchiCenter.y + yIx * yDir,
        originalX = ronchiCenter.x + xIx * xDir,
        canvasWidth = ronchiCenter.x * 2,
        OIx = (originalY * canvasWidth + originalX) * 4,
        newY = unwrapRonchiCenter.y + distortionOffsetY * yDir,
        newX = unwrapRonchiCenter.x + distortionOffsetX * xDir,
        y,
        x;

    // origImg[x]...origImg[x+3] is r,g,b,alpha
    // origImg[OIx] = origImg[OIx + 1] = origImg[OIx + 2] = 0; // test which pixels are set by setting them to black
    for(y = 0; y < ySteps; y++) {
        for (x = 0; x < xSteps; x++) {
            setPixel(unwrapImageData, newX + x * xDir, newY + y * yDir, origImg[OIx], origImg[OIx + 1], origImg[OIx + 2], origImg[OIx + 3]);
        }
    }
};
...
MLB.ronchi.plotMirrorSurfaceProfile = function () {
    ...
    waveErrorAvg = (highestWaveError + lowestWaveError) / 2;
    MSPZonesStats.wavePV = waveErrorRange;
    MSPZonesStats.waveRMS = Math.sqrt(rms / rmsCount);

    xScale = MSPChartSize.x / 2;
    if (waveErrorRange === 0) {
        yScale = MSPChartSize.y * YScalingFactor;
    } else {
        yScale = MSPChartSize.y / waveErrorRange * YScalingFactor;
    }
    ...
    MSPZones.forEach(z => {
        MSPChartData.push({
            x: z.zone * xScale,
            y: (z.waveError - waveErrorAvg) * yScale,
        });
    });

    // draw grid denoting mirror zones
    MSPChartData.forEach(d => {
        drawLine(context, constants.rulerColor, constants.rulerThickness, point(chartCenter.x + d.x, 0), point(chartCenter.x + d.x, MSPChartSize.y));
        drawLine(context, constants.rulerColor, constants.rulerThickness, point(chartCenter.x - d.x, 0), point(chartCenter.x - d.x, MSPChartSize.y));
    });
    ...
    // write out wave error results
    constants.MSPResultsLabel().html('Wavefront P-V = '
            + roundToDecimal(MSPZonesStats.wavePV, constants.wavefrontAnalysisPrecision)
            + ' and RMS = '
            + roundToDecimal(MSPZonesStats.waveRMS, constants.wavefrontAnalysisPrecision)
            + ' waves (uses the mirror surface profile data and the RoC offset).'
    );
};

MLB.ronchi.calcPlotMirrorSurfaceProfile = function () {
    MLB.ronchi.calcMirrorSurfaceProfile();
    MLB.ronchi.plotMirrorSurfaceProfile();
};

MLB.ronchi.buildAnalyzeDisplayZonalCorrections = function () {
    MLB.ronchi.calcAndDisplayRoCChangesAndMLTolerances();
    MLB.ronchi.buildCorrectionsArray();
    MLB.ronchi.analyzeZonalErrorTable();
};

MLB.ronchi.processChangeInZonalErrorTable = function () {
    MLB.ronchi.buildAnalyzeDisplayZonalCorrections();
    MLB.ronchi.calcPlotMirrorSurfaceProfile();
    MLB.ronchi.plotRonchigrams();
};
...
MLB.ronchi.analyzeZonalErrorTable = function () {
    ...
    constants.zonalErrorsResultsLabel().html('Wavefront P-V = '
            + roundToDecimal(analysis.wavefrontPV * userParabolicCorrection, constants.wavefrontAnalysisPrecision)
            + ' and RMS = '
            + roundToDecimal(analysis.wavefrontRMS * userParabolicCorrection, constants.wavefrontAnalysisPrecision)
            + ' waves (best fit curve for the 30,70,93% zones, does not use the RoC offset).'
            + '<p>Weighted ML tolerance = '
            + roundToDecimal(constants.weightedMLTolerance, constants.wavefrontAnalysisPrecision)
            + ' (based on all zones checked "Use").');
};
...
$(window).ready(function () {
    var constants = MLB.ronchi.constants,
        ...
        buildAnalyzeDisplayZonalCorrections = MLB.ronchi.buildAnalyzeDisplayZonalCorrections,
        calcPlotMirrorSurfaceProfile = MLB.ronchi.calcPlotMirrorSurfaceProfile,
    ...
    constants.btnSortZonalCorrectionTableByZone().click(() => {
        ...
        buildAnalyzeDisplayZonalCorrections();
        calcPlotMirrorSurfaceProfile();
    });
    ...
    constants.backgroundBandColorRGB().change(plotRonchigrams);
    ...
    constants.btnPasteExampleRonchigram10in().click(() => { pasteExampleRonchigram(constants.exampleImages['10in']); });
    ...
    constants.btnUnwrapExample().click(() => { pasteExampleRonchigram(constants.exampleImages['16in']); });
    constants.unwrapY().change(plotRonchigrams);
    constants.stretchX().change(plotRonchigrams);
    constants.unwrapBandRuler().change(plotRonchigrams);
