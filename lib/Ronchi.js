...
MLB.ronchi.constants = {
    ...
    exampleImages: {
    // mirrorDia, radiusOfCurvature, centralObstruction, gratingFreq, gratingOffset, userParabolicCorrection, RonchigramSize, borderSize, invertBands, RonchiGrid, zonalRuler, pastedImageTransparency, pastedImageWidth, pastedImageHeight, pastedImageOffsetX, pastedImageOffsetY, imageSource, distortionResolutionFactor, nullRuler, showBullseyeZones
          '13': [13.1,  80.6,  0,  65, -0.104,  1, 400,  0, false, false, false, 0.75, 400, 400,   0,   0, MLB.base64images._13,   3,  true, false],
        ...
        'grid': [13.1,  80.6,  0,  65, -0.454,  1, 400,  0, true,  true,  false,  0.9, 398, 402,   0,  -5, MLB.base64images._grid, 2,  true, false]
    },
    images: {},
...
MLB.ronchi.nullFirstRonchigram = function () {
    var constants = MLB.ronchi.constants,
        ...
        nullRuler = constants.nullRuler()[constants.yes].checked,
    ...
    nullContext = nullCanvas.getContext("2d"); // or .getContext("2d", {willReadFrequently: true});
    nullImageData = nullContext.createImageData(nullCanvas.width, nullCanvas.height);
    ...
    nullContext.putImageData(nullImageData, 0, 0);

    // draw rulers...

    if (nullRuler) {
        drawnullRuler(nullCanvas, nullContext, nullRonchiCenter, RGD, DRF, maxDistortion);
    }

    scalenullRonchigram(distortionLength, maxX, canvasSize, nullContext);

    // note if parabolic correction != 1
    noteParabolicCorrection(nullContext, origRonchiCenter.x);
    // relies on context fillStyle and font being set
    nullContext.fillText(constants.nullLit, 2, constants.fontSize);

    constants.RonchigramCount++;
};

MLB.ronchi.scalenullRonchigram = function (distortionLength, maxX, canvasSize, nullContext) {
    ...
    if (maxX !== 0 && scalingFactor > 1) {
        ...
        nullContext.putImageData(scaleImageData(nullContext.getImageData(0, 0, canvasSize, canvasSize), scalingFactor), -cornerOffsetXY, -cornerOffsetXY);
    }

};

MLB.ronchi.drawnullRuler = function (nullCanvas, nullContext, nullRonchiCenter, RGD, DRF, maxDistortion) {
    var constants = MLB.ronchi.constants,
        mirrorDia = +constants.mirrorDia().val(),
        RonchiGrid = constants.RonchiGrid()[constants.yes].checked,
        int = MLB.sharedLib.int,
    ...
    constants.tapes[0].forEach(t => {
        ...
        leftVerticalRulerIx = nullRonchiCenter.x - nullIxStretch;
        rightVerticalRulerIx = nullRonchiCenter.x + nullIxStretch;
        nullTapes.push(leftVerticalRulerIx);
        nullTapes.push(rightVerticalRulerIx);
        drawLine(nullContext, constants.rulerTextColor, constants.rulerThickness, point(leftVerticalRulerIx, 0), point(leftVerticalRulerIx, nullCanvas.height));
        drawLine(nullContext, constants.rulerTextColor, constants.rulerThickness, point(rightVerticalRulerIx, 0), point(rightVerticalRulerIx, nullCanvas.height));
        if (RonchiGrid) {
            // horizontal ruler
            drawLine(nullContext, constants.rulerTextColor, constants.rulerThickness, point(0, nullRonchiCenter.y + nullIx), point(nullCanvas.width, nullRonchiCenter.y + nullIx));
            drawLine(nullContext, constants.rulerTextColor, constants.rulerThickness, point(0, nullRonchiCenter.y - nullIx), point(nullCanvas.width, nullRonchiCenter.y - nullIx));
        }
        /*
    ...
    //console.log(s);
    /* default example:
        tapeBandRelativeX: 0, nulledIx: 0, distortion: NaN, distortionRatio: NaN, nullIx: 0, nullIxStretch: 0
        tapeBandRelativeX: 0.4366666666666667, nulledIx: 109, distortion: 1.1420292620135208, distortionRatio: 0.656452788320534, nullIx: 36, nullIxStretch: 36
        tapeBandRelativeX: 0.7233333333333333, nulledIx: 180, distortion: 1.3870024448314904, distortionRatio: 0.7972664559502765, nullIx: 72, nullIxStretch: 72
        tapeBandRelativeX: 0.9233333333333333, nulledIx: 230, distortion: 1.6313480674892065, distortionRatio: 0.9377193941042165, nullIx: 108, nullIxStretch: 108
       set FR to 3 and grating offset to 0.36 for outside of RoC:
        tapeBandRelativeX: 0, nulledIx: 0, distortion: NaN, distortionRatio: NaN, nullIx: 0, nullIxStretch: 0
        tapeBandRelativeX: 0.17166666666666666, nulledIx: 42, distortion: 0.9802078792556594, distortionRatio: 0.9802188794885313, nullIx: 21, nullIxStretch: 21
        tapeBandRelativeX: 0.37666666666666665, nulledIx: 94, distortion: 0.9009377016076294, distortionRatio: 0.9009478122430675, nullIx: 42, nullIxStretch: 42
        tapeBandRelativeX: 0.6966666666666667, nulledIx: 174, distortion: 0.6613751340490557, distortionRatio: 0.6613825562302523, nullIx: 58, nullIxStretch: 58
        tapeBandRelativeX: 0.9633333333333334, nulledIx: 240, distortion: 0.3577106518283049, distortionRatio: 0.3577146661813356, nullIx: 43, nullIxStretch: 43 <= note the wrap around (should match with the 3rd band's 42)
    */
};
...
MLB.ronchi.pasteExampleRonchigram = function (exampleImage) {
    ...
    constants.nullRuler()[constants.yes].checked = exampleImage[ix++];
    constants.nullRuler()[constants.no].checked = !constants.nullRuler()[constants.yes].checked;
...
MLB.ronchi.getData = function () {
    ...
    constants.nullRuler()[constants.yes].checked = parsedData.nullRuler;
    constants.nullRuler()[constants.no].checked = !parsedData.nullRuler;
...
MLB.ronchi.chainWaveErrors = (waveErrors) => {
    ...
    waveErrors.reverse().forEach((we, ix) => {
        for (ixA = ix; ixA < waveErrorsLength; ixA += 1) {
...
MLB.ronchi.getAndProcessImage = () => {
    var constants = MLB.ronchi.constants,
        processImage = MLB.ronchi.processImage,
        canvasSize = constants.canvasSize(),
        RonchigramSize = +constants.RonchigramSize().val(),
        drawLine = MLB.sharedLib.drawLine,
        point = MLB.sharedLib.point,
    ...
    processImage(imageData, newImageData, canvasSize, RonchigramSize);

    // copy the new image data to the canvas
    context.putImageData(newImageData, 0, 0);

    // for example, draw the nullTapes
    nullTapes.forEach(ix => {
        drawLine(context, constants.rulerTextColor, constants.rulerThickness, point(ix, 0), point(ix, canvas.height));
    });
};

MLB.ronchi.processImage = (imageData, newImageData, canvasSize, RonchigramSize) => {
    ...
    for (yIx = 0; yIx < canvasSize; yIx += 1) {
        ...
        for (xIx = 0; xIx < canvasSize; xIx += 1) {
            ...
            // if outside the Ronchigram then color the pixel black with no transparency, then bail to the next pixel
            ...
            // for example, grayscale the image
...
$(window).ready(function () {
    ...
    constants.nullRuler()[constants.yes].checked = true;
    ...
    constants.nullRuler().change(godProcess);
