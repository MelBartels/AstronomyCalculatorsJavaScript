...
MLB.ronchi.unwrapFirstRonchigram = function () {
    ...
    // apply the distortion quadrant which unwrapes only in 'x' and apply to each original Ronchigram quadrant, filling in gaps between the distortion 'x's;
    ...
    RGD.forEach((RGDy, yIx) => {
        ...
        RGDy.forEach((RGDx, xIx) => {
            ...
            // left side: retrieving data is center - x and putting data is center - x
            // upper left quadrant
            originalY = ronchiCenter.y - yIx;
            originalX = ronchiCenter.x - xIx;
            OIx = (originalY * canvasSize + originalX) * 4;
