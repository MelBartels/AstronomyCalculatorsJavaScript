    ...
    RGD.forEach((RGDy, yIx) => {
        RGDy.forEach((RGDx, xIx) => {
            ...
            */
            ...
            // note todo
            //x -> unwrapRonchiCenter.x + distortionOffsetX * xDir for xSteps

            // lower right quadrant
            xDir = 1;
            yDir = 1;
            unwrapSetPixel(yIx, xIx, yDir, xDir, ySteps, xSteps, distortionOffsetY, distortionOffsetX, origRonchiCenter, unwrapRonchiCenter, unwrapImageData, origImg);
...
MLB.ronchi.unwrapSetPixel = function (yIx, xIx, yDir, xDir, ySteps, xSteps, distortionOffsetY, distortionOffsetX, origRonchiCenter, unwrapRonchiCenter, unwrapImageData, origImg) {
    var setPixel = MLB.sharedLib.setPixel,
        origY = origRonchiCenter.y + yIx * yDir,
        origX = origRonchiCenter.x + xIx * xDir,
    ...
    for(y = 0; y < ySteps; y++) {
        for (x = 0; x < xSteps; x++) {
            setPixel(unwrapImageData, newX + x * xDir, newY + y * yDir, origImg[origIx], origImg[origIx + 1], origImg[origIx + 2], origImg[origIx + 3]);
        }
    }
};
