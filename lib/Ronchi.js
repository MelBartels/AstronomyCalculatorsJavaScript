...
MLB.ronchi.constants = {
    ...
    parabolicCorrectionPrecision: 2,
    // ...rounding precisions
    ...
    exampleImages: {
    // mirrorDia, radiusOfCurvature, centralObstruction, gratingFreq, gratingOffsetSeries, userParabolicCorrection, RonchigramSize, borderSize, invertBands, RonchiTape, RonchiGrid, pastedImageTransparency, pastedImageWidth, pastedImageHeight, pastedImageOffsetX, pastedImageOffsetY, imageSource, distortionResolutionFactor, unwrapBandRuler, showBullseyeZones, invertMSP
          '16': [16.25, 93.6,  0,  65,  -0.05,  1, 400,  0,  true, false, false,  0.9, 400, 400,   1,   0, MLB.base64images._16,   3,  true, false, false],
          '25': [25.1,   131,  0,  65,  0.776,  1, 400, 10,  true, false, false,  0.9, 454, 461, -16, -19, MLB.base64images._25,   2, false, false, false],
           '6': [ 5.9,  33.9,  0, 100,  0.294,  1, 400, 10,  true, false, false,  0.9, 530, 496, -60, -40, MLB.base64images._6,    2, false, false, false],
          '13': [13.1,  80.6,  0,  66, -0.104,  1, 400,  0, false, false, false, 0.75, 400, 400,   0,   0, MLB.base64images._13,   3, false, false, false],
        'grid': [13.1,  80.6,  0,  65, -0.454,  1, 400,  0,  true, true,  true,   0.9, 398, 402,   0,  -5, MLB.base64images._grid, 2,  true, false, false]
    },
    images: {},
...
MLB.ronchi.noteParabolicCorrection = function (context, ronchiCenterX) {
    ...
    txt = constants.parabCorrLit + roundToDecimal(parabCorr, constants.parabolicCorrectionPrecision);
...
MLB.ronchi.plotRonchigramsCalcRonchiTaps = function () {
    var constants = MLB.ronchi.constants,
        ...
        unwrapFirstRonchigram = MLB.ronchi.unwrapFirstRonchigram,
        adjustRonchigramDivDiscussionDivWidths = MLB.ronchi.adjustRonchigramDivDiscussionDivWidths;
    ...
    unwrapFirstRonchigram();

    adjustRonchigramDivDiscussionDivWidths();
};
...
MLB.ronchi.pasteExampleRonchigram = function (exampleImage) {
    ...
    constants.invertMSP()[constants.yes].checked = exampleImage[ix++];
    constants.invertMSP()[constants.no].checked = !constants.invertMSP()[constants.yes].checked;
...
MLB.ronchi.putData = function () {
    ...
    dataToSave = {
        ...
        invertMSP: constants.invertMSP()[constants.yes].checked
    };
...
MLB.ronchi.getData = function () {
    ...
    constants.invertMSP()[constants.yes].checked = parsedData.invertMSP;
    constants.invertMSP()[constants.no].checked = !parsedData.invertMSP;
};
...
MLB.ronchi.calcMirrorSurfaceProfilePlotWriteResults = function () {
    var constants = MLB.ronchi.constants,
        ...
        calcParabolicCorrectionForZone = MLB.calcLib.calcParabolicCorrectionForZone,
        calcSphereParabolaDifference = MLB.calcLib.calcSphereParabolaDifference,
        calcWavefrontError = MLB.ronchi.calcWavefrontError,
...
MLB.ronchi.calcWavefrontError = function () {
    var constants = MLB.ronchi.constants,
    ...
    constants.analysis = wavefrontErrorFromCorrectionsArray(mirrorDia, radiusOfCurvature, wavelengthLight, correctionsArray, constants.zoneCount);
};
...
MLB.ronchi.plotMirrorSurfaceProfileWriteResults = function () {
    ...
    context = canvas.getContext('2d');
    // call before setting context.fillStyle
    ...
    if (MSPZones.length === 0) {
        context.fillText('No data to plot', chartCenter.x, chartCenter.y);
        return;
    }
    ...
    MSPZones.forEach(z => {
        ...
        yWaveError = z.waveError - highestWaveError;
        if (invertMSP) {
            yWaveError = - yWaveError;
        }
        y = yWaveError / 2 * yScale + constants.MSPMarginY * MSPChartSize.y;
    ...
    context.fillText('vvv    mirror surface P-V wave error of ' + waveErrorSurfaceRangeY + '    vvv', 2, integerWaveErrorY - 4);

    // draw grid denoting mirror zones
    MSPChartData.forEach(d => {
        drawLine(context, constants.rulerTextColor, constants.rulerThickness, point(chartCenter.x + d.x, integerWaveErrorY), point(chartCenter.x + d.x, MSPChartSize.y));
        drawLine(context, constants.rulerTextColor, constants.rulerThickness, point(chartCenter.x - d.x, integerWaveErrorY), point(chartCenter.x - d.x, MSPChartSize.y));
    });
    ...
    constants.MSPResultsLabel().html('Mirror surface P-V = '
            ...
            + ' waves<br>'
            + 'Mirror surface P-V = '
            + roundToDecimal(constants.analysis.wavefrontPV * userParabolicCorrection, constants.wavefrontAnalysisPrecision)
            + ' and RMS = '
            + roundToDecimal(constants.analysis.wavefrontRMS * userParabolicCorrection, constants.wavefrontAnalysisPrecision)
            + ' waves (curve fit to 30,70,93% zones)'
    );
};
...
$(window).ready(function () {
    ...
    constants.invertMSP()[constants.no].checked = true;
    setPastedImageInactive();
    ...
    constants.showBullseyeZones().change(plotRonchigramsCalcRonchiTaps);
    ...
    constants.invertMSP().change(calcMirrorSurfaceProfilePlotWriteResults);
