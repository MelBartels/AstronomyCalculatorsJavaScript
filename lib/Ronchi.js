...
MLB.ronchi.constants = {
    ...
    // mirrorDia, radiusOfCurvature, centralObstruction, gratingFreq, gratingOffsetSeries, userParabolicCorrection, RonchigramSize, borderSize, invertBands, RonchiTape, RonchiGrid, pastedImageTransparency, pastedImageWidth, pastedImageHeight, pastedImageOffsetX, pastedImageOffsetY, imageSource, displayUnwrappedRonchigram, stretchX
        '16in':  [16.25, 93.6, 0,  65, -0.05, 1, 500, 0,  true, false, false, 0.86, 500, 500,   1,   0, MLB.base64images._16insideRoC, true, 1],
        '16out': [16.25, 93.6, 0,  65, 0.674, 1, 500, 0,  true, false, false, 0.86, 500, 500,   0,   0, MLB.base64images._16outsideRoC, true, 2],
        '25':    [25.1, 131,   0,  65, 0.776, 1, 400, 10, true, false, false, 0.8,  454, 461, -16, -19, MLB.base64images._25, true, 3],
        '13':    [13.1,  80.6, 0,  65, 0.570, 1, 250, 10, true, false, false, 0.75, 304, 320, -20, -28, MLB.base64images._13, true, 2],
        '10':    [10.4,  58.4, 0, 100, 0.356, 1, 400, 10, true, false, false, 0.9,  416, 428,   7,  -5, MLB.base64images._10, true, 2.5],
        '6':     [5.9,   33.9, 0, 100, 0.294, 1, 400, 10, true, false, false, 0.9,  530, 496, -60, -40, MLB.base64images._6, true, 2]
    },
    images: {},
...
MLB.ronchi.unwrapFirstRonchigram = function () {
    var constants = MLB.ronchi.constants,
        ...
        unwrapCanvas,
        stretchFactor = +constants.stretchX().val(),
        unwrapContext,
        unwrapImageData,
        pastedImageContext,
        originalX,
    ...
    unwrapCanvas.width = canvasSize * stretchFactor;
    ...
    // apply the distortion quadrant which unwraps in the 'x' direction and then apply to each original Ronchigram quadrant, filling in gaps between the distortion 'x's;
    ...
    // the unwrapped y is the same as the original y, the original x's are unwrapped out;
    ...
    RGD.forEach((RGDy, yIx) => {
        lastDistortionOffsetX = unwrapRonchiCenter.x;
        RGDy.forEach((RGDx, xIx) => {
            distortionOffsetX = int(xIx * RGDx.distortion / maxDistortion * stretchFactor);

            // upper right quadrant
            originalY = ronchiCenter.y - yIx;
            originalX = ronchiCenter.x + xIx;
            OIx = (originalY * canvasSize + originalX) * 4;
            for (dx = lastDistortionOffsetX; dx <= distortionOffsetX; dx++) {
                setPixel(unwrapImageData, unwrapRonchiCenter.x + dx, originalY, origImg[OIx], origImg[OIx + 1], origImg[OIx + 2], origImg[OIx + 3]);
            }
            ...
            // left side: retrieving data is center - x therefore putting data is center - x
            // upper left quadrant
            originalY = ronchiCenter.y - yIx;
            originalX = ronchiCenter.x - xIx;
            OIx = (originalY * canvasSize + originalX) * 4;
            ...
            lastDistortionOffsetX = distortionOffsetX;
        });
    });
