...
MLB.ronchi.plotRonchigramsCalcRonchiTape = () => {
	...
	constants.imperialSelected() ? mirrorDiaUom = mirrorDia : mirrorDiaUom = mirrorDia / 25.4;
...
MLB.ronchi.setIdLabelBackground = function (id, val) {
...
MLB.ronchi.processZonalErrors = () => {
	...
	// ML: note that since the tolerance tornado is +-, the abs values are halved
	MLToleranceAbsPeakValley = (MLToleranceHighestPeakValley - MLToleranceLowestPeakValley) / 2;
	constants.MLErrorsLabel().html(roundToDecimal(MLToleranceAbsPeakValley, constants.MLToleranceAbsDecimalPrecision));
...
$(window).ready(function () {
	...
	constants.btnIncreaseGratingOffset().click(() => {
		var gratingOffsetChange;

		if (constants.metricSelected()) {
			gratingOffsetChange = constants.gratingOffsetChangeMetric;
		} else {
			gratingOffsetChange = constants.gratingOffsetChangeImperial;
		}
		setGratingOffsetFromOffset(gratingOffsetChange);
		godProcess();
	});
