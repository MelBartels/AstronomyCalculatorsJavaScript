...
MLB.ronchi.calcMirrorSurfaceProfile = function () {
    ...
    for (ix = 0; ix < constants.zoneCount; ix++) {
        ...
        idealZonalRoC = adjustedIdealRoC + mirrorZoneSquared / adjustedIdealRoC * userParabolicCorrection;
        actualZonalRoC = radiusOfCurvature + mirrorZoneSquared / radiusOfCurvature * userParabolicCorrection * userEnteredZonalParabolicCorrection;
...
MLB.ronchi.plotMirrorSurfaceProfile = function () {
    ...
    /* convert from vertical data to horizontal chart:
       eg, if high 70% zone that needs more polishing,
       then zonal parab correction needs to be > 1,
       ideal - actual of connected x endpoints = a positive number: see above comments in calcMirrorSurfaceProfile() where > 1 parab correction swings the zone's x endpoint closer to focus, increasing the x value,
       d.diffX > 0, therefore MSPChartData[3].y > 0,
       therefore points[ix].y > other zone y's,
       plot shows rise (lower y values) to the inside of the zone,
       in the star test, this zone appears long, high, undercorrected,
       at the polishing station, the inside of this zone needs to be polished down, on towards the center of the mirror, so that this zone focuses less 'long'
    */
    MSPPlotData.forEach(d => {
        MSPChartData.push({
