...
MLB.ronchi.plotRonchiBands = function (mirrorDia, radiusOfCurvature, gratingFreq, gratingOffset, scalingFactor, imageData, cg, invertBands, RonchiTape, RonchiGrid, userParabolicCorrection) {
    ...
    for (y = 0; y < scaledMirrorRadius; y++) {
        ...
        for (x = 0; x < scaledMirrorRadius; x++) {
            ...
            // for spherical mirror, zonalRoC=RoC; for full parabolic mirror w/ no zonal error, zonalRoC=RoC + mirrorZone/RoC; mirrorZone/RoC = 0 @ mirror center, sagitta*2 @ mirror edge
            ...
            if (RonchiTape) { // inverts bands in a strip of scaledTapeHeight height (doubles when quadrant is copied 4x)
                if ((band && !invertBands) || (!band && invertBands)) { // draw Ronchi band
                    if (y > scaledTapeHeight) { // draw the regions above/below tape, otherwise don't draw
                        quadrantSetPixel(imageData, cg, x, y, red, green, blue, opaque);
                    }
                } else { // don't draw bands
                    if (y < scaledTapeHeight) { // instead draw black tape if fits within height, inverting the Ronchi band if it fits within the tape
                ...
                if ((band && !invertBands) || (!band && invertBands)) { // draw Ronchi band
                    quadrantSetPixel(imageData, cg, x, y, red, green, blue, opaque);
                    if (RonchiGrid) {
                        quadrantSetPixel(imageData, cg, y, x, red, green, blue, opaque);
                    }
                }
            }
        }
...
MLB.ronchi.drawRonchigramOnCanvas = function (canvas, gratingOffset, invertBands, RonchiTape, RonchiGrid, userParabolicCorrection) {
    var constants = MLB.ronchi.constants,
    ...
    plotRonchiBands(mirrorDia, radiusOfCurvature, gratingFreq, gratingOffset, scalingFactor, imageData, ronchiCenter, invertBands, RonchiTape, RonchiGrid, userParabolicCorrection);
...
MLB.ronchi.plotRonchigrams = function () {
    ...
    for (ix = 0; ix < gratingOffsetSeriesValuesLength; ix++) {
        ...
        drawRonchigramOnCanvas($('[id=RonchiCanvas' + ix + ']')[0], gratingOffsetSeriesValues[ix], invertBands, RonchiTape, RonchiGrid, userParabolicCorrection);
    }
