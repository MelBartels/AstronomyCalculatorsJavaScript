...
MLB.ronchi.constants = {
	...
	// mirrorDia, radiusOfCurvature, centralObstruction, gratingFreq, gratingOffset, userParabolicCorrection, RonchigramSize, invertBands, RonchiGrid, circularZonalRulerCount, pastedImageTransparency, pastedImageWidth, pastedImageHeight, pastedImageOffsetX, pastedImageOffsetY, imageName, imageSource, distortionResolutionFactor, showBullseyeZones, thinBands
		'thirteenInside':        [13.1,  80.6, 3.1,  65, -0.104,  1, 400, false, false, 1, 0.75, 400, 400,   0,   0, '', MLB.base64images._13i,  3, false, false],
		'thirteenInsideThinned': [13.1,  80.6, 3.1,  65, -0.106,  1, 400, true,  false, 1, 1.0,  400, 400,  -1,   0, '', MLB.base64images._13i,  3, false, true ],
		'thirteenOutside':       [13.1,  80.6, 3.1,  65,  0.576,  1, 400, false, false, 1, 0.75, 400, 400,   0,   0, '', MLB.base64images._13o,  3, false, false],
		'thirteenGrid':          [13.1,  80.6, 3.1,  65, -0.454,  1, 400, true,  true,  1,  0.9, 398, 402,   0,  -5, '', MLB.base64images._grid, 2, false, false],
		'twelveF5Under':         [12,     120,   3, 100,   -0.2,  1, 400, false, false, 1, 0.85, 400, 400,   0,   0, '', MLB.base64images._12F5, 2, false, false],
		'twelveF5Under2':        [12,     120,   3, 100,   -0.2,  1, 400, false, false, 1,    1, 400, 400,   0,   0, '', MLB.base64images._12F5, 2, false, true ]
	},
	images: {},
	...
	btnPasteExampleRonchigram12F5Under2: () => {
		return $('input[id=btnPasteExampleRonchigram12F5Under2]');
	},
	pastedImageActive: () => {
		return $('[name=pastedImageActive]');
	},
	pastedImageActiveIsChecked: function () {    // not () => because of scope issues
...
note that transitions exclude user entered zonal correction factors
...
*/
MLB.ronchi.createTapeBandsFromTransitions = (mirrorRadius, transitions) => {
	var constants = MLB.ronchi.constants,
...
MLB.ronchi.calcRonchiBands = (gratingFreq, gratingOffset, scalingFactor, userParabolicCorrection) => {
	...
	// do it all again but with no userErrorCorrectionFactor; this so that the Ronchi bands and the software nulled graph are not distorted by user entered zonal correction factors
	for (y = 0; y < scaledMirrorRadius; y++) {
		ySquared = y * y;
		for (x = 0; x < scaledMirrorRadius; x++) {
			scaledMirrorZoneSquared = ySquared + x * x;
...
MLB.ronchi.plotRonchigramsCalcRonchiTape = () => {
	...
	if (constants.imperialSelected()) {
		mirrorDiaUom = mirrorDia;
	} else {
		mirrorDiaUom = mirrorDia / 25.4;
	}
	wavesCorrection = inchesToWavesGreenLight(calcSphereParabolaDifference(mirrorDiaUom, focalRatio));
...
MLB.ronchi.calcNullRonchiBandRulers = (RonchiCenter, DRF, maxDistortion, scalingFactor) => {
	var constants = MLB.ronchi.constants,
	...
	// for example, if there are 7 bands across the mirror's face, which is a center band with 3 bands to each side,
	// then there will be 8 nullRonchiBands (the center and 3 bands to one side all being duplicated) with values [200, 200, 143, 257, 85, 315, 27, 373]
...
MLB.ronchi.processZonalErrors = () => {
	...
	// ML:
	// note that since the tolerance tornado is +-, the abs values are halved
	MLToleranceAbsPeakValley = (MLToleranceHighestPeakValley - MLToleranceLowestPeakValley) / 2;
	if (MLToleranceAbsPeakValley > 1) {
		MLBackgroundColor = constants.badColor;
		MLWarning = ' (exceeds tolerance)';
	} else if (MLToleranceAbsPeakValley > 0.5) {
	...
	constants.MLErrorsLabel().html(roundToDecimal(MLToleranceAbsPeakValley, constants.MLToleranceAbsDecimalPrecision) + MLWarning);
