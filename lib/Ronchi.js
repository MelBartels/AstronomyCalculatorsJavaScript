...
MLB.ronchi.calcRonchiBands = function (mirrorDia, radiusOfCurvature, gratingFreq, gratingOffset, scalingFactor, userParabolicCorrection) {
    ...
    for (y = 0; y < scaledMirrorRadius; y++) {
        ...
        for (x = 0; x < scaledMirrorRadius; x++) {
            ...
            xGratingLineNumberDoubled = int((xAtGrating / scaledLineWidth) + 0.5);
            // if modulus xGratingLineNumberDoubled is true, then ray passes through grating
            band = xGratingLineNumberDoubled % 2 === 0;
            ...
            xGratingLineNumberDoubledSphere = Math.abs(scaledGratingOffset * 2 * x / scaledRadiusOfCurvature / scaledLineWidth); // no parab correction
            RGD[y][x] = {sphere: xGratingLineNumberDoubledSphere, parabola: xAtGrating / scaledLineWidth};
        }
    }
...
MLB.ronchi.stringifyRonchigramDistortions = function (distortionIx, y) {
    ...
    xs.forEach(x => s += x.sphere + ', ' + x.parabola + ';');
