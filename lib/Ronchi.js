...
MLB.ronchi.unwrapFirstRonchigram = function () {
    ...
    for (yIx = 0; yIx < RGD.length; yIx++) {
        for (xIx = 0; xIx < RGD[yIx].length; xIx++) {
            ...
            if (unwrapY) {
                distortionOffsetY = int(yIx * distortion / maxDistortion);
            } else {
                distortionOffsetY = yIx;
            }
            distortionOffsetX = int(xIx * distortion / maxDistortion * stretchX);

            // upper right quadrant
            originalY = ronchiCenter.y - yIx;
            originalX = ronchiCenter.x + xIx;
            OIx = (originalY * canvasSize + originalX) * 4;
            newY = unwrapRonchiCenter.y - distortionOffsetY;
            newX = unwrapRonchiCenter.x + distortionOffsetX;

            // origImg[OIx] = origImg[OIx + 1] = origImg[OIx + 2] = 0; // test which pixels are set by setting them to black
            setPixel(unwrapImageData, newX, newY, origImg[OIx], origImg[OIx + 1], origImg[OIx + 2], origImg[OIx + 3]);
            setPixel(unwrapImageData, newX + 1, newY, origImg[OIx], origImg[OIx + 1], origImg[OIx + 2], origImg[OIx + 3]);
            setPixel(unwrapImageData, newX, newY - 1, origImg[OIx], origImg[OIx + 1], origImg[OIx + 2], origImg[OIx + 3]);
            setPixel(unwrapImageData, newX + 1, newY - 1, origImg[OIx], origImg[OIx + 1], origImg[OIx + 2], origImg[OIx + 3]);
