...
MLB.ronchi.drawnullRuler = function (nullCanvas, nullContext, nullRonchiCenter, RGD, DRF, maxDistortion) {
    ...
    constants.tapes[0].forEach(t => {
        ...
        // vertical ruler
        drawLine(nullContext, constants.rulerTextColor, constants.rulerThickness, point(nullRonchiCenter.x + nullIxStretch, 0), point(nullRonchiCenter.x + nullIxStretch, nullCanvas.height));
        drawLine(nullContext, constants.rulerTextColor, constants.rulerThickness, point(nullRonchiCenter.x - nullIxStretch, 0), point(nullRonchiCenter.x - nullIxStretch, nullCanvas.height));
        if (RonchiGrid) {
            // horizontal ruler
