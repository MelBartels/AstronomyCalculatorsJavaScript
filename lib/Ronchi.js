...
MLB.ronchi.constants = {
    ...
    exampleImages: {
    // mirrorDia, radiusOfCurvature, centralObstruction, gratingFreq, gratingOffsetSeries, userParabolicCorrection, RonchigramSize, borderSize, invertBands, RonchiTape, RonchiGrid, pastedImageTransparency, pastedImageWidth, pastedImageHeight, pastedImageOffsetX, pastedImageOffsetY, imageSource, stretchX, distortionResolutionFactor, unwrapBandRuler, showBullseyeZones
          '16': [16.25, 93.6,  0,  65,  -0.05,  1, 400,  0,  true, false, false,  0.9, 400, 400,   1,   0, MLB.base64images._16,   1, 3,  true, false],
          '25': [25.1,   131,  0,  65,  0.776,  1, 400, 10,  true, false, false,  0.9, 454, 461, -16, -19, MLB.base64images._25,   1, 2, false, false],
           '6': [ 5.9,  33.9,  0, 100,  0.294,  1, 400, 10,  true, false, false,  0.9, 530, 496, -60, -40, MLB.base64images._6,    1, 2, false, false],
          '13': [13.1,  80.6,  0,  66, -0.104,  1, 400,  0, false, false, false, 0.75, 400, 400,   0,   0, MLB.base64images._13,   1, 3, false, false],
        'grid': [13.1,  80.6,  0,  65, -0.454,  1, 400,  0,  true, true,  true,   0.9, 398, 402,   0,  -5, MLB.base64images._grid, 1, 2,  true, false]
    },
    images: {},
...
MLB.ronchi.getAndFixStretchX = function () {
    var constants = MLB.ronchi.constants,
        stretchX = +constants.stretchX().val();

    if (stretchX === 0) {
        stretchX = 1;
        constants.stretchX().val('1');
    }
    return stretchX;
};
...
MLB.ronchi.adjustRonchigramDivDiscussionDivWidths = function () {
    var constants = MLB.ronchi.constants,
        ...
        RonchigramSize = +constants.RonchigramSize().val(),
        stretchX = MLB.ronchi.getAndFixStretchX(),
    ...
    divWidth += (stretchX - 1) * RonchigramAndBorderSize;
...
MLB.ronchi.unwrapFirstRonchigram = function () {
    ...
    if (unwrapBandRuler) {
        drawUnwrapBandRuler(unwrapCanvas, unwrapContext, unwrapRonchiCenter, stretchX, RGD, DRF, maxDistortion);
    }

    constants.RonchigramCount++;
};

MLB.ronchi.drawUnwrapBandRuler = function (unwrapCanvas, unwrapContext, unwrapRonchiCenter, stretchX, RGD, DRF, maxDistortion) {
    var constants = MLB.ronchi.constants,
        mirrorDia = +constants.mirrorDia().val(),
        int = MLB.sharedLib.int,
        point = MLB.sharedLib.point,
