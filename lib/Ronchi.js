...
MLB.ronchi.constants = {
    ...
    lastTotalStdDeviation: undefined,
    // ...parabolic distortions
...
MLB.ronchi.calcUnwrapVerticalStdDeviation = () => {
    var constants = MLB.ronchi.constants,
        calcVerticalStdDeviation = MLB.ronchi.calcVerticalStdDeviation,
        int = MLB.sharedLib.int,
    ...
    totalStdDeviation = calcVerticalStdDeviation(imageData, canvasSize, RonchigramSize);

    changeStdDeviation = int(constants.lastTotalStdDeviation - totalStdDeviation);
    changeStdDeviationStr = 'Unwrap band straightness factor = ' + int(totalStdDeviation) + ';<br>';
    if (changeStdDeviation > 0) {
        changeStdDeviationStr += 'better';
    } else if (changeStdDeviation < 0) {
        changeStdDeviationStr += 'worse';
    } else {
        changeStdDeviationStr += 'no change';
    }
    changeStdDeviationStr += '; change = ' + changeStdDeviation + '.';
    constants.unwrapStraightnessLabel().html(changeStdDeviationStr);
    constants.lastTotalStdDeviation = totalStdDeviation;
};

// separate function / separation of concerns / separate calculation from visual / display components
MLB.ronchi.calcVerticalStdDeviation = (imageData, canvasSize, RonchigramSize) => {
    var int = MLB.sharedLib.int,
        ...
        stdDeviation,
        totalStdDeviation = 0,
        includeColumnHeightCriteria = 0.5;
    ...
    for (xIx = 0; xIx < canvasSize; xIx += 1) {
        ...
        if (data[xIx].count * data[xIx].count > mirrorRadiusSquared * includeColumnHeightCriteria) {
            totalStdDeviation += data[xIx].redStdDeviation + data[xIx].greenStdDeviation + data[xIx].blueStdDeviation;
        }
    }

    return totalStdDeviation;
};
...
// magic numbers to get the div to fit to the right on a single row
MLB.ronchi.adjustRonchigramDivDiscussionDivWidths = function () {
    ...
    constants.discussionDiv().width(screen.width - divWidth - 100);
};
...
MLB.ronchi.adjustZonalErrorsNotesDiv = function () {
    ...
    constants.zonalErrorsNotesDiv().width(Math.floor(screen.width - constants.zonalErrorsTableDiv().width() - constants.zonalErrorsViewDiv().width() - 150));
};
...
MLB.ronchi.processZonalErrors = function () {
    ...
    constants.waveErrorsLabel().html('Peak to valley wavefront = '
