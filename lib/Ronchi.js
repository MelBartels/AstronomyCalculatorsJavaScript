...
MLB.ronchi.constants = {
    ...
    // mirrorDia, radiusOfCurvature, centralObstruction, gratingFreq, gratingOffset, userParabolicCorrection, RonchigramSize, invertBands, RonchiGrid, zonalRuler, pastedImageTransparency, pastedImageWidth, pastedImageHeight, pastedImageOffsetX, pastedImageOffsetY, imageSource, distortionResolutionFactor, showBullseyeZones, thinBands
        'thirteen':       [13.1,  80.6, 3.1,  65, -0.104,  1, 400, false, false, false, 0.75, 400, 400,   0,   0, MLB.base64images._13,   3, false, false],
        'thirteen2':      [13.1,  80.6, 3.1,  65, -0.106,  1, 400, true,  false, false, 1.0,  400, 400,  -1,   0, MLB.base64images._13,   3, false, true],
        'thirteenGrid':   [13.1,  80.6, 3.1,  65, -0.454,  1, 400, true,  true,  false,  0.9, 398, 402,   0,  -5, MLB.base64images._grid, 2, false, false],
        'twelveF5Under':  [12,     120,   3, 100, -0.178,  1, 400, false, false, false, 0.85, 400, 400,  -3,   0, MLB.base64images._12F5, 2, false, false],
        'twelveF5Under2': [12,     120,   3, 100, -0.178,  1, 400, false, false, false,    1, 400, 400,  -3,   0, MLB.base64images._12F5, 2, false, true]
    },
    images: {},
    // ...images
...
MLB.ronchi.drawRulers = function (context, RonchiCenter, mirrorRadius, scalingFactor) {
    ...
    if (zonalRuler) {
        for (ix = 1; ix < zoneCount; ix++) {
            drawCircle(context, RonchiCenter, scaledMirrorRadius * ix / zoneCount, constants.rulerThickness, constants.rulerColor);
        }
    }
};
...
MLB.ronchi.nullFirstRonchigram = function () {
    ...
    calcNullRonchiBandRulers(RonchiCenter, RGD, DRF, maxDistortion, scalingFactor);
    calcZonalCircularRulers(RonchiCenter, RGD, DRF, maxDistortion, scalingFactor);
    calcCentralObstructionNull(RonchiCenter, RGD, DRF, maxDistortion, scalingFactor);
...
MLB.ronchi.calcNulledStretchedScaledIx = (v, mirrorDia, RGD, DRF, maxDistortion, scalingFactor) => {
...
MLB.ronchi.calcNullRonchiBandRulers = function (RonchiCenter, RGD, DRF, maxDistortion, scalingFactor) {
    var constants = MLB.ronchi.constants,
    ...
    RonchiBands[0].forEach(t => {
        nullIxStretchScaled = calcNulledStretchedScaledIx (t, mirrorDia, RGD, DRF, maxDistortion, scalingFactor);
...
MLB.ronchi.drawNullRonchiBandRulers = (context, canvasSize) => {
    var constants = MLB.ronchi.constants,
    ...
    nullRonchiBands.forEach(ix => drawLine(context, constants.rulerColor, rulerThickness, point(ix, 0), point(ix, canvasSize)));
    if (RonchiGrid) {
        nullRonchiBands.forEach(ix => drawLine(context, constants.rulerColor, rulerThickness, point(0, ix), point(canvasSize, ix)));
    }
};

MLB.ronchi.calcZonalCircularRulers = function (RonchiCenter, RGD, DRF, maxDistortion, scalingFactor) {
    var constants = MLB.ronchi.constants,
        zoneCount = constants.zoneCount,
        mirrorDia = +constants.mirrorDia().val(),
        calcNulledStretchedScaledIx = MLB.ronchi.calcNulledStretchedScaledIx,
    ...
    for (zIx = 0; zIx < zoneCount; zIx++) {
        z = mirrorDia / 2 * zIx / zoneCount;

        nullIxStretchScaled = calcNulledStretchedScaledIx (z, mirrorDia, RGD, DRF, maxDistortion, scalingFactor);
...
MLB.ronchi.drawZonalCircularRulers = (context, canvasSize) => {
    var constants = MLB.ronchi.constants,
    ...
    zonalCircularRulersNulled.forEach(ix => drawCircle(context, point(canvasSize / 2, canvasSize / 2), ix, rulerThickness, rulerColor));
};

MLB.ronchi.calcCentralObstructionNull = (RonchiCenter, RGD, DRF, maxDistortion, scalingFactor) => {
    var constants = MLB.ronchi.constants,
        calcNulledStretchedScaledIx = MLB.ronchi.calcNulledStretchedScaledIx,
        mirrorDia = +constants.mirrorDia().val(),
        centralObstruction = +constants.centralObstruction().val(),
    ...
    nullIxStretchScaled = calcNulledStretchedScaledIx(centralObstruction / 2, mirrorDia, RGD, DRF, maxDistortion, scalingFactor);
    constants.centralObstructionNullIx = nullIxStretchScaled;
};

MLB.ronchi.drawCentralObstructionNull = (context, canvasSize) => {
    var constants = MLB.ronchi.constants,
        centralObstructionNullIx = constants.centralObstructionNullIx,
        point = MLB.sharedLib.point,
        fillCircle = MLB.sharedLib.fillCircle;

    fillCircle(context, point(canvasSize / 2, canvasSize / 2), centralObstructionNullIx, constants.blackColor);
};
...
MLB.ronchi.nullSetPixel = function (yIx, xIx, yDir, xDir, distortionOffsetY, distortionOffsetX, RonchiCenter, nullImageData, origImg) {
    var setPixel = MLB.sharedLib.setPixel,
        ...
        origCanvasWidth = RonchiCenter.x * 2,
...
MLB.ronchi.processNulledImage = () => {
    ...
    processNulledImageData(imageData, newImageData, canvasSize);
...
MLB.ronchi.drawPostNullRonchigramInformation = () => {
    ...
    drawNullRonchiBandRulers(context, canvasSize);
    if (zonalRuler) {
        drawZonalCircularRulers(context, canvasSize);
    }

    // central obstruction
    drawCentralObstructionNull(context, canvasSize);
    // note if parabolic correction != 1
...
MLB.ronchi.processNulledImageData = (imageData, newImageData, canvasSize) => {
    var constants = MLB.ronchi.constants,
        white = 0,
        black = 1,
        contrast = 1.5,
