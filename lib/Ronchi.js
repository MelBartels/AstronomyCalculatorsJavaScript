...
MLB.ronchi.nullFirstRonchigram = function () {
    ...
    calcNullRuler(nullRonchiCenter, RGD, DRF, maxDistortion); // in case nullRuler needed subsequently
    if (nullRuler) {
        drawNullRuler(context, canvasSize);
    }

    scaleNullRonchigram(distortionLength, maxX, canvasSize, context);
...
MLB.ronchi.scaleNullRonchigram = function (distortionLength, maxX, canvasSize, context) {
...
MLB.ronchi.calcNullRuler = function (nullRonchiCenter, RGD, DRF, maxDistortion) {
    var constants = MLB.ronchi.constants,
        mirrorDia = +constants.mirrorDia().val(),
        int = MLB.sharedLib.int,
        tapeBandRelativeX,
...
MLB.ronchi.drawNullRuler = (context, canvasSize) => {
    var constants = MLB.ronchi.constants,
    ...
    nullTapes.forEach(ix => drawLine(context, constants.rulerTextColor, constants.rulerThickness, point(ix, 0), point(ix, canvasSize)));
    if (RonchiGrid) {
        // horizontal ruler
        nullTapes.forEach(ix => drawLine(context, constants.rulerTextColor, constants.rulerThickness, point(0, ix), point(canvasSize, ix)));
    }
};
...
MLB.ronchi.getAndProcessImage = () => {
    ...
    // for example, draw the nullTapes (relies on update to date null ruler)
    drawNullRuler(context, canvasSize);
};
