...
MLB.ronchi.fillStyleFromRGB = function(RGBval) {
    var opacity = 255;

    return 'rgba(' + RGBval + ', ' + opacity + ')';
};

MLB.ronchi.noteParabolicCorrection = function (context, ronchiCenterX) {
    var constants = MLB.ronchi.constants,
        roundToDecimal = MLB.sharedLib.roundToDecimal,
        fillStyleFromRGB = MLB.ronchi.fillStyleFromRGB,
        parabCorr = +constants.userParabolicCorrection().val(),
        txt;

    context.fillStyle = fillStyleFromRGB(constants.rulerTextRGB().val());
    ...
    context.fillText(txt, ronchiCenterX * 1.5, constants.fontSize);
};
