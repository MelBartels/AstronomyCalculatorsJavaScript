...
MLB.objectLib.findClosestObjects = function (minCount, maxDistanceDeg, minMagnitude, minDecDeg) {
    ...
    // sort by size: small to large for next stage...
    groupsStr3.sort((a, b) => {
        return a.split(',').length > b.split(',').length? 1: -1;
    });
    //console.log('\nafter sort by size, before match remove...');
    ...
    //console.log('removing object cluster where any object found in another cluster...');
    ...
    // resort by largest to smallest
    groupsStr4.sort((a, b) => {
        return a.split(',').length < b.split(',').length? 1: -1;
    });
    //console.log('\nafter removal of any matching object...');
