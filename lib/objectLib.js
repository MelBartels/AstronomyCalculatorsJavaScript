...
MLB.objectLib.buildGroupObjectStr = function (group) {
    ...
    group.forEach((rec) => {
        groupStr += rec.rec.OBJECT + ', ';
    });

    return groupStr;
};

MLB.objectLib.findClosestObjects = function (minCount, maxDistanceDeg, minMagnitude, minDecDeg) {
    var calcAngularSepUsingRaDec = MLB.coordLib.calcAngularSepUsingRaDec,
        ...
        groupsStr,
        uniqueGroupsStr,
        filteredUniqueGroupsStr,
        groupRecCount,
        largestGroupRecCount = 0,
        largestGroupIx;
    ...
    //  console.log('group ' + ix + ': ' + buildGroupObjectStr(group.group));
    //});
    ...
    // remove dups
    uniqueGroupsStr = [...new Set(groupsStr)];
    //uniqueGroupsStr.forEach((groupStr, ix) => {
    //  console.log('unique group ' + ix + ': ' + groupStr);
    //});
    // remove naive subset dups (does NOT catch cases of 'a, b, d, ' included in 'a, b, c, d')
    filteredUniqueGroupsStr = uniqueGroupsStr.filter((groupStr, ix, uniqueGroupsStr) => {
        if (ix < uniqueGroupsStr.length - 1) {
            return !uniqueGroupsStr[ix + 1].includes(groupStr);
        }
        return true;
    });
    filteredUniqueGroupsStr.forEach((groupStr, ix) => {
        console.log('unique group ' + ix + ': ' + groupStr);
    });

    // largest group?
    filteredUniqueGroupsStr.forEach((groupStr, ix) => {
        groupRecCount = groupStr.split(',').length;
        if (groupRecCount > largestGroupRecCount) {
            largestGroupRecCount = groupRecCount;
            largestGroupIx = ix;
        }
    });
    console.log('largest group count of ' + largestGroupRecCount + ': ' + filteredUniqueGroupsStr[largestGroupIx]);

    return filteredUniqueGroupsStr;
};

MLB.objectLib.sortedBySizeFilteredUniqueGroupsStr = function (groupStr) {
	groupStr.sort((a, b) => {
		return a.split(',').length < b.split(',').length? 1: -1;
	});	
	groupStr.forEach((gStr, ix) => {
        console.log('unique group of size ' + gStr.split(',').length + ': ' + gStr);
    });
};
