...
MLB.NewtDesigner.calcVisualDetection = function () {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        ...
        objectMPAM = results.objectSurfaceBrightness - uom.sqrArcminToSqrArcsecCircularMagnitudeChange,
        detectableLit;

    if (results.contrastPercent < 3) {
        detectableLit = config.impossibleToDetectLit;
    } else if (results.contrastPercent < 6) {
    ...
    common.visualDetectionResultsTable().html('<b>Results:</b><br>Calculated object surface brightness = '
            + roundToDecimal(results.objectSurfaceBrightness, config.decimalPointsMagnitude)
            + ' MPAS ('
            + roundToDecimal(objectMPAM, config.decimalPointsMagnitude)
            + ' MPAM)<br>Object + sky background brightness = '
            + roundToDecimal(results.objectPlusSkyBackgroundSurfaceBrightness, config.decimalPointsMagnitude)
            + ' MPAS<br>Contrast of object+sky to sky &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; = '
            + roundToDecimal(results.contrastPercent, config.decimalPointsMagnitude)
            + '% ('
            + detectableLit
            +')');
};
