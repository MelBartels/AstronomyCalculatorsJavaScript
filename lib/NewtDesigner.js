...
MLB.NewtDesigner.calcDiag = function () {
	...
	for (diagIx = 0; diagIx < diagonalsLength; diagIx += 1) {
		...
		// ok to show diag sizes that don't meet criteria for edge illumination
...
MLB.NewtDesigner.graphDiagIllum = function () {
	...
	for (ix = 0; ix < suitableDiagsLength; ix += 1) {
		...
		diagIllumArray = getDiagIllumArray(aperture, focalLength, diag.suitableDiags[ix], focalPlaneToDiagDistance, offAxisIncrement, maxFieldDia);
		...
		focuserBarrelIllumArray = getDiagIllumArray(diag.suitableDiags[ix], focalPlaneToDiagDistance, focuserBarrelTubeID, focuserBarrelTubeLength, offAxisIncrement, maxFieldDia);

		illumArray = [...diagIllumArray];

		if (includeFocuserBarrelVignetting) {
			illumArrayLength = illumArray.length;
			// not ix: already in use in the outer loop
			for (ixI = 0; ixI < illumArrayLength; ixI += 1) {
				illumArray[ixI][1] = diagIllumArray[ixI][1] - (1 - focuserBarrelIllumArray[ixI][1]);
			}
		}
		calcs.push(illumArray);
...
MLB.NewtDesigner.writeDiagResults = function () {
	...
	focuserBarrelVignettesStr = '<br><br>Focuser barrel (see the Focuser Designer) vignettes? &emsp;';
	...
	common.diagResults().html('Smallest possible diagonal  = '
			...
			+ integratedVignettedIllumsStr
			+ '<br>Equivalent aperture ranges from edge of field to center of field for each diagonal size:<br>&emsp;'
			+ equivalentApertureStr
...
MLB.NewtDesigner.updateTelescopeResults = function () {
	...
	common.telescopeDesignerResultsLabel().html('brightness gain = '
			...
			+ uomWeightLit
			+ ", mirror CG from front edge = <label id='mirrorCGLabel'> </label>"
			+ uomLengthLit
