...
MLB.NewtDesigner.scrollToTop = function (btnId, designId, imageId, div) {
...
MLB.NewtDesigner.processURLhash = function () {
    ...
    if (window.location.hash) {
        ...
        if (hash === config.jumpTo.diagonal) {
            scrollToTop(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit, config.uomFocalRatioApertureSharedParmsLit);

        } else if (hash === config.jumpTo.telescopeValue) {
            scrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.telescopeValue);

        } else if (hash === config.jumpTo.telescopePerformance) {
            scrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.telescopePerformance);

        } else if (hash === config.jumpTo.apertureManagement) {
            scrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.apertureManagement);

        } else if (hash === config.jumpTo.turbulence) {
            processBtnAndPanel(common.btnTurbulence());
            scrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.turbulence);

        } else if (hash === config.jumpTo.airy) {
            processBtnAndPanel(common.btnAiry());
            scrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.airy);

        } else if (hash === config.jumpTo.telescopeRequirements) {
            processBtnAndPanel(common.btnTelescopeRequirements());
            scrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.telescopeRequirements);

        } else if (hash === config.jumpTo.formulae) {
            scrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.formulae);

        } else if (hash === config.jumpTo.magnification) {
            scrollToTop(config.btnSelectEyepiecesLit, config.designEyepiecesLit, config.designEyepieceImageLit, config.jumpTo.magnification);

        } else if (hash === config.jumpTo.twoUnknownOptimizations) {
            processBtnAndPanel(common.btnTwoUnknownOptimizations());
            scrollToTop(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit, config.jumpTo.twoUnknownOptimizations);

        } else if (hash === config.jumpTo.secondarySizeExperiment) {
            processBtnAndPanel(common.btnSecondarySizeExperiment());
            scrollToTop(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit, config.jumpTo.secondarySizeExperiment);

        } else if (hash === config.jumpTo.diagonalOffsetStudy) {
            processBtnAndPanel(common.btnDiagonalOffsetStudy());
            scrollToTop(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit, config.jumpTo.diagonalOffsetStudy);

        } else if (hash === config.jumpTo.offAxisMask) {
            scrollToTop(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit, config.jumpTo.offAxisMask);

        } else if (hash === config.jumpTo.movementFriction) {
            scrollToTop(config.btnSelectRockerLit, config.designRockertLit, config.designRockerImageLit, config.jumpTo.movementFriction);
        }
    }
};
...
$(window).ready(function () {
    ...
    // write out title, version, overwriting the 'loading...' msg
