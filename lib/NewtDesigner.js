...
MLB.NewtDesigner.config = {
    ...
    */
    ...
    diagOffsetLabel2Lit: 'Diagonal offset (along diagonal face)',
...
MLB.NewtDesigner.common = {
    ...
    btnCalcAllDiagonalOffsets: function () {
        return $('[id=btnCalcAllDiagonalOffsets]');
    },
    diagOffsetsComparedLabel: function () {
        return $('[id=diagOffsetsComparedLabel]');
    },
    ...
    diagOffset2: function () {
        return $('[id=diagOffset2]');
    },
    diagOffset2Val: function () {
        return +this.diagOffset2().val();
    },
    diagOffset2Label: function () {
        return $('[id=diagOffset2Label]');
    },
...
MLB.NewtDesigner.calcAllDiagonalOffsets = function () {
    ...
    common.diagOffsetsComparedLabel().html('offset from focal point = '
        + roundToDecimal(diagOffsetFocalPoint, config.decimalPointsDimension)
        + uomLengthLit
        + '<br>offset from circle of full illumination = '
        + roundToDecimal(diagOffsetFullIllum, config.decimalPointsDimension)
        + uomLengthLit
        + '<br>offset from edge of field = '
        + roundToDecimal(diagOffsetFieldEdge, config.decimalPointsDimension)
        + uomLengthLit
        + '<br>offset from eye '
        + roundToDecimal(eyeAboveFocalPlane, config.decimalPointsDimension)
        + uomLengthLit
        + ' above focal plane = '
        + roundToDecimal(diagOffsetEyeToDiag, config.decimalPointsDimension)
        + uomLengthLit);        
};
...
MLB.NewtDesigner.graphDiagIllum = function () {
    ...
    uomLengthLit = common.getUomLengthLit();
    offset = -calcDiagOffsetUsingFullIllumField(aperture, common.telescopeFocalLengthVal(), state.diagSize, common.focalPlaneToDiagDistanceVal());
    ...
    common.diagResults().html('diagonal size for least obstruction = '
            ...
            + RMSCausedByCentralObstructionStr
            + '<br><br>'
            + config.diagOffsetLabelLit
            + ' = '
            + roundToDecimal(offset, config.decimalPointsDiag)
            + uomLengthLit
            + '<br>'
            + config.diagOffsetLabel2Lit
            + ' = '
            + roundToDecimal(offset * 1.414, config.decimalPointsDiag)
            + uomLengthLit);
...
MLB.NewtDesigner.changeUom = function (ignoreLengthConversion) {
    ...
    common.diagOffset2Label().html(config.diagOffsetLabel2Lit + uomDistanceLit + ' = ');
    ...
    common.diagOffset2().val(roundToDecimal(common.diagOffset2Val() * lengthConversionFactor, config.decimalPointsDiag));
...
MLB.NewtDesigner.updateSelectedThenScrollToTop = function (accordianBtn, btnId, designId, imageId, div) {
    ...
    if (accordianBtn) {
        processAcordianBtnAndPanel(accordianBtn);
    }
...
$(window).ready(function () {
    var state = MLB.NewtDesigner.state,
        ...
        changeUom = MLB.NewtDesigner.changeUom,
        calcAllDiagonalOffsets = MLB.NewtDesigner.calcAllDiagonalOffsets,
    ...
    common.btnCalcAllDiagonalOffsets().click(calcAllDiagonalOffsets);
