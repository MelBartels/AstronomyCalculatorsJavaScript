...
MLB.NewtDesigner.setEyeOptSelectedEyepiece = function (idIx, selectedIndex) {
	...
	if (selectedIndex === 0) {    // that is, if 'select an eyepiece' first row is picked
		common.tableElement(config.EyeOptManufacturerID, idIx).html('');
		common.tableElement(config.EyeOptTypeID, idIx).html('');
		common.tableElement(config.EyeOptFocalLengthID, idIx).html('');
		common.tableElement(config.EyeOptFieldStopID, idIx).html('');
		common.tableElement(config.EyeOptExitPupilID, idIx).html('');
		common.tableElement(config.EyeOptApparentFieldID, idIx).html('');
		common.tableElement(config.EyeOptFOVID, idIx).html('');
		common.tableElement(config.EyeOptMagnificationID, idIx).html('');
		common.tableElement(config.EyeOptMagnificationPerInchID, idIx).html('');
		common.tableElement(config.EyeOptResolutionID, idIx).html('');
		common.tableElement(config.EyeOptLimitingMagnitudeID, idIx).html('');
		common.tableElement(config.EyeOpEtendueID, idIx).html('');
		common.tableElement(config.EyeOptComaID, idIx).html('');
		return;
	}
...
$(window).ready(function () {
	...
	common.AEFEyepieceSelect().val('Tele Vue: TVO Ethos 21.0mm 99.2deg');
	updateAEF();

	seedComaCorrector('TeleVue', 'Paracorr II', common.comaCorrectorSelect, common.comaCorrectorMag);
	setEyeOptSelectedEyepieceUsingSelectStr(0, 'Tele Vue: TVO Nagler5 31.0mm 81.0deg');
	setEyeOptSelectedEyepieceUsingSelectStr(1, 'Nikon: NIK NAV-HW 17.0mm 102.7deg');
