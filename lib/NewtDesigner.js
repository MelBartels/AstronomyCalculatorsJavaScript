...
MLB.NewtDesigner.setEyeOptSelectedEyepiece = function (idIx, selectedIndex) {
	var roundToDecimal = MLB.sharedLib.roundToDecimal,
		...
		limitingMagnitude = MLB.calcLib.limitingMagnitude,
		getComaCorrectorMagnificationFactor = MLB.NewtDesigner.getComaCorrectorMagnificationFactor,
		updateEyepieceState = MLB.NewtDesigner.updateEyepieceState,
		comaCorrectorMag = getComaCorrectorMagnificationFactor(),
		focalRatio = common.focalRatioVal(),
		aperture = common.apertureVal(),
		apertureInches = common.apertureInchesVal(),
	...
	if (selectedIndex === 0) {    // ie, '--select an eyepiece--' is selected
...
MLB.NewtDesigner.copySelectedEyepieceToEyepieceTable = () => {
	...
	while (nextAvailRow < config.maxEyepieceRows) {
		if (!state.eyepieceRowsSet[nextAvailRow]) {
			break;
		}
		nextAvailRow += 1;
	}
...
MLB.NewtDesigner.buildData = () => {
	...
	return {
		...
		eyepieceRowSet: state.eyepieceRowsSet,
		eyepieces: state.selectedEyepieces,
...
MLB.NewtDesigner.getDataUpdateUI = function (parsedData) {
	...
	state.eyepieceRowsSet = parsedData.eyepieceRowSet;
	state.selectedEyepieces = parsedData.eyepieces;
	// old eyepiece list (before March 2026)
	if (parsedData.eyepieceList === '0') {
		common.btnEyepieceList()[0].checked = true;
	// new eyepiece list
	} else if (parsedData.eyepieceList === '1') {
		common.btnEyepieceList()[1].checked = true;
	// if no eyepiece list selection and if eyepiece definitions exist, then likely they came from the old eyepiece list
	} else if (parsedData.eyepieceList === undefined && state.eyepieceRowsSet.length > 0) {
		common.btnEyepieceList()[0].checked = true;
	}
	// after setting btnEyepieceList() now fill eyepiece row selection column with correct eyepiece data: needed when switching eyepiece json during design import
	insertEyepiecesSelectLit();
	clearEyepieceSelections();
	fillEyepieceSelections();
	// fill eyepiece table using local vars
	eyepieceRowsSet = state.eyepieceRowsSet;
	selectedEyepieces = state.selectedEyepieces;
	for (eyepieceIx = 0; eyepieceIx < config.maxEyepieceRows; eyepieceIx += 1) {
		if (eyepieceRowsSet[eyepieceIx]) {
			eyepieceSelect = selectedEyepieces[eyepieceIx];
			setEyeOptSelectedEyepieceUsingSelectStr(eyepieceIx, eyepieceSelect);
		} else {
			removeEyeOptSelectedEyepiece(eyepieceIx);
		}
	}
	// sorting the eyepieces table causes the row order to become disordered, affecting the import (eg, row #2 might be in the 8th position)
	sortEyepieceTable(config.focalLengthIx);
...
// note: rows will become disordered but each row will continue to host its original remove button and selected eyepiece
...
MLB.NewtDesigner.updateEyepieceState = () => {
	var state = MLB.NewtDesigner.state,
		listSelectedEyepieces = MLB.NewtDesigner.listSelectedEyepieces,
		result;

	result = listSelectedEyepieces();
	state.eyepieceRowsSet = result.eyepieceRowsSet;
	state.selectedEyepieces = result.selectedEyepieces;
};
...
MLB.NewtDesigner.listSelectedEyepieces = () => {
	...
	for (eyepieceRowIx = 1; eyepieceRowIx <= config.maxEyepieceRows; eyepieceRowIx += 1) {
		selection = eyepieceTable.rows[eyepieceRowIx].cells[0].children[0];
		selectedEyepieceStr = selection.options[selection.selectedIndex].text;
		selectedEyepieces.push(selectedEyepieceStr);
		eyepieceRowsSet.push(selectedEyepieceStr !== config.selectEyepieceLit);
	}
	return {selectedEyepieces: selectedEyepieces,
			eyepieceRowsSet: eyepieceRowsSet
	};
};
...
$(window).ready(function () {
	...
	common.AEFEyepieceSelect().val('Tele Vue: TVO Ethos 21mm 99.2deg');
	updateAEF();

	seedComaCorrector('TeleVue', 'Paracorr II', common.comaCorrectorSelect, common.comaCorrectorMag);
	setEyeOptSelectedEyepieceUsingSelectStr(0, 'Tele Vue: TVO Nagler5 31mm 81deg');
	setEyeOptSelectedEyepieceUsingSelectStr(1, 'Nikon: NIK NAV-HW 17mm 102.7deg');
