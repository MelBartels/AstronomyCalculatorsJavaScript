// copyright Mel Bartels, 2016 - 2024
...
MLB.NewtDesigner.writeDiagResults = function () {
	...
	diag.suitableDiags.map(function (diag) {
		centralObstructionsStr += diag
				+ ': ' + roundToDecimal(100 * diag / aperture, config.decimalPointsPercent)
				+ '% linear, '
				+ roundToDecimal(100 * diag * diag / (aperture * aperture), config.decimalPointsPercent)
				+ '% area; ';
...
MLB.NewtDesigner.updateMirrorCellFromMirrorSupportPoints = function (mirrorSupportPoints) {
	...
	explanatoryText += '<br>Mirror surface area per support point: '
