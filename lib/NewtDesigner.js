...
// copyright Mel Bartels, 2016 - 2019
...
MLB.NewtDesigner.config = {
    version: '16-Oct-2019.b',
    titleText: 'Newtonian Telescope Designer by Mel Bartels',
    ...
    /* designers: each need three literals: button, designer div and image div
                  also add a common.btn...() function,
                       ...
                       and add a strategy to updateDesigners()
    */
...
MLB.NewtDesigner.common = {
    ...
    mirrorCellDesignerResultsLabel: function () {
        return $('[name=mirrorCellDesignerResultsLabel]');
    },
    ...
    mirrorCellDetailsLabel: function () {
        return $('[name=mirrorCellDetailsLabel]');
    },
    ...
    btnUpdateBaffleDiagonalFromDiagonalDesigner: function () {
        return $('[id=btnUpdateBaffleDiagonalFromDiagonalDesigner]');
    },
    diagOffset: function () {
    ...
    focuserBaffleResults: function () {
        return $('[id=focuserBaffleResults]');
    },
    focuserBaffleDesignerResultsLabel: function () {
        return $('[name=focuserBaffleDesignerResultsLabel]');
    },
    ...
    focalPointPerpendicularOffsetFromEdgeOfPrimary: function () {
        return $('[name=focalPointPerpendicularOffsetFromEdgeOfPrimary]');
    },
    ...
    focalPointPerpendicularOffsetFromEdgeOfPrimaryLabel: function () {
        return $('[name=focalPointPerpendicularOffsetFromEdgeOfPrimaryLabel]');
    },
    ...
    diagToFocuserBaffleDistance: function () {
        return $('[name=diagToFocuserBaffleDistance]');
    },
    ...
    partElements: function () {
        return $('[name^=' + this.config.CGIDs.part + ']');
    },
    weightElements: function () {
        return $('[name^=' + this.config.CGIDs.weight + ']');
    },
    distanceElements: function () {
        return $('[name^=' + this.config.CGIDs.distance + ']');
    },
    verticalOffsetElements: function () {
        return $('[name^=' + this.config.CGIDs.verticalOffset + ']');
    },
...
MLB.NewtDesigner.writeOffaxisMaskResults = function (diagOffset) {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        config = MLB.NewtDesigner.config,
        common = MLB.NewtDesigner.common,
        ...
        offaxisMaskDia = (common.apertureVal() - common.optimizedDiagSizeVal()) / 2 + diagOffset,
...
MLB.NewtDesigner.graphDiagIllumSubr = function (chart, diagonals) {
    ...
    state.suitableDiags = suitableDiags;
...
MLB.NewtDesigner.graphIntegratedIllum = function () {
    ...
    // state.diagIntegratedVignettedIllum is array of arrays: [diagonal m.a., integrated illum percent], ...
    state.diagIntegratedVignettedIllum.map(function (item) {
        series.push(roundToDecimal(1 - magnitudeDrop(item[1]), config.decimalPointsDiag));
    });
    $.jqplot(common.diagIntegratedIllumChartID(), [series], {
        title: 'Field integrated illumination',
        axes: {
            xaxis: {
...
MLB.NewtDesigner.graphDiagIllum = function () {
    ...
    state.suitableDiags.map(function (diag) {
    ...
    writeOffaxisMaskResults(offset);
};

// state.diagSize = suitableDiags[0] and state.diagOffset = offset assigned in graphDiagIllumSubr()
MLB.NewtDesigner.updateBaffleDesignerFromDiagonalDesigner = function () {
    ...
    common.diagOffset().val(roundToDecimal(state.diagOffset, config.decimalPointsDiag));
};

MLB.NewtDesigner.updateLowriderBaffleDesignerFromBaffleDesigner = function () {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        config = MLB.NewtDesigner.config,
        common = MLB.NewtDesigner.common,
...
MLB.NewtDesigner.graphBaffles = function () {
    ...
    scaledFocalPlaneToDiagDistance = state.scaling.scalingFactor * focalPlaneToDiagDistance;
    scaledDiagSize = state.scaling.scalingFactor * diagSize;
    ...
    scaledProjectedFocuserBaffleRadius = state.scaling.scalingFactor * projectedFocuserBaffleRadius;
    ...
    scaledDiagOffset = state.scaling.scalingFactor * diagOffset;
...
MLB.NewtDesigner.graphFocuserBaffles = function () {
    ...
    diagSizeMinorAxis = common.optimizedDiagSizeVal();
...
MLB.NewtDesigner.graphLowrider = function () {
    ...
    state.lowriderModel = calcFoldedNewtonian(aperture, focalRatio, lowriderSecondaryMirrorSize, 0, focalPointPerpendicularOffsetFromEdgeOfPrimary, focalPlaneToSecondaryDistance);
...
MLB.NewtDesigner.graphRocker = function () {
    ...
    scaledTelescopeTubeOD = state.scaling.scalingFactor * telescopeTubeOD;
    scaledWoodThickness = state.scaling.scalingFactor * woodThickness;
    scaledCGHeightToClearRockerBottomDistance = state.scaling.scalingFactor * CG.heightToClearRockerBottomDistance;
    scaledRockerSideLength = state.scaling.scalingFactor * rockerSideLength;
    scaledRockerSideHeight = state.scaling.scalingFactor * rockerSideHeight;
    scaledCGToBearingPointY = state.scaling.scalingFactor * altBearingRadius * Math.cos(altBearingSeparationHalfAngleRad);
    scaledFrontRockerBoardOutsideWidth = state.scaling.scalingFactor * frontRockerBoardOutsideWidth;
    ...
    // bearing arc
    bearingArcLeftPt = point(CGPt.x - state.scaling.scalingFactor * altBearingRadius * Math.sin(altBearingSeparationHalfAngleRad), CGPt.y + scaledCGToBearingPointY);
    bearingArcRightPt = point(CGPt.x + state.scaling.scalingFactor * altBearingRadius * Math.sin(altBearingSeparationHalfAngleRad), bearingArcLeftPt.y);
    ...
    padBoardHeight = (rockerSideBoardRect.y - bearingArcRightPt.y) / state.scaling.scalingFactor;
    ...
    // draw bearing arc
    context.beginPath();
    context.arc(CGPt.x, CGPt.y, state.scaling.scalingFactor * altBearingRadius, uom.qtrRev - altBearingSeparationHalfAngleRad, uom.qtrRev + altBearingSeparationHalfAngleRad);
    context.lineWidth = config.canvasLineWidth;
    ...
    text = config.rockerFrontBoardHeightText + roundToDecimal(frontRockerFrontBoardRect.height / state.scaling.scalingFactor, config.decimalPointsDimension) + uomLengthLit;
...
MLB.NewtDesigner.graphFlexRocker = function () {
    ...
    // scaled dimensions
    scaledAzInnerRadius = state.scaling.scalingFactor * azInnerRadius;
    scaledAzOuterRadius = state.scaling.scalingFactor * azOuterRadius;
    scaledAltBearingRadius = state.scaling.scalingFactor * altBearingRadius;
    scaledAltBearingSeparation = state.scaling.scalingFactor * altBearingSeparation;
    scaledAltBearingSideToSideSeparation = state.scaling.scalingFactor * altBearingSideToSideSeparation;
    scaledAltBearingHeight = state.scaling.scalingFactor * altBearingHeight;
    scaledBaseRingHeight = state.scaling.scalingFactor * baseRingHeight;
    scaledFlexRockerThickness = state.scaling.scalingFactor * flexRockerThickness;
    scaledPadSizeSideUom = state.scaling.scalingFactor * flexRocker.friction.padSizeSideUom;
    scaledPadThicknessUom = state.scaling.scalingFactor * padThicknessUom;
    scaledCGHeight = state.scaling.scalingFactor * flexRocker.CGHeight;
...
MLB.NewtDesigner.graphEquatorialTable = function () {
    ...
    */
    ...
    new ETSideViewDrawArcExtension(context, config.canvasFrontBearingStyle, platformSideRect.endX, platformSideRect.endY, frontViewSouthDrawnArc.leftLowBearingPt.y, compLat);
    new ETSideViewDrawArcExtension(context, config.canvasBackBearingStyle, platformSideRect.x, platformSideRect.endY, frontViewNorthDrawnArc.leftLowBearingPt.y, compLat);
...
/*  <tr>
        ...
        <td>
            <input class="inputText" name="CGVerticalOffset0" value="" onfocus="select();" type="text">
        </td>
    </tr>
    <tr>
        <td>
        ...
        <td>
            <input class="inputText" name="CGVerticalOffset1" value="" onfocus="select();" type="text">
        </td>
    </tr>
    ...
*/
...
MLB.NewtDesigner.seedCGTable = function () {
    ...
    $('[name=' + CGIDs.weight + CGIxs.mirrorMount + ']').val(weights.mirrorMount);
    $('[name=' + CGIDs.weight + CGIxs.tube + ']').val(weights.tube);
    $('[name=' + CGIDs.weight + CGIxs.altitudeBearings + ']').val(weights.altitudeBearings);
    $('[name=' + CGIDs.weight + CGIxs.focuser + ']').val(weights.focuser);
    $('[name=' + CGIDs.weight + CGIxs.diagonal + ']').val(weights.diagonal);
    $('[name=' + CGIDs.weight + CGIxs.spider + ']').val(weights.spider);
    $('[name=' + CGIDs.weight + CGIxs.eyepiece + ']').val(weights.eyepiece);
    ...
    $('[name=' + CGIDs.distance + CGIxs.primaryMirror + ']').val(roundToDecimal(-common.convertInchesToUom(config.primaryMirrorThicknessInches) / 2, config.decimalPointsCG));
    $('[name=' + CGIDs.distance + CGIxs.mirrorMount + ']').val(roundToDecimal(-common.convertInchesToUom(config.primaryMirrorThicknessInches + config.primaryMirrorCellThicknessInches / 2), config.decimalPointsCG));

    // set vertical offsets
    $('[name=' + CGIDs.verticalOffset + CGIxs.altitudeBearings + ']').val(roundToDecimal(common.convertInchesToUom(verticalOffsetsInches.altitudeBearings), config.decimalPointsCG));
    $('[name=' + CGIDs.verticalOffset + CGIxs.focuser + ']').val(roundToDecimal(common.convertInchesToUom(verticalOffsetsInches.focuser), config.decimalPointsCG));
    $('[name=' + CGIDs.verticalOffset + CGIxs.eyepiece + ']').val(roundToDecimal(common.convertInchesToUom(verticalOffsetsInches.eyepiece), config.decimalPointsCG));
};
...
// factors determined empirically by inspecting 4" F2, F12 and 40" F2, F12
MLB.NewtDesigner.setScalingFactor = function () {
    var state = MLB.NewtDesigner.state,
        config = MLB.NewtDesigner.config,
        common = MLB.NewtDesigner.common,
        aperture = common.apertureVal(),
    ...
    state.scaling = scalingFactor(config.canvasWidth, config.canvasHeight, aperture * focalRatio * 1.5, aperture * 2.5, config.canvasBorder);
};

MLB.NewtDesigner.updateFieldsDependentOnFocalRatio = function () {
    var state = MLB.NewtDesigner.state,
        common = MLB.NewtDesigner.common,
        setScalingFactor = MLB.NewtDesigner.setScalingFactor,
...
MLB.NewtDesigner.updateFieldsDependentOnAperture = function () {
    ...
    $('[name=' + config.CGIDs.weight + config.CGIxs.primaryMirror + ']').val(roundToDecimal(weight, config.decimalPointsCG));
...
MLB.NewtDesigner.getDataUpdateUI = function (parsedData) {
    ...
    for (CGRow = 0; CGRow < CGLength; CGRow += 1) {
        $('[name=' + config.CGIDs.part + CGRow + ']').val(parsedData.CGs[CGRow].part);
        $('[name=' + config.CGIDs.weight + CGRow + ']').val(parsedData.CGs[CGRow].weight);
        $('[name=' + config.CGIDs.distance + CGRow + ']').val(parsedData.CGs[CGRow].distance);
        $('[name=' + config.CGIDs.verticalOffset + CGRow + ']').val(parsedData.CGs[CGRow].verticalOffset);
    }
...
MLB.NewtDesigner.updateTelescopeResults = function () {
    ...
    common.telescopeDesignerResultsLabel().html('telescope focal length = '
            ...
            + roundToDecimal(theoreticalResolutionArcsec, config.decimalPointsResolution)
            + ' arc seconds<br>min magnification = '
            ...
            + ' waves tangent to edge; minimum difference = '
...
MLB.NewtDesigner.checkMirrorSupportPointsRadioButtons = function (supportPoints) {
    ...
    for (ix = 0; ix < btnMirrorSupportPointsLength; ix += 1) {
        btnMirrorSupportPoints[ix].checked = (btnMirrorSupportPoints[ix].value == supportPoints);
    }
};
...
MLB.NewtDesigner.updateMirrorCellFromMirrorSupportPoints = function (mirrorSupportPoints) {
    ...
    // draw mirror center
    drawCircle(context, centerPt, 3, config.canvasLineWidth, config.canvasStructureStyle);
    ...
    switch (mirrorSupportPoints) {
        ...
        case 27:
            ...
            for (ix = 0; ix < 3; ix += 1) {
                ...
                drawCircle(context, innerPoint, 3, config.canvasLineWidth, config.canvasStructureStyle);
                drawCircle(context, outerPoint1, 3, config.canvasLineWidth, config.canvasStructureStyle);
                drawCircle(context, outerPoint2, 3, config.canvasLineWidth, config.canvasStructureStyle);
                // line between outer points
                ...
                drawCircle(context, CGpoint, 3, config.canvasLineWidth, config.canvasStructureStyle);               
            }
...
/* called from:
    focalRatioSliderChanged() with updateDiags;
    apertureSliderChange() with updateDiags;
    focalRatio().change() with updateDiags;
    aperture().change() with updateDiags;
    focalPlaneToDiagDistance().change();
    focuserSelect().change();
    updateSelected() which is called whenever a new designer is selected;
*/
MLB.NewtDesigner.updateDesigners = function (updateDiags) {
    var state = MLB.NewtDesigner.state,
        config = MLB.NewtDesigner.config,
        common = MLB.NewtDesigner.common,
        ...
        updateLowriderBaffleDesignerFromBaffleDesigner = MLB.NewtDesigner.updateLowriderBaffleDesignerFromBaffleDesigner;
    ...
    case config.designLowriderBafflingLit:
        ...
        if (updateDiags) {
            updateLowriderBaffleDesignerFromBaffleDesigner();
        }
        graphLowrider();
        updateCGDistances();
        break;
...
/* called from:
    $(window).ready() with updateDiagsCGDistances;
    btnUom().click() with updateDiagsCGDistances
    getDataUpdateUI();
*/
MLB.NewtDesigner.runAllDesigners = function (updateDiagsCGDistances) {
...
$(window).ready(function () {
    ...
    // write out title
    common.titleText().html('<h2>' + config.titleText + ', version ' + config.version + '</h2>');
    ...
    common.btnUom().click(function () {
        changeUom();
        runAllDesigners('updateDiagsCGDistances');
    });
    ...
    common.btnUpdateBaffleDiagonalFromDiagonalDesigner().click(function () {
        updateBaffleDesignerFromDiagonalDesigner();
        graphBaffles();
    });
    ...
    // .../NewtDesigner.html#diagonal
    if (window.location.hash) {
        if (window.location.hash.substring(1) === 'diagonal') {
            updateSelected(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit);
        }
    }
