...
MLB.NewtDesigner.removeEyeOptSelectedEyepiece = function (eyepieceRow) {
	...
	// set the selected row to row 0 which ia selectObjectLit  ('---select an object---')
	$('[id^=' + MLB.NewtDesigner.config.EyeOptSelectID + ']')[eyepieceRow][0].selected = 'selected';
...
MLB.NewtDesigner.getDataUpdateUI = function (parsedData) {
	...
	// does an eyepiece definition exist?
	eyepieceRowSet = parsedData.eyepieceRowSet;
	eyepieceRowSetLength = eyepieceRowSet.length;
	for (eyepieceRow = 0; eyepieceRow < eyepieceRowSetLength; eyepieceRow += 1) {
		if (eyepieceRowSet[eyepieceRow]) {
			eyepieceDefined = true;
			break;
		}
	}
