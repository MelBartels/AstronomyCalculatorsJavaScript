...
MLB.NewtDesigner.updateSelectedThenScrollToTop = function (btnId, designId, imageId, div) {
...
MLB.NewtDesigner.processBtnAndPanel = function (btn) {
    var panel;

    // add/remove active class to change button stying
    btn.classList.toggle('active');
    // hide/show panel that immediately follows the button
...
MLB.NewtDesigner.processURLhash = function () {
    var config = MLB.NewtDesigner.config,
        common = MLB.NewtDesigner.common,
        processBtnAndPanel = MLB.NewtDesigner.processBtnAndPanel,
    ...
    if (window.location.hash) {
        ...
        if (hash === config.jumpTo.diagonal) {
            updateSelectedThenScrollToTop(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit, config.uomFocalRatioApertureSharedParmsLit);

        } else if (hash === config.jumpTo.innovation) {
            processBtnAndPanel(common.btnInnovation());
            updateSelectedThenScrollToTop(config.btnSelectIntroLit, config.designIntroLit, config.designIntroImageLit, config.jumpTo.innovation);

        } else if (hash === config.jumpTo.natureOfTelescopeDesign) {
            processBtnAndPanel(common.btnNatureOfTelescopeDesign());
            updateSelectedThenScrollToTop(config.btnSelectIntroLit, config.designIntroLit, config.designIntroImageLit, config.jumpTo.natureOfTelescopeDesign);

        } else if (hash === config.jumpTo.telescopeValue) {
            updateSelectedThenScrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.telescopeValue);

        } else if (hash === config.jumpTo.telescopePerformance) {
            updateSelectedThenScrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.telescopePerformance);

        } else if (hash === config.jumpTo.apertureManagement) {
            processBtnAndPanel(common.btnApertureManagement());
            updateSelectedThenScrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.apertureManagement);

        } else if (hash === config.jumpTo.turbulence) {
            processBtnAndPanel(common.btnTurbulence());
            updateSelectedThenScrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.turbulence);

        } else if (hash === config.jumpTo.airy) {
            processBtnAndPanel(common.btnAiry());
            updateSelectedThenScrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.airy);

        } else if (hash === config.jumpTo.telescopeRequirements) {
            processBtnAndPanel(common.btnTelescopeRequirements());
            updateSelectedThenScrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.telescopeRequirements);

        } else if (hash === config.jumpTo.formulae) {
            updateSelectedThenScrollToTop(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit, config.jumpTo.formulae);

        } else if (hash === config.jumpTo.magnification) {
            updateSelectedThenScrollToTop(config.btnSelectEyepiecesLit, config.designEyepiecesLit, config.designEyepieceImageLit, config.jumpTo.magnification);

        } else if (hash === config.jumpTo.twoUnknownOptimizations) {
            processBtnAndPanel(common.btnTwoUnknownOptimizations());
            updateSelectedThenScrollToTop(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit, config.jumpTo.twoUnknownOptimizations);

        } else if (hash === config.jumpTo.secondarySizeExperiment) {
            processBtnAndPanel(common.btnSecondarySizeExperiment());
            updateSelectedThenScrollToTop(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit, config.jumpTo.secondarySizeExperiment);

        } else if (hash === config.jumpTo.diagonalOffsetStudy) {
            processBtnAndPanel(common.btnDiagonalOffsetStudy());
            updateSelectedThenScrollToTop(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit, config.jumpTo.diagonalOffsetStudy);

        } else if (hash === config.jumpTo.offAxisMask) {
            updateSelectedThenScrollToTop(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit, config.jumpTo.offAxisMask);

        } else if (hash === config.jumpTo.vibration) {
            processBtnAndPanel(common.btnVibration());
            updateSelectedThenScrollToTop(config.btnSelectMountLit, config.designMountLit, config.designMountImageLit, config.jumpTo.vibration);

        } else if (hash === config.jumpTo.movementFriction) {
            updateSelectedThenScrollToTop(config.btnSelectRockerLit, config.designRockertLit, config.designRockerImageLit, config.jumpTo.movementFriction);
...
$(window).ready(function () {
    ...
    // wire up accordian sections (detailed text on a particular subject)
    common.accordians().each(function () {
        $(this).click(function() {
            processBtnAndPanel(this);
        });
    });
    ...
    common.titleText().html('<h2>' + config.titleText + '</h2>');
    common.versionText().html('<br><br><b>Version: ' + config.version + '</b>');
