...
MLB.NewtDesigner.config = {
    ...
    // lits
    CGIDs: ['CGPart', 'CGWeight', 'CGDistance', 'CGVerticalOffset']
};
...
MLB.NewtDesigner.common = {
    ...
    verticalOffsetElements: function () {
        return $('[name^=' + this.config.CGIDs[3] + ']');
    },
...
MLB.NewtDesigner.seedCGTable = function () {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        ...
        CGWeightLit = config.CGIDs[1],
        CGDistanceLit = config.CGIDs[2],
        CGVerticalOffsetLit = config.CGIDs[3];
...
MLB.NewtDesigner.updateFieldsDependentOnAperture = function () {
    ...
    $('[name=' + config.CGIDs[1] + config.CGIxs.primaryMirror + ']').val(roundToDecimal(weight, config.decimalPointsCG));
...
MLB.NewtDesigner.getDataUpdateUI = function () {
    ...
    for (CGRow = 0; CGRow < CGLength; CGRow += 1) {
        $('[name=' + config.CGIDs[1] + CGRow + ']').val(parsedData.CGs[CGRow].weight);
        $('[name=' + config.CGIDs[2] + CGRow + ']').val(parsedData.CGs[CGRow].distance);
        $('[name=' + config.CGIDs[3] + CGRow + ']').val(parsedData.CGs[CGRow].verticalOffset);
    }
