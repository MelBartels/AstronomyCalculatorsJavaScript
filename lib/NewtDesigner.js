...
MLB.NewtDesigner.adjustScaling = (initialScale, adjustment) => {
	return Math.pow(initialScale, adjustment);
};
...
MLB.NewtDesigner.graphBinoscope = () => {
	...
	drawText(context, 'eyepiece length', points.opticalPath.eyepieceLength, config.canvasStructureLightStyle);
...
MLB.NewtDesigner.graphRocker = () => {
	...
	scalingFactor = adjustScaling(scalingFactor, state.sliderRockerScaleValue);
...
MLB.NewtDesigner.graphFlexRocker = () => {
	...
	// readjust per user scaling
	scalingFactor = adjustScaling(scalingFactor, state.sliderFlexRockerScaleValue);
