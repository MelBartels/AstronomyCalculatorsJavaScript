...
MLB.NewtDesigner.config = {
    ...
    uomFRApertureHeaderLit: 'uomFRApertureHeader',
...
MLB.NewtDesigner.common = {
    ...
    uomFRApertureHeader: function () {
        return $('[id=' + this.config.uomFRApertureHeaderLit + ']');
    },
...
MLB.NewtDesigner.seedCGTable = function () {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        ...
        CGWeightLit = config.CGIDs.weight,
        CGDistanceLit = config.CGIDs.distance,
        CGVerticalOffsetLit = config.CGIDs.verticalOffset;

    // set weights, one for each value in the weights array
    $('[name=' + CGWeightLit + CGIxs.mirrorMount + ']').val(weights.mirrorMount);
    $('[name=' + CGWeightLit + CGIxs.tube + ']').val(weights.tube);
    $('[name=' + CGWeightLit + CGIxs.altitudeBearings + ']').val(weights.altitudeBearings);
    $('[name=' + CGWeightLit + CGIxs.focuser + ']').val(weights.focuser);
    $('[name=' + CGWeightLit + CGIxs.diagonal + ']').val(weights.diagonal);
    $('[name=' + CGWeightLit + CGIxs.spider + ']').val(weights.spider);
    $('[name=' + CGWeightLit + CGIxs.eyepiece + ']').val(weights.eyepiece);
    ...
    $('[name=' + CGDistanceLit + CGIxs.primaryMirror + ']').val(roundToDecimal(-common.convertInchesToUom(config.primaryMirrorThicknessInches) / 2, config.decimalPointsCG));
    $('[name=' + CGDistanceLit + CGIxs.mirrorMount + ']').val(roundToDecimal(-common.convertInchesToUom(config.primaryMirrorThicknessInches + config.primaryMirrorCellThicknessInches / 2), config.decimalPointsCG));

    // set vertical offsets
    $('[name=' + CGVerticalOffsetLit + CGIxs.altitudeBearings + ']').val(roundToDecimal(common.convertInchesToUom(verticalOffsetsInches.altitudeBearings), config.decimalPointsCG));
    $('[name=' + CGVerticalOffsetLit + CGIxs.focuser + ']').val(roundToDecimal(common.convertInchesToUom(verticalOffsetsInches.focuser), config.decimalPointsCG));
    $('[name=' + CGVerticalOffsetLit + CGIxs.eyepiece + ']').val(roundToDecimal(common.convertInchesToUom(verticalOffsetsInches.eyepiece), config.decimalPointsCG));
};
...
MLB.NewtDesigner.buildData = function () {
    ...
    for (CGRow = 0; CGRow < CGTableLength; CGRow += 1) {
        CGs.push({weight: weightElements[CGRow].value, distance: distanceElements[CGRow].value, verticalOffset: verticalOffsetElements[CGRow].value});
    }
...
MLB.NewtDesigner.updateTelescopeResults = function () {
    ...
    mirrorSupportPoints = calcMinimumMirrorSupportPoints(common.convertUomToInches(aperture));
...
MLB.NewtDesigner.updateDesigners = function () {
    ...
    case config.designTelescopeLit:
        common.uomFRApertureHeader().show();
    ...
    case config.designExportImportLit:
        common.uomFRApertureHeader().hide();
        common.diagSharedParms().hide();
        break;
