...
MLB.NewtDesigner.updateTelescopeResults = function () {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        ...
        calcAndDisplayMirrorWeight = MLB.NewtDesigner.calcAndDisplayMirrorWeight,
        focalRatio = common.focalRatioVal(),
        aperture = common.apertureVal(),
        apertureInches = common.apertureInchesVal(),
    ...
    common.telescopeDesignerResultsLabel().html('brightness gain = '
            ...
            + uomLengthLit
            + '^3'
            + '<br>mirror edge thickness = '
            + "<input class='inputText' id='mirrorEdgeThickness' onfocus='select();' type='text'>"
            + uomLengthLit
            + ' meniscus? '
            + "<input id='chBoxIsMeniscus' type='checkbox'>"
            + "&emsp; mirror weight = <label id='mirrorWeightLabel'> </label>"
            + uomWeightLit
            + '<br>effective diameter if mirror is slumped = '
    ...
    common.chBoxIsMeniscus().change(function () {
        ...
        calcAndDisplayMirrorWeight(apertureInches, focalRatio, state.mirrorEdgeThicknessInches, state.isMeniscus);
    });
...
MLB.NewtDesigner.calcAndDisplayMirrorWeight = function (apertureInches, focalRatio, mirrorEdgeThicknessInches, isMeniscus) {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        state = MLB.NewtDesigner.state,
        config = MLB.NewtDesigner.config,
        common = MLB.NewtDesigner.common,
        calcMirrorWeight = MLB.calcLib.calcMirrorWeight,
        mirrorWeight = calcMirrorWeight(apertureInches, focalRatio, mirrorEdgeThicknessInches, isMeniscus);

    common.mirrorWeightLabel().html(roundToDecimal(common.convertLbsToUom(mirrorWeight), config.decimalPointsResolution));
};
