...
/*
Adding features:
    add to NewtDesigner.js:
        label:
            add wrapper function to MLB.NewtDesigner.common() label section
            add label update call to MLB.NewtDesigner.processUomChange();
        inputText:
            add wrapper functions to MLB.NewtDesigner.common() label and field values sections
            add default value to NewtDesigner.html or calculate the value
*/
...
MLB.NewtDesigner.common = {
    ...
    designMount: function () {
        return $('[id=' + this.config.designMountLit + ']');
    },
    designET: function () {
...
MLB.NewtDesigner.calcVignettedIllumPercent = function () {
    var common = MLB.NewtDesigner.common,
...
MLB.NewtDesigner.graphBaffles = function () {
    ...
    */
    ...
    // write out notes
    vignettedIllum = calcVignettedIllumPercent();
...
MLB.NewtDesigner.buildEyepieceHtmlTable = function () {
    ...
    for (ix = 0; ix < config.eyepieceRows; ix += 1) {
        htmlStr = "<tr>\r\n"
                ...
                + "<td id='" + config.EyeOpEtendueID + ix + "'></td>\r\n"
...
MLB.NewtDesigner.seedEyeOptEyepiece = function (idIx, manufacturer, type, focalLengthmm) {
    var config = MLB.NewtDesigner.config,
...
MLB.NewtDesigner.getLocalStorageName = function () {
    var config = MLB.NewtDesigner.config,
        common = MLB.NewtDesigner.common;

    return config.NewtDesignerLit + common.dataName().val();
};

MLB.NewtDesigner.putData = function () {
    var common = MLB.NewtDesigner.common,
        getLocalStorageName = MLB.NewtDesigner.getLocalStorageName,
        dataToSave;

    dataToSave = {
        imperial: common.imperial(),
        apertureVal: common.apertureVal()
    };
...
MLB.NewtDesigner.getDataUpdateUI = function () {
    var common = MLB.NewtDesigner.common,
        ...
        updateFollowOnFieldsStrategy = MLB.NewtDesigner.updateFollowOnFieldsStrategy,
        getLocalStorageName = MLB.NewtDesigner.getLocalStorageName,
        data = localStorage.getItem(getLocalStorageName()),
        parsedData = JSON.parse(data);

    // mlb note to do .imperial be in state too???
    ...
    common.aperture().val(parsedData.apertureVal);
...
// sort descending focal length, manufacturer, type;
MLB.NewtDesigner.sortEyepiecesJsonByFLManufacturerType = function () {
    var eyepiecesJson = MLB.eyepiecesJson;
...
MLB.NewtDesigner.clearEyepieceSelections = function () {
    ...
    for (ix = 0; ix < config.eyepieceRows; ix += 1) {
        $('#' + config.EyeOptSelectID + ix + ' option').remove();
    }
};
...
MLB.NewtDesigner.setSelectedEyepieceFromTable = function () {
    ...
    while (true) {
        manufacturerStr = common.tableElement(config.EyeOptManufacturerID, ix).html();
        if (manufacturerStr) {
            eyepieceStr = manufacturerStr
                    + ' '
                    + common.tableElement(config.EyeOptTypeID, ix).html()
                    + ' '
                    + common.tableElement(config.EyeOptFocalLengthID, ix).html();
            // eg, eyepieceStr = 'TeleVue Ethos 17mm'
            $('#' + config.EyeOptSelectID + ix).val(eyepieceStr);
        }
        ix += 1;
        if (ix === config.eyepieceRows) {
            break;
        }
    }
};
...
MLB.NewtDesigner.sortEyepiecesByFLAddToSelections = function () {
    var sortEyepiecesJsonByFLManufacturerType = MLB.NewtDesigner.sortEyepiecesJsonByFLManufacturerType,
        clearEyepieceSelections = MLB.NewtDesigner.clearEyepieceSelections,
        fillEyepieceSelections = MLB.NewtDesigner.fillEyepieceSelections,
        setSelectedEyepieceFromTable = MLB.NewtDesigner.setSelectedEyepieceFromTable;

    sortEyepiecesJsonByFLManufacturerType();
    clearEyepieceSelections();
    fillEyepieceSelections();
    setSelectedEyepieceFromTable();
};
...
$(window).ready(function () {
    var updateSelected = MLB.NewtDesigner.updateSelected,
        ...
        seedEyeOptEyepiece = MLB.NewtDesigner.seedEyeOptEyepiece,
    ...
    common.btnSelectMount().click(function () {
        updateSelected(config.btnSelectMountLit, config.designMountLit);
    });
    ...
    common.btnGetData().click(getDataUpdateUI);
    ...
    // once through all designers once
    updateTelescopeResults();
    updateEyepieceOptimizerRows();
    graphDiagIllum();
    graphBaffles();
