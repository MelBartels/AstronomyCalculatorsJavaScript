...
MLB.NewtDesigner.common = {
    ...
    telescopeFocalLengthUOMlabel: function () {
        return $('[name=telescopeFocalLengthUOMlabel]');
    },
    chBoxLockFocalLength: function () {
        return $('[name=chBoxLockFocalLength]');
    },
    ...
    chBoxAutoCalcRockerWeight: function () {
        return $('[name=chBoxAutoCalcRockerWeight]');
    },
...
$(window).ready(function () {
    ...
    if (window.location.hash) {
        if (window.location.hash.substring(1) === 'diagonal') {
            updateSelected(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit);
        } else if (window.location.hash.substring(1) === 'telescopeValue') {
            updateSelected(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit);
            $('html,body').animate({
                scrollTop: $('#telescopeValue').offset().top
            });
        } else if (window.location.hash.substring(1) === 'telescopePerformance') {
            updateSelected(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit);
            $('html,body').animate({
                scrollTop: $('#telescopePerformance').offset().top
            });
        } else if (window.location.hash.substring(1) === 'apertureManagement') {
            updateSelected(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit);
            $('html,body').animate({
                scrollTop: $('#apertureManagement').offset().top
            });
        } else if (window.location.hash.substring(1) === 'formulae') {
            updateSelected(config.btnSelectTelescopeLit, config.designTelescopeLit, config.designTelescopeImageLit);
            $('html,body').animate({
                scrollTop: $('#formulae').offset().top
            });
        } else if (window.location.hash.substring(1) === 'magnification') {
            updateSelected(config.btnSelectEyepiecesLit, config.designEyepiecesLit, config.designEyepieceImageLit);
            $('html,body').animate({
                scrollTop: $('#magnification').offset().top
            });
        } else if (window.location.hash.substring(1) === 'offAxisMask') {
            updateSelected(config.btnSelectDiagonalLit, config.designDiagonalLit, config.designDiagonalImageLit);
            $('html,body').animate({
                scrollTop: $('#offAxisMask').offset().top
            });
        } else if (window.location.hash.substring(1) === 'movementFriction') {
            updateSelected(config.btnSelectRockerLit, config.designRockertLit, config.designRockerImageLit);
            $('html,body').animate({
                scrollTop: $('#movementFriction').offset().top
            });
        }
    }
});
