...
MLB.NewtDesigner.writeVisualDetectionResults = function () {
    ...
    if (state.visualDetectionBestPupil === undefined) {
        common.visualDetectionResultsTable().html('');
        return;
    }
    ...
    $.each(json, function (ix, row) {
        htmlStr += '<tr>';
        htmlStr += '<td class="visualResultsWidth">' + row.label + '</td><td>' + row.result + '</tr>';
        htmlStr += '</tr>';
    });
    htmlStr += '</table></tbody>';

    common.visualDetectionResultsTable().html(htmlStr);
};
