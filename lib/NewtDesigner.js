...
MLB.NewtDesigner.graphBaffles = function () {
    ...
    baffleScalingResults = baffleScalingFactor(config.canvasWidth, config.canvasHeight, aperture * focalRatio * 1.5, aperture * 2.5, config.canvasBorder);
...
MLB.NewtDesigner.graphLowrider = function () {
    ...
    focalPt = point(mirrorCenterPt.x + state.scalingFactor * state.lowriderModel.focalPointToPrimaryMirrorDistance, mirrorCenterPt.y - state.scalingFactor * (aperture / 2 + focalPointPerpendicularOffsetFromEdgeOfPrimary));
    ...
    */
    ...
    // calculated: text = config.primaryMirrorToFocalPlaneVerticalHeightText + roundToDecimal((mirrorUpperFacePt.y - focalPt.y) / state.scalingFactor, config.decimalPointsDimension) + uomLengthLit;
    ...
    scaledFocalPlaneToFoldingMirrorSlantedDistance = Math.sqrt(Math.pow(focalPt.y - diagPt.y, 2) + Math.pow(focalPt.x - diagPt.x, 2)) / state.scalingFactor;
