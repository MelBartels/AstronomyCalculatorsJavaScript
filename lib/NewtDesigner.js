...
MLB.NewtDesigner.state = {
    ...
    // state.diagSize assigned in graphDiagIllumSubr()
    diagSize: undefined,
    // state.diagOffset assigned in graphDiagIllum()
...
eg, from 1st entry:
"Catalogs: New General Catalog of Nebulae & Clusters (All types of objects except dark nebulae), Lynds (bright nebula); Type: Bright Nebula; Consellation: Perseus; Classification: elongated/extended; Description: very faint; very large; much elongated/extended north suddenly/south; diffused; Notes: California Nebula;0.6 deg from Xi Per"
*/
...
MLB.NewtDesigner.copySelectedObject = function (object) {
    ...
    common.objectApparentMagnitude().val(parsedObject[1].trim());
    common.objectSizeArcMin1().val(parsedObject[2].replace('m', '').trim());
    common.objectSizeArcMin2().val(parsedObject[3].replace('m', '').trim());
};

MLB.NewtDesigner.createObjectFieldsString = function (astroObj) {
...
MLB.NewtDesigner.createObjectDescriptionString = function (astroObj) {
...
MLB.NewtDesigner.searchObjectCatalog = function () {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        ...
        createObjectFieldsString = MLB.NewtDesigner.createObjectFieldsString,
        createObjectDescriptionString = MLB.NewtDesigner.createObjectDescriptionString,
        Saguaro81Abbr = MLB.Saguaro81AbbrJson.Saguaro81Abbr,
        searchStr = common.searchObjectStringVal(),
        regEx = new RegExp(searchStr, 'gi'),
        searchResults = Saguaro81Abbr.filter(rec => rec.OBJECT.match(regEx) || rec.OTHER.match(regEx) || rec.NOTES.match(regEx)),
        objectFieldsString,
...
MLB.NewtDesigner.writeOffaxisMaskResults = function (diagOffset) {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        ...
        offaxisMaskDia = (common.apertureVal() - state.diagSize) / 2 + diagOffset,
...
MLB.NewtDesigner.graphDiagIllum = function () {
    ...
    state.diagOffset = diagOffsetFullIllum;
    state.diagIntegratedVignettedIllum.map(function (item) {
    ...
    common.diagResults().html('diagonal size for least obstruction = '
            ...
            + ' = '
            + roundToDecimal(diagOffsetFullIllum, config.decimalPointsDiag)
            + uomLengthLit
            + '<br>'
            + config.diagOffseAlongFacetLabelLit
            + ' = '
            + roundToDecimal(diagOffsetFullIllum * 1.414, config.decimalPointsDiag)
            + uomLengthLit
            + '<br>'
            + config.diagOffsetFocalPointLabelLit
            + ' = '
            + roundToDecimal(diagOffsetFocalPoint, config.decimalPointsDiag)
            + uomLengthLit
            + '<br>'
            ...
            + uomLengthLit
            + ' above) = '
            + roundToDecimal(diagOffsetEyeToDiag, config.decimalPointsDiag)
            + uomLengthLit
            + '<br>'
            + config.diagOffsetFieldEdgeLabelLit
            + ' = '
            + roundToDecimal(diagOffsetFieldEdge, config.decimalPointsDiag)
            + uomLengthLit);

    writeOffaxisMaskResults(diagOffsetFullIllum);
};
...
MLB.NewtDesigner.updateBaffleDesignerFromDiagonalDesigner = function () {
    ...
    common.sharedDiagSize().val(roundToDecimal(state.diagSize, config.decimalPointsDiag));
...
MLB.NewtDesigner.updateLowriderBaffleDesignerFromBaffleDesigner = function () {
    ...
    common.lowriderSecondaryMirrorSize().val(roundToDecimal(state.diagSize, config.decimalPointsDiag));
    common.focalPlaneToSecondaryDistance().val(roundToDecimal(state.diagSize * common.focalRatioVal(), config.decimalPointsDimension));
