...
MLB.NewtDesigner.config = {
    ...
    */
    ...
    diagSizesLabelLit: 'Diagonal sizes (minor axis) to select from are',
...
MLB.NewtDesigner.common = {
    ...
    chBoxBinoscopeLockSecondaryBendingAngleChecked: function () {
...
MLB.NewtDesigner.plotVisualDetection = function () {
    ...
    $.jqplot(common.visualDetectionCalcChartID(), [eps], {
        title: 'Visual Detection Chart for ' + parms.objName + '<br>(values greater than zero are detectable)',
        axes: {
            xaxis: {
...
MLB.NewtDesigner.VisualDetectCalcSingleton = (function () {
    ...
    function createInstance() {return new MLB.calcLib.VisualDetectCalc();
    }
...
MLB.NewtDesigner.calcBinoscopeFocalPlaneToSecondaryDistance = function () {
    ...
    return common.apertureVal() / 2 + common.binoscopeFocalPointPerpendicularOffsetFromEdgeOfPrimaryVal() + common.binoscopeFocalPlaneToTertiaryDistanceVal();
};

//mlb note todo
MLB.NewtDesigner.updateBinoscopeFocalPlaneToSecondaryDistanceBasedOnSecondaryBendingAngle = function () {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        config = MLB.NewtDesigner.config,
        common = MLB.NewtDesigner.common,
        uom = MLB.sharedLib.uom,
        ...
        secondaryToTiertiaryDistance = (aperture / 2 + tertiaryOffsetFromEdgeOfPrimary) / Math.cos((90 - binoscopeSecondaryBendingAngleDeg) * uom.degToRad),
        focalPlaneToSecondaryDistance = secondaryToTiertiaryDistance + focalPlaneToTertiaryDistance;

    common.binoscopeFocalPlaneToSecondaryDistance().val(roundToDecimal(focalPlaneToSecondaryDistance, config.decimalPointsBinoscope));
};
...
MLB.NewtDesigner.updateBinoscopeFocalPlaneToSecondaryDistance = function () {
    var roundToDecimal = MLB.sharedLib.roundToDecimal,
        ...
        shortenedDistanceDueToTertiaryBendingAngle = Math.sin(binoscopeTertiaryBendingAngleDeg * uom.degToRad) * binoscopeFocalPlaneToTertiaryDistance,
        calculatedBinoscopeFocalPlaneToSecondaryDistance = calcBinoscopeFocalPlaneToSecondaryDistance(),
        binoscopeFocalPlaneToSecondaryDistance = roundToDecimal(calculatedBinoscopeFocalPlaneToSecondaryDistance - shortenedDistanceDueToTertiaryBendingAngle, config.decimalPointsBinoscope);

    common.binoscopeFocalPlaneToSecondaryDistance().val(binoscopeFocalPlaneToSecondaryDistance);
};

MLB.NewtDesigner.useSecondaryBendingAngle = function () {
    var common = MLB.NewtDesigner.common;

    return common.chBoxBinoscopeLockSecondaryBendingAngleChecked() && !isNaN(common.binoscopeSecondaryBendingAngleDegVal()) && common.binoscopeSecondaryBendingAngleDegVal() > 0;
};

MLB.NewtDesigner.binoscopeWorkflow = function () {
    var common = MLB.NewtDesigner.common,
        useSecondaryBendingAngle = MLB.NewtDesigner.useSecondaryBendingAngle,
        updateBinoscopeFocalPlaneToSecondaryDistanceBasedOnSecondaryBendingAngle = MLB.NewtDesigner.updateBinoscopeFocalPlaneToSecondaryDistanceBasedOnSecondaryBendingAngle,
        graphBinoscope = MLB.NewtDesigner.graphBinoscope,
        graphBinoscopeFrontView = MLB.NewtDesigner.graphBinoscopeFrontView;

    if (useSecondaryBendingAngle()) {
        updateBinoscopeFocalPlaneToSecondaryDistanceBasedOnSecondaryBendingAngle();
    }
    graphBinoscope();
...
MLB.NewtDesigner.updateFieldsDependentOnApertureSubr = function () {
    ...
    if (useSecondaryBendingAngle()) {
        ...
        // fix rounding issue when binoscopeFocalPlaneToSecondaryDistance is calculated then rounded for display: can round downward to an illegal value so add back in half of the max potential rounding error
        common.binoscopeFocalPlaneToSecondaryDistance().val(roundToDecimal(common.binoscopeFocalPlaneToSecondaryDistanceVal() + 0.0005, config.decimalPointsBinoscope));
    }
...
MLB.NewtDesigner.buildData = function () {
    ...
    return {
        ...
        lockBinoscopeSecondaryBendingAngle: common.chBoxBinoscopeLockSecondaryBendingAngleChecked(),
        binoscopeSecondaryBendingAngleDeg: common.binoscopeSecondaryBendingAngleDeg(),
        binoscopeTertiaryBendingAngleDeg: common.binoscopeTertiaryBendingAngleDeg(),
        IPD: common.IPDVal(),
        binoscopeSecondaryAxisDownwardTiltAngleDeg: common.binoscopeSecondaryAxisDownwardTiltAngleDegVal(),
        CGs: CGs,
...
MLB.NewtDesigner.getDataUpdateUI = function (parsedData) {
    var state = MLB.NewtDesigner.state,
        ...
        updateBinoscopeFocalPlaneToSecondaryDistance = MLB.NewtDesigner.updateBinoscopeFocalPlaneToSecondaryDistance,
        lockBinoscopeSecondaryBendingAngle;
    ...
    lockBinoscopeSecondaryBendingAngle = parsedData.lockBinoscopeSecondaryBendingAngle;
    if (lockBinoscopeSecondaryBendingAngle === undefined) {
        lockBinoscopeSecondaryBendingAngle = false;
    }
    common.chBoxBinoscopeLockSecondaryBendingAngle().prop('checked', lockBinoscopeSecondaryBendingAngle);
...
MLB.NewtDesigner.processURLhash = function () {
    ...
    case jumpTo.movementFriction:
        updateSelectedThenScrollToTop(undefined, config.btnSelectRockerLit, config.designRockertLit, config.designRockerImageLit, jumpTo.movementFriction);
        break;
    case jumpTo.proposedDesign30inch:
        importMelBartels30InchDesign();
        updateSelectedThenScrollToTop(undefined, config.btnSelectLowriderBafflingLit, config.designLowriderBafflingLit, config.designLowriderImageLit, config.uomFocalRatioApertureSharedParmsLit);
        break;
...
$(window).ready(function () {
    var state = MLB.NewtDesigner.state,
        ...
        attemptToUpdateBinoscopeBasedOnTertiaryBendingAngle = MLB.NewtDesigner.attemptToUpdateBinoscopeBasedOnTertiaryBendingAngle,
        updateCurrentlySelectedDesigner = MLB.NewtDesigner.updateCurrentlySelectedDesigner,
    ...
    common.IPD().change(graphBinoscopeFrontView);
    common.binoscopeSecondaryAxisDownwardTiltAngleDeg().change(graphBinoscopeFrontView);
    common.btnUpdateBinoscopeFrontView().click(binoscopeWorkflow);
