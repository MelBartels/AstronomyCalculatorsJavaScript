...
MLB.NewtDesigner.graphBinoscope = () => {
	var roundToDecimal = MLB.sharedLib.roundToDecimal,
		...
		calcFocalPointOffsetFromEdgeOfPrimary = MLB.NewtDesigner.calcFocalPointOffsetFromEdgeOfPrimary,
		adjustScaling = MLB.NewtDesigner.adjustScaling,
	...
	focalPointOffsetFromEdgeOfPrimary = calcFocalPointOffsetFromEdgeOfPrimary(aperture, focalPlaneToSecondaryDistance, focalPlaneToTertiaryDistance, secondaryBendingAngleDeg * uom.degToRad, eyepieceDivergenceAngleDeg * uom.degToRad);
	// get the binoscope model:
	state.binoscopeModel = calcBinoscope(aperture, focalRatio, focalPointOffsetFromEdgeOfPrimary, focalPlaneToTertiaryDistance, focalPlaneToSecondaryDistance, secondaryBendingAngleDeg, eyepieceDivergenceAngleDeg, maxFieldDia);
	// make a local reference
	binoscopeModel = state.binoscopeModel;
	binoscopeModel.aperture = aperture;
	binoscopeModel.focalRatio = focalRatio;
	binoscopeModel.telescopeFocalLength = telescopeFocalLength;
	binoscopeModel.focalPlaneToSecondaryDistance = focalPlaneToSecondaryDistance;
	binoscopeModel.focalPlaneToTertiaryDistance = focalPlaneToTertiaryDistance;
	binoscopeModel.secondaryBendingAngleDeg = secondaryBendingAngleDeg;
	binoscopeModel.eyepieceDivergenceAngleDeg = eyepieceDivergenceAngleDeg;
	binoscopeModel.focalPointOffsetFromEdgeOfPrimary = focalPointOffsetFromEdgeOfPrimary;
	binoscopeModel.focalPointToSecondaryVerticalDistance = focalPointOffsetFromEdgeOfPrimary + aperture / 2;
	...
	// tertiarySizedForP3Field (P3 = final focal plane)
	...
	// P2 (P2 = focal plane produced by the secondary mirror)
	points.P2 = {};
	...
	// outlines full illumination from secondary to focal plane
	...
	// outlines full illumination from secondary to focal plane
	drawLine(context, config.canvasOpticalPathAltStyle, config.canvasLineWidth, points.tertiarySizedForP3Field.upper, points.focalPlane2.top);
	drawLine(context, config.canvasOpticalPathAltStyle, config.canvasLineWidth, points.tertiarySizedForP3Field.lower, points.focalPlane2.bottom);
	...
	drawLine(context, config.canvasOpticalPathStyle, config.canvasLineWidth, points.tertiarySizedForP3.upper, points.focalPlane2.center);
	drawLine(context, config.canvasOpticalPathStyle, config.canvasLineWidth, points.tertiarySizedForP3.lower, points.focalPlane2.center);
	// draw P2
	// outlines full illumination from secondary to focal plane
	drawLine(context, config.canvasGlassStyle, config.canvasLineWidth, points.P2.upper, points.P2.lower);
	...
	*/
	...
	resultsString = 'secondary mirror: tilted at '
			...
			+ roundToDecimal(binoscopeModel.focalPointOffsetFromEdgeOfPrimary, config.decimalPointsDimension)
			+ uomLengthLit
			+ '<br>vertical height: secondary to focal point = '
			+ roundToDecimal(binoscopeModel.focalPointToSecondaryVerticalDistance, config.decimalPointsDimension)
			+ uomLengthLit
			+ '<br>field diameter as set in the diagonal designer = '
			+ roundToDecimal(common.maxFieldDiaDiagVal(), config.decimalPointsDimension)
			+ uomLengthLit
			//+ '<br>equivalent monoscope+binoviewer aperture = '
			...
			+ uomLengthLit
			+ '<br>equivalent monoscope aperture for nebulosity = '
...
MLB.NewtDesigner.graphBinoscopeFrontView = () => {
	var roundToDecimal = MLB.sharedLib.roundToDecimal,
		...
		tertiaryXOffsetFromMirrorCenter,
		tertiaryYOffsetFromMirrorCenter,
		humanFaceWidthUom,
		humanFaceHeightUom,
	...
	// calc
	apertureRadius = binoscopeModel.aperture / 2;
	binoscopeSecondaryAxisDownwardTiltAngleRad = binoscopeSecondaryAxisDownwardTiltAngleDeg * uom.degToRad;
	focalPointXOffsetFromMirrorCenter = Math.cos(binoscopeSecondaryAxisDownwardTiltAngleRad) * (binoscopeModel.focalPointToSecondaryVerticalDistance);
	focalPointYOffsetFromMirrorCenter = Math.sin(binoscopeSecondaryAxisDownwardTiltAngleRad) * (binoscopeModel.focalPointToSecondaryVerticalDistance);
	primaryMirrorsSeparation = 2 * focalPointXOffsetFromMirrorCenter - binoscopeModel.aperture + common.convertMmToUom(IPD);
	tertiaryXOffsetFromMirrorCenter = Math.cos(binoscopeSecondaryAxisDownwardTiltAngleRad) * (binoscopeModel.secondaryToTertiaryVerticalLength);
	tertiaryYOffsetFromMirrorCenter = Math.sin(binoscopeSecondaryAxisDownwardTiltAngleRad) * (binoscopeModel.secondaryToTertiaryVerticalLength);

	eyepieceTipdownFromSecToTertTipdownAndTertBend = calcEyepieceTipdownFromSecToTertTipdownAndTertBend(eyepieceDivergenceAngleDeg, binoscopeSecondaryAxisDownwardTiltAngleDeg);
	...
	// scaling
	scalingFactor = adjustScaling(state.scaling.scalingFactor, state.sliderBinoscopeScaleValue);
	scaling.aperture = scalingFactor * binoscopeModel.aperture;
	...
	scaling.tertiaryXOffsetFromMirrorCenter = scalingFactor * tertiaryXOffsetFromMirrorCenter;
	scaling.tertiaryYOffsetFromMirrorCenter = scalingFactor * tertiaryYOffsetFromMirrorCenter;
	...
	points.leftTertiaryFocalPoint = point(points.leftPrimaryMirrorCenter.x + scaling.tertiaryXOffsetFromMirrorCenter, points.leftPrimaryMirrorCenter.y + scaling.tertiaryYOffsetFromMirrorCenter);
	points.rightTertiaryFocalPoint = point(points.rightPrimaryMirrorCenter.x - scaling.tertiaryXOffsetFromMirrorCenter, points.rightPrimaryMirrorCenter.y + scaling.tertiaryYOffsetFromMirrorCenter);
	...
	fillCircle(context, points.leftFocalPoint, scaling.humanEyeRadiusUom, config.canvasHumanEyeStyle);
	fillCircle(context, points.rightFocalPoint, scaling.humanEyeRadiusUom, config.canvasHumanEyeStyle);
	...
	// tertiaries
	drawCircle(context, points.leftTertiaryFocalPoint, scaling.tertiaryRadius, config.canvasLineWidth, config.canvasStructureStyle);
	drawCircle(context, points.rightTertiaryFocalPoint, scaling.tertiaryRadius, config.canvasLineWidth, config.canvasStructureStyle);
	// connect secondaries to focal points / eyes
	...
	resultsString = 'primary mirror edges are separated by: '
			...
			+ uomLengthLit
			+ ' (note that eyepiece length will increase mirror separation if eyepiece divergence angle > 0)'
