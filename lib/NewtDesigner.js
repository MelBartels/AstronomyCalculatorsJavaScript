...
MLB.NewtDesigner.setScalingFactor = function () {
    ...
    state.scaling = baffleScalingFactor(config.canvasWidth, config.canvasHeight, aperture * focalRatio * 1.5, aperture * 2.5, config.canvasBorder);
};
...
MLB.NewtDesigner.updateTelescopeResults = function () {
    ...
    mirrorSupportPoints = calcMinimumMirrorSupportPoints(common.convertUomToInches(apertureInches));
    ...
    common.telescopeDesignerResultsLabel().html('telescope focal length = '
            ...
            + uomLengthLit
            + '<br><br>mirror support points required = '
            + mirrorSupportPoints
            + '<br>mirror sagitta = '
            + roundToDecimal(sagitta, config.decimalPointsDimension)
            + uomLengthLit
